...
def area_sentence(stv, label, provisional, devs, snap, room_name):
    ...
    if stv == "off":
        ...
        return ns or "Off"
    if stv.startswith("watch_"):
        # A COMMANDED activity leads the sentence (Dave, 3 Aug: "if an
        # activity is used to turn on a device then it should advise
        # that" — we commanded it, power confirmed it, so we KNOW what's
        # on). The content info rides behind it when the source reports
        # any; when it doesn't, the activity alone is still the truth —
        # never a fall to bare monitored facts on a commanded room.
        base = _state_sentence(stv, label, devs, snap, room_name, home=False)
        info = playing_info(devs, snap)
        return base + (" · " + info) if info else base
    info = playing_info(devs, snap)
    if info:
        return info
