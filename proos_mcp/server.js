...
const RECOVER_TOOL = {
  name: 'recover_device',
  description: "Recover a wedged device using its configured ProOS recovery — an integration reload by default, or a smart-plug power-cycle if the installer assigned one (the SAME self-heal ProOS runs automatically). Use when awareness shows a device is faulted/wedged (answering the network but lost to ProOS) and you want to restore it now. Non-destructive.",
...
// CONSENT — in-chat access control. Every tier gets the tool, but the executor enforces
// WHO may change WHICH grant: the homeowner controls installer_access, the installer
// controls tech_access (Protech vendor support), tech/owner may set either.
const CONSENT_TOOL = {
  name: 'set_access',
  description: "Grant or revoke ProOS access when the person asks. installer_access = whether the homeowner lets their installer manage this home (the HOMEOWNER controls this). tech_access = whether Protech vendor support may access it (the INSTALLER controls this). Set only the grant the caller is entitled to change. Examples: homeowner 'turn off installer access' -> installer_access:false; installer 'enable Protech support' -> tech_access:true.",
  input_schema: { type: 'object', properties: { installer_access: { type: 'boolean' }, tech_access: { type: 'boolean' } } }
};
...
async function executeTool(name, input, ctx = {}) {
  ...
  switch (name) {
    ...
    case 'recover_device': {
      ...
      // Prefer Core's recovery — it runs the device's CONFIGURED method (reload, or a
      // smart-plug power-cycle when the installer assigned one), same as auto self-heal.
      const r = await corePost('/watchers/recover', ctx.token, { entity: eid });
      if (r && typeof r.ok !== 'undefined') {
        const how = r.method === 'power_cycle' ? 'a power cycle' : 'an integration reload';
        return r.ok
          ? { success: true, method: r.method, message: `Recovered ${eid} via ${how} — give it a moment to come back.` }
          : { success: false, method: r.method, message: r.error || `Recovery couldn't run for ${eid}.` };
      }
      // Fallback if Core is unreachable: direct integration reload.
      let entry = '';
      ...
      return { success: true, method: 'reload', message: `Reloaded the connection for ${eid} — give it a moment to come back.` };
    }
    // ── IN-CHAT CONSENT (every tier; per-grant gating enforced here) ──
    case 'set_access': {
      const lvl = ctx.level || 'user';
      const want = {};
      if (typeof input.installer_access === 'boolean') want.installer_access = input.installer_access;
      if (typeof input.tech_access === 'boolean') want.tech_access = input.tech_access;
      if (!Object.keys(want).length) return { success: false, message: 'Nothing to change — specify installer_access and/or tech_access.' };
      // Homeowner controls installer access; installer controls tech access; tech/owner both.
      if ('installer_access' in want && !(lvl === 'user' || lvl === 'tech'))
        return { success: false, message: "Installer access is the homeowner's switch — an installer can't change it." };
      if ('tech_access' in want && lvl === 'user')
        return { success: false, message: 'Turning Protech tech support on or off is done by the installer, not the homeowner.' };
      const r = await corePost('/consent', ctx.token, want);
      if (!r) return { success: false, message: "Couldn't update access right now." };
      const eff = r.effective || {};
      return { success: true, installer_access: eff.installer, tech_access: eff.tech, message: 'Access updated.' };
    }
    default:
...
INSTALLER ACCESS (you're in control of this)
- You decide whether your installer can access your home. If you ask to turn installer access on or off ("let my installer in", "turn off installer access"), do it with set_access (installer_access). Confirm plainly — "Done, your installer's access is off." The ACCESS CONSENT line above tells you the current state.
- You don't manage Protech tech-support access — that's set by your installer. If asked, say your installer handles that.
...
- When a device is wedged — reachable on the network but lost to ProOS — restore it with recover_device. It runs the device's configured recovery: an integration reload, or a smart-plug power-cycle if one's assigned. Diagnose first, then recover, then report what you found and what you did.

TECH-SUPPORT ACCESS (you control this)
- You decide whether Protech vendor tech support can access this home. Enable or disable it with set_access (tech_access) when asked — "enable Protech support", "cut tech support access". The homeowner controls YOUR installer access; you can't grant yourself that. The ACCESS CONSENT line shows current state.
...
AWARENESS & SELF-HEAL
- get_home_health / get_room_health / get_awareness_history give you ProOS's live status and the fault/recovery record; recover_device runs a device's configured recovery on demand — reload or smart-plug power-cycle — the same path as automatic self-heal. Root-cause first: cite the awareness verdict or the history event that shows WHY, then recover. Cross-check against read_error_log / list_integrations when the cause isn't obvious.
- You can view and set both access grants with set_access (installer_access, tech_access) when support requires it.
...
const INSTALLER_TOOLS = [...HA_TOOLS, ...AWARENESS_TOOLS, RECOVER_TOOL, CONSENT_TOOL];
const HA_TOOLS_TECH = [...INSTALLER_TOOLS, ...TECH_TOOLS_EXTRA];

// User mode gets a restricted toolset — control + scenes + awareness READ + consent (their own
// installer-access switch), but NO automation/registry writes and NO self-heal. Explicit
// allow-list (a new tool is locked out by default).
const USER_TOOL_NAMES = ['call_service', 'get_states', 'create_scene', 'process_conversation', 'flag_for_installer'];
const USER_TOOLS = [...HA_TOOLS.filter(t => USER_TOOL_NAMES.includes(t.name)), ...AWARENESS_TOOLS, CONSENT_TOOL];
...
app.post('/assist', async (req, res) => {
  ...
  try {
    ...
    const c = (who && who.consent) || {};
    const consentLine = `ACCESS CONSENT: installer access ${c.installer ? 'ON' : 'OFF'} · Protech tech support ${c.tech ? 'ON' : 'OFF'}`;
