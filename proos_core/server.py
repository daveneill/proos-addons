...
# Avatars are written as a native /local/ file (HA serves <config>/www at /local/,
# the same mechanism provision.py uses for the dashboards). This avoids HA's
# multipart image-upload endpoint, whose boundary is stripped by the Supervisor
# proxy (see auth.py's note that the proxy is unreliable). The person's picture
# then points at this stable URL — still 100% native HA person data.
_HA_CONFIG_DIR = "/homeassistant" if os.path.isdir("/homeassistant") else "/config"
_AVATAR_SUBDIR = "proos_avatars"


def _avatar_paths(user_id: str):
    safe = re.sub(r"[^A-Za-z0-9_-]", "", user_id or "") or "user"
    folder = os.path.join(_HA_CONFIG_DIR, "www", _AVATAR_SUBDIR)
    return folder, os.path.join(folder, safe + ".jpg"), safe


def _save_avatar_file(user_id: str, data: bytes) -> str:
    """Write the avatar JPEG into <ha_config>/www/proos_avatars and return the
    /local/ URL HA serves it at (cache-busted so the new image shows at once)."""
    folder, path, safe = _avatar_paths(user_id)
    os.makedirs(folder, exist_ok=True)
    tmp = path + ".tmp"
    with open(tmp, "wb") as fh:
        fh.write(data)
    os.replace(tmp, path)
    return "/local/%s/%s.jpg?v=%d" % (_AVATAR_SUBDIR, safe, int(time.time()))


def _delete_avatar_file(user_id: str) -> None:
    _folder, path, _safe = _avatar_paths(user_id)
    try:
        os.remove(path)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        parts = [p for p in self.path.split("?")[0].strip("/").split("/") if p]
        try:
            # ── Avatar write-shim: set/clear a user's picture on their native HA
            # person. /me/picture is self-service (any signed-in user, incl. a
            # non-admin homeowner); /users/<id>/picture lets an admin/tech set
            # someone else's. Raw image body = set; empty body = remove. This is
            # the ONLY privileged step - reads happen natively from HA states.
            if parts == ["me", "picture"] or (len(parts) == 3 and parts[0] == "users" and parts[2] == "picture"):
                ...
                try:
                    if not raw:
                        _delete_avatar_file(target)
                        return self._send(200, users.clear_picture(_ws_call, target))
                    url = _save_avatar_file(target, raw)
                    return self._send(200, users.set_picture(_ws_call, target, url))
                except Exception as e:
                    return self._send(500, {"error": str(e)})
