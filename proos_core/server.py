...
def _assist_speak(where, reply):
    ...
    # The chosen DEFAULT VOICE rides the call — only with its own engine
    # (a voice belongs to the engine that offers it; sending it to a
    # fallback engine would be an invented capability).
    _vs = _voice_load()
    if _vs.get("voice") and _vs.get("engine") == eng:
        data["options"] = {"voice": _vs["voice"]}
        out["voice"] = _vs["voice"]
    try:
        ...
        try:
            ...
            if parts == ["voice", "status"]:
                ...
                if eng:
                    ...
                    for _l2 in langs:
                        try:
                            _res = _ws_call("tts/engine/voices",
                                            engine_id=eng, language=_l2) or {}
                            _vv = _res.get("voices") or []
                            if _vv:
                                out["voices"] = _vv
                                out["voices_language"] = _l2
                                break
                        except Exception as _e:                  # noqa: BLE001
                            _last = str(_e)[:150]
                    if not out["voices"]:
                        out["why"] = ("the engine lists no voices"
                                      + ((" — " + _last) if _last else ""))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["voice", "default"]:
                ...
                if "voice" in b:
                    vs["voice"] = str(b.get("voice")) if b.get("voice") else None
                _voice_save(vs)
                if _journal_mod is not None:
                    try:
