...
def _watch_rediscover_loop():
    ...
    while True:
        ...
        # create-if-absent + self-heal: it fills gaps and refreshes UNEDITED scripts
        # to the current room (e.g. drops a removed source's step) while never
        # clobbering an installer's edits.
        try:
            _sres = sync.sync_all(_client)
            _tot = _sres.get("totals") or {}
            _created = _tot.get("created", 0)
            _refreshed = _tot.get("refreshed", 0)
            if _created or _refreshed:
                print(f"  sync · self-heal created {_created}, refreshed {_refreshed} activity script(s)", flush=True)
        except Exception as e:
