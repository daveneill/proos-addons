...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if len(parts) == 4 and parts[0] == "apps" and parts[1] == "art":
                # PRODUCT artwork service (public — loaded by <img>/CSS):
                #   /apps/art/tile/<slug>.png  bundled exact tile (ships with Core)
                #   /apps/art/sq/<slug>.png?name=<app>  real icon, server-cached
                if not _appart:
                    return self._send(404, {"error": "artwork unavailable"})
                fn = re.sub(r"\.(png|svg)$", "", parts[3])
                nm = (parse_qs(urlparse(self.path).query).get("name") or [fn])[0]
                p = None
                if parts[2] == "tile":
                    p = _appart.tile_path(fn)
                elif parts[2] == "sq":
                    p = _appart.sq_path(nm)
                elif parts[2] == "logo":
                    # genuine brand logo file (Wikimedia Commons), auto-resolved
                    p = _appart.logo_path(nm)
                if not p:
