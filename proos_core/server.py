...
def _reload_integration(entity, action):
    """Watcher recovery action (SAFE): reload the HA config entry behind an
    entity. Never touches the device -- it only restarts the integration that
    wedged. Resolves entity -> config_entry_id via the entity registry, then
    reloads. Returns True on success; the Watcher audits the outcome."""
    if action != "reload_integration":
        return False
    try:
        reg = _ws_call("config/entity_registry/list")
        rows = reg.get("result", reg) if isinstance(reg, dict) else reg
        entry_id = None
        for e in (rows or []):
            if e.get("entity_id") == entity:
                ...
                break
        if not entry_id:
            print(f"[watcher] recovery: no config entry for {entity}", flush=True)
            return False
        res = _ws_call("config_entries/reload", entry_id=entry_id)
        ok = True if res is None else (
            res.get("success", True) if isinstance(res, dict) else True)
        print(f"[watcher] recovery: reloaded {entity} (entry {entry_id}) ok={ok}", flush=True)
        return bool(ok)
    except Exception as e:
        print(f"[watcher] recovery reload failed for {entity}: {e}", flush=True)
        return False
...
def _oauth_run(domain: str, session_id: str):
    ...
    def _on_url(url):
        with _OAUTH_LOCK:
            ...
            if j is not None:
                ...
                j["ts"] = time.time()
    try:
        entries = _ma.run_provider_auth(domain, session_id, _on_url)
        with _OAUTH_LOCK:
            j = _OAUTH_JOBS.get(session_id)
            if j is not None:
                j["entries"] = entries
    ...
    def do_POST(self):
        ...
        try:
            ...
            if len(parts) == 4 and parts[:2] == ["music", "providers"] and parts[3] == "oauth":
                ...
                with _OAUTH_LOCK:
                    ...
                    if job is None or job.get("status") == "error":
                        ...
                        _OAUTH_JOBS[sid] = job
                        threading.Thread(target=_oauth_run, args=(domain, sid),
            ...
            if parts == ["terminal"]:
                ...
                if not (terminal and users and u and (users.is_tech(u.get("id")) or u.get("is_owner"))):
                    return self._send(403, {"error": "tech access required"})
                return self._send(200, terminal.run(self._body().get("command", ""), user=(u.get("name") or u.get("id"))))
            if parts == ["navconfig"]:
                u = getattr(self, "_user", None)
            ...
            if parts == ["watchers", "test-recovery"]:
                ...
                try:
                    if entry_id:
                        ...
                        ok = True if res is None else (
                            res.get("success", True) if isinstance(res, dict) else True)
                        print(f"[watcher] test-recovery: reloaded entry {entry_id} ok={ok}", flush=True)
                    else:
