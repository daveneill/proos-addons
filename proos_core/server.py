    ...
    def do_POST(self):
        ...
        try:
            ...
            if len(parts) == 4 and parts[:2] == ["music", "providers"] and parts[3] == "save":
                b = self._body()
                # SESSION-FIRST: after an OAuth flow, MA holds the auth result
                # (refresh_token…) server-side against the SESSION'S socket and
                # masks secrets in everything it returns — a save on any other
                # channel arrives tokenless and KeyErrors. When the dialog's
                # session connection is alive, the save MUST ride it.
                _sid = b.get("session_id")
                if _sid and _ma.has_session(_sid):
                    try:
                        return self._send(200, _ma.save_provider(
                            unquote(parts[2]), b.get("values") or {},
                            b.get("instance_id"), session_id=_sid))
                    except Exception as e:  # noqa: BLE001
                        return self._send(502, {"error": str(e)})
