...
def factory_reset(clear_proos_data=False, keep_auth=False):
    ...
    # Streaming-provider logins live in the MUSIC SERVER's own data — no
    # HA-side wipe reaches them, and they are the previous home's accounts
    # (observed live: Spotify/Apple Music came back enabled after a reset).
    # Runs EARLY, while the current owner still exists to authorise the
    # ingress channel. Best-effort: a missing/unlinked MA is a no-op.
    try:
        if _ma:
        ...
        out["errors"].append("music providers: %s" % e)
