...
def _atv_ledger():
    ...
    HA's copy can be lost — an entry gets removed and re-paired, a restore rolls
    back .storage. Ours lives in the add-on's data volume, so a box never has to
    be re-taught. Shape: {entry_id: {package_id: name}}."""
    try:
...
def _atv_entry_for(entity_id):
    """(entry_id, apps_dict) for an Android TV media_player, or (None, {}).

    Reads the entry store for the current options — a read is safe; only writes
    have to go through the flow."""
    try:
        for e in (_client.entity_registry() or []):
            if e.get("entity_id") == entity_id:
                eid = e.get("config_entry_id")
                if not eid:
                    return None, {}
                with open(_HA_STORAGE_ENTRIES) as f:
                    d = json.load(f)
                for en in d["data"]["entries"]:
                    if en.get("entry_id") == eid:
                        if en.get("domain") not in _ATV_DOMAINS:
                            return None, {}
                        return eid, dict((en.get("options") or {}).get("apps") or {})
                return None, {}
    except Exception:
        pass
    return None, {}
...
def _atv_enable_ime(entry_id):
    """Current enable_ime for the entry — the options flow requires it on every
    init submit, and we must not silently flip the installer's setting."""
    try:
        with open(_HA_STORAGE_ENTRIES) as f:
            d = json.load(f)
        for en in d["data"]["entries"]:
            if en.get("entry_id") == entry_id:
                return bool((en.get("options") or {}).get("enable_ime", True))
    except Exception:
        pass
    return True
...
def _atv_flow_apps(entry_id, add=None, remove=None):
    """Add/remove apps on an Android TV entry through HA's options flow.

    add: {package_id: name}; remove: iterable of package_id. One flow, one save,
    entry reloads itself. Raises with the step that failed so the caller can say
    something useful instead of failing silently."""
    add = {k: v for k, v in (add or {}).items() if k and "." in k}
    remove = [p for p in (remove or []) if p]
    if not add and not remove:
        return 0
    ime = _atv_enable_ime(entry_id)
    ...
    try:
        ...
        for pkg, name in add.items():
            ...
            if s.get("step_id") != "apps":
                raise RuntimeError("could not open the add-app form (%r)" % s.get("step_id"))
            s = _client._req("POST", url, {"app_id": pkg, "app_name": name,
                                           "app_icon": ""}) or {}
            if s.get("step_id") != "init":
                raise RuntimeError("%s was rejected (%s)" % (pkg, s.get("errors") or s.get("step_id")))
        for pkg in remove:
            s = _client._req("POST", url, {"apps": pkg, "enable_ime": ime}) or {}
            if s.get("step_id") != "apps":
                raise RuntimeError("could not open %s to remove it" % pkg)
            s = _client._req("POST", url, {"app_delete": True}) or {}
            if s.get("step_id") != "init":
                raise RuntimeError("%s could not be removed" % pkg)
        done = _client._req("POST", url, {"enable_ime": ime}) or {}
        if done.get("type") != "create_entry":
            raise RuntimeError("HA did not save the app list (%r)" % done.get("type"))
    except Exception:
...
def _atv_write_apps(entity_id, apps):
    """Make the entry's app list exactly `apps` ([{id, name}, ...])."""
    entry_id, cur = _atv_entry_for(entity_id)
    if not entry_id:
    ...
    # Record what we intend BEFORE asking HA, so a failed push is still learned
    # and gets replayed on the next pass.
    _atv_ledger_put(entry_id, want)
    add = {p: n for p, n in want.items()
           if p not in cur or (cur[p] or {}).get("app_name") != n}
    remove = [p for p in cur if p not in want]
    try:
        _atv_flow_apps(entry_id, add=add, remove=remove)
    except Exception as e:  # noqa: BLE001
        return {"error": str(e), "count": len(cur)}
    print("  [androidtv] %s: %d apps saved (+%d/-%d)"
          % (entity_id, len(want), len(add), len(remove)), flush=True)
...
def _atv_learn_pass():
    """One sweep per Android TV media_player: capture the app on screen, and
    restore anything ProOS has learned before that HA no longer has."""
    try:
    ...
    for e in ents:
        eid = e.get("entity_id")
        try:
            entry_id, apps = _atv_entry_for(eid)
            if not entry_id:
                continue
            want = {k: (v or {}).get("app_name") or k for k, v in apps.items()}
            # Replay: everything we ever learned for this box belongs in the list.
            known = ledger.get(entry_id) or {}
            missing = {p: n for p, n in known.items() if p not in want}
            want.update(missing)
            # Capture: whatever is open right now.
            ...
            if res.get("ok"):
                if new:
                    print("  [androidtv] learned %s on %s (%d apps)"
                          % (new, eid, res.get("count", 0)), flush=True)
                if missing:
                    print("  [androidtv] restored %d saved app(s) on %s"
                          % (len(missing), eid), flush=True)
            else:
                print("  [androidtv] %s: %s" % (eid, res.get("error")), flush=True)
        except Exception:
            continue
    ...
    def do_GET(self):
        ...
        try:
            ...
            if len(parts) == 4 and parts[0] == "apps" and parts[1] == "art" and parts[2] == "tile":
                ...
                p = _appart.tile_path(re.sub(r"\.png$", "", parts[3]))
                if not p:
            ...
            if parts == ["androidtv", "apps"]:
                ...
                try:
                    ...
                    live = {}
                # Anything ProOS has learned that HA's list is missing — shown so
                # a lost list is visible rather than mysterious.
                known = (_atv_ledger().get(entry_id) or {})
                return self._send(200, {
                    "entity_id": eid,
                    "apps": [{"id": k, "name": (v or {}).get("app_name") or k}
                             for k, v in sorted(apps.items(), key=lambda kv: ((kv[1] or {}).get("app_name") or kv[0]).lower())],
                    "pending": [{"id": k, "name": v} for k, v in sorted(known.items(), key=lambda kv: kv[1].lower())
                                if k not in apps],
                    "live": live})
            if parts == ["apps", "art", "list"]:
                ...
                if not (u and (u.get("is_owner") or (users and users.is_tech(u.get("id"))))):
                    return self._send(403, {"error": "tech access required"})
                return self._send(200, _appart.list_tiles() if _appart else {"tiles": []})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["apps", "art", "upload"]:
                ...
                b = self._body() or {}
                res = _appart.save_upload(b.get("name") or b.get("slug") or "", b.get("image") or "")
                return self._send(400 if res.get("error") else 200, res)
            ...
            if parts == ["assist", "chat"]:
                ...
                return self._send(200, _assist.chat(
                    ...
                    home_name=_ASSIST_HOME_NAME, ma=_ma))
