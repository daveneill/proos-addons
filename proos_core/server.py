...
def _ma_discovery():
    """(host, port, auth_token) for the MA server, from the Supervisor discovery
    record -- the same token HA's own integration uses. None off add-on / not found."""
    if not os.environ.get("SUPERVISOR_TOKEN"):
        return None
    try:
        recs = _sv("GET", "/discovery").get("discovery", [])
    except Exception:
        return None
    for d in recs:
...
def main():
    ...
    _ma = MaCommissioner(_ma_discovery)
