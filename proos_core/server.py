    ...
    def do_GET(self):
        ...
        try:
            ...
            if len(parts) == 3 and parts[0] == "rooms" and parts[2] == "activities":
                ...
                # Re-read the room from its (committed) record on every request so the
                # dashboard reflects a pro commit IMMEDIATELY. Controllers are cached and
                # built once, so without this the activity list is frozen at first sight
                # and never picks up a re-commit -- the "pro updates, dashboard doesn't"
                # bug. Best-effort: a refresh failure falls back to the last good list.
                try:
                    ctrl.refresh()
                except Exception:
                    pass
