...
def _quarantine_loop():
    ...
    while True:
        ...
        # No-auto-integration: disable Sonos/Cast/etc. that HA auto-configured via discovery
        # (device integrations only — HA infra like matter/thread is left alone) until the
        # installer adds them on purpose. Disable, not delete, so HA won't re-discover-loop.
        try:
            if project is not None and _opt("auto_integration_reap", True):
                res = project.reap_auto_integrations(_client)
                dz = (res or {}).get("disabled") or []
                if dz:
                    print(f"  no-auto-integration · disabled auto-configured: {', '.join(dz)}", flush=True)
        except Exception as e:
            print(f"  no-auto-integration · loop error: {e}", flush=True)
