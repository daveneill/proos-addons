    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["shield", "sync"]:
                # Certified capability lives in the ProOS Android TV integration
                # (it owns the pairing, the ADB session and the app list). Core
                # just asks it to sync — no ADB, no storage reads here.
                u = self._user
                if not (u and (u.get("is_admin") or u.get("is_owner")
                               or (users and users.is_tech(u.get("id"))))):
                    return self._send(403, {"error": "installer access required"})
                ...
                if not eid:
                    return self._send(400, {"error": "entity_id required"})
                data = {"entity_id": eid}
                if (b.get("host") or "").strip():
                    data["host"] = b["host"].strip()
                try:
                    _client._req("POST", "/api/services/proos_androidtv/sync_apps", data)
                except Exception as e:  # noqa: BLE001
                    msg = str(e)
                    if "not found" in msg.lower() or "404" in msg:
                        msg = ("the ProOS Android TV integration isn't installed on this "
                               "system — add it to custom_components and restart HA")
                    return self._send(400, {"error": msg})
                # Read back what the device now exposes, so Pro can report truth.
                apps = []
                try:
                    st = _client._req("GET", "/api/states/%s" % eid) or {}
                    apps = [a for a in ((st.get("attributes") or {}).get("source_list") or [])
                            if isinstance(a, str)]
                except Exception:
                    apps = []
                return self._send(200, {"ok": True, "count": len(apps),
                                        "apps": [{"name": a} for a in apps]})
