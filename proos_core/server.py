...
try:
    from proos import handover
except Exception:  # optional - QR handover must not stop boot
    handover = None
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if parts == ["handover", "status"]:
                # Installer polls this to see whether the homeowner has scanned
                # the QR yet. Installer/owner gated (a code is not a secret to leak
                # here — status never returns the code or credentials).
                u = getattr(self, "_user", None)
                if not (handover and users and u and (users.is_tech(u.get("id")) or u.get("is_owner"))):
                    return self._send(403, {"error": "installer access required"})
                qs = parse_qs(urlparse(self.path).query)
                return self._send(200, handover.status((qs.get("code", [""])[0])))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["handover", "create"]:
                # Installer/owner issues a one-time claim code for a homeowner they
                # just created. Bound to that homeowner's credentials; short-lived.
                if not handover:
                    return self._send(503, {"error": "handover module not loaded"})
                u = getattr(self, "_user", None)
                if not (users and u and (users.is_tech(u.get("id")) or u.get("is_owner"))):
                    return self._send(403, {"error": "installer access required"})
                b = self._body() or {}
                try:
                    return self._send(200, handover.create(
                        (b.get("username") or ""), (b.get("password") or ""),
                        name=b.get("name"), ttl=b.get("ttl")))
                except Exception as e:
                    return self._send(400, {"error": str(e)})
            if parts == ["handover", "claim"]:
                # PUBLIC: the homeowner has no token yet — the one-time code IS the
                # credential. Exchanges it for a token and burns it.
                if not handover:
                    return self._send(503, {"error": "handover module not loaded"})
                b = self._body() or {}
                try:
                    return self._send(200, handover.claim(b.get("code") or ""))
                except handover.HandoverError as e:
                    code = 429 if e.kind == "throttled" else 400
                    return self._send(code, {"error": str(e), "kind": e.kind})
                except Exception as e:
                    return self._send(400, {"error": str(e)})
            if parts == ["handover", "revoke"]:
                if not handover:
                    return self._send(503, {"error": "handover module not loaded"})
                u = getattr(self, "_user", None)
                if not (users and u and (users.is_tech(u.get("id")) or u.get("is_owner"))):
                    return self._send(403, {"error": "installer access required"})
                b = self._body() or {}
                return self._send(200, handover.revoke(b.get("code") or ""))
            if parts == ["terminal"]:
                u = getattr(self, "_user", None)
                if not (terminal and users and u and (users.is_tech(u.get("id")) or u.get("is_owner"))):
                    return self._send(403, {"error": "tech access required"})
