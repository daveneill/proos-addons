...
def _homekit_brand(name="ProOS", title="ProOS Apple Home", entry_id=None):
    ...
    auto-generated 'HASS Bridge' name. HA's API can't change a bridge's name
    (config_entries/update only touches the title), so Core rewrites the name in
    the config-entries store in place; the caller then restarts HA so the bridge
    re-advertises under the new name. Do this while the bridge is UNPAIRED —
    renaming after pairing forces a re-pair.

    Needs homeassistant_config:rw in config.yaml. Safe + idempotent: backs up,
    validates the rewritten JSON, and atomically replaces the file. Returns the
    number of homekit entries renamed (0 if already branded)."""
...
# installer exposes nothing: an include filter that matches no real entity
        ...
        try:
            ...
            if parts == ["homekit", "brand"]:
                ...
                try:
                    ...
                    return self._send(500, {"error": str(e)})
                do_restart = bool(renamed) and body.get("restart", True)
