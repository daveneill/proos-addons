    ...
    def do_GET(self):
        ...
        try:
            ...
            if len(parts) == 3 and parts[0] == "rooms" and parts[2] == "activities":
                ...
                # list_activities() already snapshots LIVE state for the verdicts on every
                # call, so this read stays fresh. The room's cluster/activity SET only
                # changes on a commit, and we refresh the controller there (POST /project)
                # -- NOT on every poll, which was hammering Core's single-threaded server
                # with a full cluster rebuild per 5s dashboard poll.
        ...
        try:
            ...
            if parts and parts[0] == "project":
                ...
                if parts == ["project", "suggest"]:
                    # No‑auto‑room: before scanning, drop any newly‑added device HA parked in
                    # a room (by name‑match) into Unassigned, so nothing looks placed until the
                    # installer places it. One‑time baseline + once‑per‑device (see project.py);
                    # on by default, disable with option auto_room_quarantine:false.
                    if _opt("auto_room_quarantine", True):
                        try:
                            project.quarantine_new_devices(_client)
                        except Exception:
                            pass
                ...
                if parts == ["project", "activities", "regenerate"]:
                    ...
                    b = self._body() or {}
                    return self._send(200, project.regenerate_activity(_client, project.load(), b.get("area"), b.get("object_id")))
                if parts == ["project", "assign"]:
                    # Pull an unassigned AV device into a room: set its HA area so the next
                    # suggest picks it up. {entity_id, area_id}.
                    b = self._body() or {}
                    eid, aid = b.get("entity_id"), b.get("area_id")
                    if not eid or not aid:
                        return self._send(400, {"error": "entity_id and area_id required"})
                    try:
                        _client.set_entity_area(eid, aid)
                        return self._send(200, {"ok": True, "entity_id": eid, "area_id": aid})
                    except Exception as e:
