...
def _oauth_run(domain: str, session_id: str, action: str = "auth"):
    ...
    def _on_url(url):
        with _OAUTH_LOCK:
            ...
            if j is not None:
                ...
                j["ts"] = time.time()
    try:
        entries = _ma.run_provider_auth(domain, session_id, _on_url, action=action)
        with _OAUTH_LOCK:
            j = _OAUTH_JOBS.get(session_id)
            if j is not None:
                j["entries"] = entries
            ...
            if len(parts) == 4 and parts[:2] == ["music", "providers"] and parts[3] == "oauth":
                ...
                with _OAUTH_LOCK:
                    ...
                    if job is None or job.get("status") == "error":
                        ...
                        _OAUTH_JOBS[sid] = job
                        threading.Thread(target=_oauth_run, args=(domain, sid, action),
