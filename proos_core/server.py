...
def apply_auto_reachability():
    ...
    _wit = sum(1 for sp in merged.values() if _rmod.is_witness(sp))
    print(f"  reachability · {len(auto)} auto + {len(manual)} manual = "
          f"{len(merged)} device{'' if len(merged)==1 else 's'}: {_wit} with a platform "
          f"witness (tracker/ping), {len(merged) - _wit} address only (single-signal)",
          flush=True)
