...
# (app_id), so an installer opens each app once with the remote and Pro captures
# it. Written straight into the entry store (same proven path as the HomeKit
# filter — the options flow is multi-step and brittle), then the entry reloads.
            ...
            if e.get("entity_id") == entity_id:
                ...
                for en in d["data"]["entries"]:
                    if en.get("entry_id") == eid:
                        if en.get("domain") not in ("androidtv_remote", "androidtv"):
                            return None, {}
...
def _atv_write_apps(entity_id, apps):
    """Replace the entry's app list with {package_id: {app_name}} and reload."""
    entry_id, _cur = _atv_entry_for(entity_id)
    if not entry_id:
    ...
    for a in (apps or []):
        ...
        if not pkg or "." not in pkg:
            continue
        nm = str((a or {}).get("name") or "").strip() or pkg.split(".")[-1].title()
        clean[pkg] = {"app_name": nm, "app_icon": ""}
    with open(_HA_STORAGE_ENTRIES) as f:
        d = json.load(f)
    total = len(d["data"]["entries"])
    hit = False
    for en in d["data"]["entries"]:
        if en.get("entry_id") == entry_id:
            opts = dict(en.get("options") or {})
            opts["apps"] = clean
            en["options"] = opts
            hit = True
            break
    ...
    with open(tmp, "w") as f:
        json.dump(d, f)
    with open(tmp) as f:
        json.load(f)                      # prove it parses before swapping
    assert len(d["data"]["entries"]) == total
    os.replace(tmp, _HA_STORAGE_ENTRIES)
    try:
        _client._req("POST", "/api/config/config_entries/entry/%s/reload" % entry_id, {})
    except Exception as e:  # noqa: BLE001
        return {"ok": True, "count": len(clean), "note": "saved — restart HA to apply (%s)" % e}
    print("  [androidtv] %s: %d apps saved" % (entity_id, len(clean)), flush=True)
...
def _atv_learn_pass():
    """One sweep: for each Android TV media_player, if the app on screen isn't
    in its saved list yet, add it."""
    try:
        ents = [e for e in (_client.entity_registry() or [])
                if (e.get("entity_id") or "").startswith("media_player.")
                and e.get("platform") in ("androidtv_remote", "androidtv")]
    except Exception:
        return
    for e in ents:
        eid = e.get("entity_id")
        try:
            ...
            if not pkg or "." not in pkg or _ATV_SKIP.search(pkg):
                continue
            entry_id, apps = _atv_entry_for(eid)
            if not entry_id or pkg in apps:
                continue
            merged = [{"id": k, "name": (v or {}).get("app_name") or k} for k, v in apps.items()]
            merged.append({"id": pkg, "name": _atv_pretty(pkg, at.get("app_name"))})
            res = _atv_write_apps(eid, merged)
            if res.get("ok"):
                print("  [androidtv] learned %s on %s (%d apps)"
                      % (pkg, eid, res.get("count", 0)), flush=True)
        except Exception:
            continue
