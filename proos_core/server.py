...
def _assist_awareness():
    ...
    def _room_record(area_id):
        ...
        proj = project.load()
        rec = project._resolve_rec(proj, aid) or project._resolve_rec(proj, name)
        if not rec:
        ...
        def _nm(e):
            return names.get(e) or (((rec.get("meta") or {}).get(e) or {}).get("name")) or (e or "").split(".", 1)[-1].replace("_", " ").title()
        ...
        try:
            ...
            acts = getattr(get_controller(name), "activities", None) or {}
...
def _activity_scope():
    ...
    try:
        ...
        for wt in (getattr(w, "watches", None) or []):
            e = wt.get("entity")
            if e:
                from proos import generator as _gen
                m[e] = (wt.get("name") or e,
                        _gen._slug(wt.get("area") or "site"))
    except Exception:                                            # noqa: BLE001
        pass
    try:
        ...
        for key, rec in (proj.get("areas") or {}).items():
            ...
            for e in project._area_members(rec):
                if e and e not in m:
                    ...
                    m[e] = (nm or e.split(".", 1)[-1].replace("_", " ").title(),
                            slug)
    except Exception:                                            # noqa: BLE001
        pass
