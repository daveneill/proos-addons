...
from proos.ma_ws import MaAuthFailed
...
_MA_API_PORT = 8095  # MA public API: /auth/login (REST) + /ws (WebSocket)


def _discovery_records():
    """Raw Supervisor discovery records, or []. Add-ons can't list /discovery
    (HA-core only -> 401), so this is best-effort and quiet on failure."""
    if not os.environ.get("SUPERVISOR_TOKEN"):
        return []
    try:
        return _sv("GET", "/discovery").get("discovery", [])
    except Exception:
        return []
...
def _ma_discovery_conn():
    """(host, port, auth_token) from the live Supervisor discovery record, or None."""
    for d in _discovery_records():
        if d.get("service") == "music_assistant":
            cfg = d.get("config", {}) or {}
            return (cfg.get("host"), cfg.get("port"), cfg.get("auth_token"))
    return None


def _ma_conn():
    """Connection MA commissioning uses: a token persisted at setup time first,
    falling back to a live discovery record (which we persist when present, since
    HA clears the discovery message once the integration is configured)."""
    persisted = _load_ma_conn()
    if persisted:
        return persisted
    live = _ma_discovery_conn()
    if live and live[0] and live[2]:
        _save_ma_conn(*live)
        return live
    return live  # may be None / partial — caller surfaces it
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["music", "connect"]:
                # One-time "Connect Music": installer's HA login -> ProCore mints
                # its own long-lived MA token and stores it. The password is used
                # transiently for login only and is never persisted or returned.
                b = self._body()
                user = (b.get("username") or "").strip()
                pw = b.get("password") or ""
                if not user or not pw:
                    return self._send(400, {"error": "username and password are required"})
                res = _ma.mint_token(user, pw, _ma_host_candidates(), port=_MA_API_PORT)
                _save_ma_conn(res["host"], res["port"], res["token"])
                print(f"  MA · connected via ProHost (host {res['host']})", flush=True)
                return self._send(200, {"ok": True, "connected": True, "host": res["host"]})
...
def main():
    ...
    # Music layer: ProOS owns the music_assistant integration. Try to bring it up
    # on boot, retrying briefly in case the add-on's discovery lands just after us.
    ...
    print("  music layer ready -> GET /music, POST /music/setup")
    # MA commissioning: ProOS talks to the MA server's own API (providers/players)
    # via the add-on discovery token. Read-only boot probe proves the connection.
    _ma = MaCommissioner(_ma_conn)

    def _ma_probe():
        time.sleep(20)  # let MA settle after a cold boot
        if not _load_ma_conn():
            print("  MA · not connected yet — run ProHost 'Connect Music' "
                  "(POST /music/connect)", flush=True)
            return
        try:
            inv = _ma.inventory()
            sv = inv.get("server", {})
    ...
    print("  MA commissioning ready -> GET /music/inventory|providers|players")
