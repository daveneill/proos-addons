...
def _ha_image_upload(data: bytes, content_type: str, filename: str = "avatar.jpg") -> str:
    """Upload image bytes to HA's native image store over Core's owner
    connection and return the served picture path (/api/image/serve/<id>/original).
    Used by the avatar write-shim so the app never has to touch HA directly."""
    boundary = "----proos" + os.urandom(8).hex()
    pre = (
        "--%s\r\n"
        "Content-Disposition: form-data; name=\"file\"; filename=\"%s\"\r\n"
        "Content-Type: %s\r\n\r\n" % (boundary, filename, (content_type or "image/jpeg"))
    ).encode()
    body = pre + data + ("\r\n--%s--\r\n" % boundary).encode()
    url = _cfg["base_url"].rstrip("/") + "/api/image/upload"
    req = urllib.request.Request(url, data=body, method="POST")
    req.add_header("Authorization", "Bearer " + _cfg["token"])
    req.add_header("Content-Type", "multipart/form-data; boundary=%s" % boundary)
    with urllib.request.urlopen(req, timeout=30) as resp:
        out = json.loads(resp.read().decode() or "{}")
    img = out.get("id") or (out.get("result") or {}).get("id")
    if not img:
        raise RuntimeError("image upload returned no id: %r" % out)
    return "/api/image/serve/%s/original" % img
        ...
        try:
            ...
            if parts == ["me", "picture"] or (len(parts) == 3 and parts[0] == "users" and parts[2] == "picture"):
                ...
                try:
                    ...
                    url = _ha_image_upload(raw, ctype)
