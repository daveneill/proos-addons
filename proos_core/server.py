...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if _p not in self._POST_OPEN:
            ...
            if _u is None:
                return self._send(403, {"error": "authentication required"})
            if _p.startswith(self._POST_INSTALLER) and not (
        ...
        try:
            ...
            if parts and parts[0] == "project":
                ...
                if parts == ["project", "suggest"]:
                    ...
                    # commission but whose record is missing/uncommitted is
                    # rebuilt from those labels — the labels ARE the
                    # commission. Explicit scan only (never a read path:
                    # navigation must not write). Journalled loudly.
                    try:
                        _reg2 = _client.entity_registry()
                        _devs2 = {d.get("id"): d.get("area_id")
                                  for d in (_client.device_registry() or [])}
                        _areas2 = {a.get("area_id"): a.get("name")
                                   for a in (_client.area_registry() or [])}
                        _st2 = project.load()
                        _healed = []
                        for _aid, _nm in _areas2.items():
                            if project.committed_record(_st2, _aid):
                                continue          # record is fine
                            _mine = [e for e in _reg2
                                     if (e.get("area_id")
                                         or _devs2.get(e.get("device_id")))
                                     == _aid]
                            if not any(project.LABEL_AV in (e.get("labels") or [])
                                       for e in _mine):
                                continue          # never commissioned — normal
                            _srcl = {}
                            for _e in _mine:
                                if project.LABEL_DISPLAY in (_e.get("labels") or []):
                                    _s = _client.snapshot(
                                        [_e["entity_id"]]) or {}
                                    _sv = _s.get(_e["entity_id"]) or {}
                                    _srcl[_e["entity_id"]] = (
                                        (_sv.get("attributes") or {})
                                        .get("source_list") or [])
                            _new = project.record_from_labels(
                                _mine, _aid, _nm, _srcl)
                            if _new:
                                _st2.setdefault("areas", {})[_aid] = _new
                                _healed.append(_nm or _aid)
                        if _healed:
                            project.save(_st2)
                            try:
                                from proos import journal as _j2
                                for _nm in _healed:
                                    _j2.emit(_nm, "record_healed",
                                             {"note": "the room's record was "
                                                      "missing; rebuilt from "
                                                      "its committed labels"})
                            except Exception:
                                pass
                            print("  [project] record self-heal: %s"
                                  % _healed, flush=True)
