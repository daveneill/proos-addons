...
# ── HTTPS-served Apple sign-in page (STATIC, written into HA's /www) ─────────
# Apple's MusicKit refuses authorize() from a non-secure context — measured
# live 2026-07-24: the same developer token that Apple's API accepts (200)
# rejects with AUTHORIZATION_ERROR on http://<lan>:8770 and opens the sign-in
# popup happily from an https origin. Core's :8770 is plain http on the LAN,
# so the page must be served from an https origin the home already has: HA's
# /local static hosting via the remote (Nabu Casa) URL — the same mechanism
# the dashboard PWA uses. The page gets session + dev token + opener origin
# via query params and hands the Music User Token back to Pro with
# postMessage (no mixed-content calls anywhere).
_APPLE_AUTH_STATIC = """<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
<title>ProOS Music \u2014 Connect Apple Music</title>
<style>
*{box-sizing:border-box}html,body{margin:0;height:100%;background:#0b0b0f;color:#fff;font-family:-apple-system,'Inter','Helvetica Neue',sans-serif;-webkit-font-smoothing:antialiased}
...
button{width:100%;padding:15px;border:none;border-radius:13px;font-size:16px;font-weight:600;font-family:inherit;cursor:pointer;background:#fff;color:#000}
button:disabled{opacity:.45;cursor:default}
.msg{margin-top:16px;font-size:13px;color:#8b8b93;min-height:18px}.msg.err{color:#ff6b5e}
...
<script>
  ...
  var APP_TOKEN=q.get('t')||'', SESSION=q.get('s')||'', OPENER_ORIGIN=q.get('o')||'*';
  ...
  if(!window.isSecureContext){ fail('This page must be opened over HTTPS.'); go.disabled=true; }
  else if(!APP_TOKEN){ fail('Missing developer token \u2014 reopen from ProOS Pro.'); go.disabled=true; }
  document.addEventListener('musickitloaded', async function(){
    if(!window.isSecureContext || !APP_TOKEN) return;
    try{
      ...
      go.onclick=async function(){
        ...
        try{
          ...
          try{ if(window.opener) window.opener.postMessage({proos_apple:{session:SESSION, music_user_token:token}}, OPENER_ORIGIN); }catch(e){}
          go.textContent='Connected'; msg.className='msg';
          msg.textContent='Apple Music connected \u2014 return to ProOS Pro and Save.';
          setTimeout(function(){ try{ window.close(); }catch(e){} }, 1800);
        }catch(e){ fail('Sign-in failed: '+((e&&(e.name||e.message))||'unknown')); }
      };
    }catch(e){ fail('Could not start Apple Music. '+(e&&e.message||'')); }
  });
  setTimeout(function(){ if(typeof MusicKit==='undefined' && window.isSecureContext && APP_TOKEN){ fail('Apple MusicKit failed to load.'); } }, 8000);
</script></body></html>"""


def _write_apple_auth_page():
    """(Re)write the static https sign-in page into HA's /www so it serves at
    /local/proos_apple_auth.html on every origin HA has \u2014 including the
    remote https one, which is the only place Apple sign-in can work."""
    try:
        www = os.path.join(_HA_CONFIG_DIR, "www")
        os.makedirs(www, exist_ok=True)
        p = os.path.join(www, "proos_apple_auth.html")
        with open(p, "w", encoding="utf-8") as fh:
            fh.write(_APPLE_AUTH_STATIC)
        return True
    except Exception as e:  # noqa: BLE001
        print(f"  apple auth page write failed: {e}", flush=True)
        return False
...
<script>
  ...
  var go=document.getElementById('go'), msg=document.getElementById('msg');
  function fail(t){ go.disabled=false; go.textContent='Sign in with Apple'; msg.textContent=t; msg.className='msg err'; }
  // Up-front diagnostics: Apple MusicKit authorize() requires a SECURE CONTEXT
  // (https or localhost) and a valid developer token. On plain http over the
  // LAN \u2014 how Core serves :8770 \u2014 authorize() rejects with AUTHORIZATION_ERROR
  // before anything reaches the music server. Say so plainly instead of a bare
  // error, and don't even offer sign-in when it can't possibly work.
  if(!window.isSecureContext){
    fail('This page is running over an insecure connection ('+location.protocol+'//'+location.host+'). Apple Sign-In needs HTTPS \u2014 open ProOS over its secure (https) address, then try again.');
    go.disabled=true; go.style.opacity='.45';
  } else if(!APP_TOKEN){
    fail('Apple Music isn\u2019t set up in the music server yet (no developer token). Add the Apple Music service in Music Services first.');
    go.disabled=true; go.style.opacity='.45';
  }
  document.addEventListener('musickitloaded', async function(){
    if(!window.isSecureContext || !APP_TOKEN) return;   // already reported above
    try{
      await MusicKit.configure({ developerToken: APP_TOKEN, app:{ name:'ProOS Music', build:'1.0.0' } });
      var music=MusicKit.getInstance();
      go.disabled=false; go.textContent='Sign in with Apple';
      go.onclick=async function(){
        ...
        try{
          ...
          var nm=(e&&(e.name||e.message))||'unknown';
          // Apple returns AUTHORIZATION_ERROR both for insecure context AND an
          // expired/invalid developer token \u2014 distinguish so we're not chasing
          // the wrong one next time.
          if(/AUTHORIZATION_ERROR/i.test(nm)) nm='AUTHORIZATION_ERROR \u2014 Apple rejected the request. Usually a non-HTTPS page or an expired music-server developer token.';
          fail('Sign-in failed: '+nm);
        }
      };
    }catch(e){ fail('Could not start Apple Music. '+(e&&e.message||'')); }
  });
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if parts == ["music", "apple_dev_token"]:
                # Dev token for the HTTPS sign-in popup. Refresh the static page
                # on every read so it can never go stale across Core updates.
                u = getattr(self, "_user", None)
                if not u:
                    return self._send(401, {"error": "sign in required"})
                _write_apple_auth_page()
                t = _apple_dev_token()
                if not t:
                    return self._send(503, {"error": "Music server has no Apple "
                                            "developer token yet — is ProOS Music linked?"})
                return self._send(200, {"token": t})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if len(parts) == 4 and parts[:2] == ["music", "providers"] and parts[3] == "remove":
                # Remove a service instance outright (admin channel). parts[2]
                # is the INSTANCE id — immutable, never the display name.
                try:
...
def main():
    ...
    _write_apple_auth_page()   # https Apple sign-in page — see _APPLE_AUTH_STATIC
