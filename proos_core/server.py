...
def _discovery_records():
    """Raw Supervisor discovery records (or [] off add-on / on error)."""
    if not os.environ.get("SUPERVISOR_TOKEN"):
        return []
...
def main():
    ...
    def _ma_probe():
        time.sleep(20)  # let MA + its discovery settle after a cold boot
        try:
            inv = _ma.inventory()
            sv = inv.get("server", {})
            ...
            print(f"  MA · connect probe failed: {e}", flush=True)
            # Diagnostic: show what discovery actually exposes (token value redacted).
            try:
                recs = _discovery_records()
                summary = [{"service": d.get("service"),
                            "config_keys": sorted((d.get("config") or {}).keys())}
                           for d in recs]
                print(f"  MA · discovery records: {summary}", flush=True)
                print(f"  MA · persisted conn present: {bool(_load_ma_conn())}", flush=True)
            except Exception as e2:
                print(f"  MA · discovery diagnostic failed: {e2}", flush=True)
