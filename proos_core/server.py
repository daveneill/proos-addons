    ...
    def do_GET(self):
        ...
        try:
            ...
            if parts == ["scenes", "photos"]:
                # The dashboard reads the per-scene photo/name overrides here.
                return self._send(200, {"photos": _scenephotos.load() if _scenephotos else {}})
            if parts == ["scenes", "photo_search"]:
                # Homeowner photo picker: match candidate images to a description.
                q = (parse_qs(urlparse(self.path).query).get("q") or [""])[0]
                return self._send(200, {"results": _scenephotos.search(q, 8) if _scenephotos else []})
            if parts == ["whoami"]:
                u = getattr(self, "_user", None)
                if not u:
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["scenes", "photo"]:
                ...
                # {entity_id, photo?|query?, name?, remove?}. A `query` is
                # matched to an image; `photo` is a chosen URL; `remove` clears.
                if not _scenephotos:
                ...
                photo = b.get("photo")
                if photo is None and b.get("query"):
                    photo = _scenephotos.match(b.get("query"))
