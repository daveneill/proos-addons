    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["users", "change-password"]:
                ...
                # Used by the installer's forced first-login change; clears the
                # must-change flag so the gate doesn't fire again.
                u = getattr(self, "_user", None)
                if not (users and u):
                    return self._send(403, {"error": "authentication required"})
                newpw = (self._body() or {}).get("new_password") or ""
                if len(newpw) < 6:
                    return self._send(400, {"error": "password must be at least 6 characters"})
                try:
                    users.set_password(_ws_call, u.get("id"), newpw)
                    if provision:
