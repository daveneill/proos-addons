...
def _atv_write_apps(entity_id, apps):
    ...
    # Record the intent BEFORE asking HA: a push that fails is still learned,
    # and the next pass replays it.
    _atv_ledger_put(entry_id, want)
    try:
        committed = _atv_flow_apps(entry_id, want, verbose=True)
    except Exception as e:  # noqa: BLE001
        print("  [androidtv] %s: FAILED — %s" % (entity_id, e), flush=True)
        return {"error": str(e), "count": len(applied)}
    saved = dict(committed.get("apps") or {})
    if len(saved) < len(want):
        # The flow completed but the integration kept fewer apps than we sent.
        # Say so loudly with the evidence — this is an integration fault, not a
        # ProOS one, and guessing about it has cost enough time already.
        msg = ("saved %d of %d apps — the androidtv_remote integration on this "
               "system dropped the rest at commit (options returned: %s)"
               % (len(saved), len(want), sorted(saved) or "none"))
        print("  [androidtv] %s: %s" % (entity_id, msg), flush=True)
        return {"error": msg, "count": len(saved), "committed": sorted(saved)}
    # Now the only test that counts: does the entity publish them?
    _t2 = __import__("time")
    published = {}
    for _ in range(6):                          # entry reloads; give it a moment
        ...
        if len(published) >= len(want):
            break
    print("  [androidtv] %s: %d apps saved, %d published as source_list"
          % (entity_id, len(saved), len(published)), flush=True)
    if not published:
        return {"ok": True, "count": len(saved), "published": 0,
                "note": ("saved into the integration, but it isn't publishing a "
                         "source list yet — this box may need a restart, or its "
                         "driver doesn't expose apps")}
    return {"ok": True, "count": len(saved), "published": len(published)}
    ...
    for e in ents:
        ...
        try:
            ...
            if res.get("ok") and res.get("published"):
                ...
                # So say it once and stop hammering HA every pass.
                _ATV_PUSH[entry_id] = False
                print("  [androidtv] %s: this box's driver won't keep an app list "
                      "(%s) — ProOS is using its own list, apps still work"
                      % (eid, res.get("error") or "nothing published"), flush=True)
        except Exception as ex:  # noqa: BLE001
