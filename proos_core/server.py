...
try:
    ...
    _appctl = None
try:
    from proos import shieldapps as _shieldapps   # Android TV app harvester (ADB)
except Exception:  # noqa: BLE001
    _shieldapps = None
try:
    from proos import appart as _appart   # product-level app artwork service
except Exception:  # noqa: BLE001
    _appart = None
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _auth(self):
        ...
        if auth.REQUIRE and self._user is None:
            ...
            # App artwork is public brand imagery loaded by <img>/CSS (no auth
            # header possible) — never sensitive, always allowed.
            if path not in auth.PUBLIC_PATHS and not path.startswith("apps/art/"):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if parts == ["apps", "art", "status"]:
                # One-look diagnosis: what artwork is bundled/cached/missed.
                return self._send(200, _appart.status() if _appart else {"error": "unavailable"})
            if parts == ["apps", "art", "list"]:
                # Tile manager (Pro → Tech Tools): everything served, with origin.
                # Tech/owner only — product artwork management, not commissioning.
                u = self._user
            ...
            if len(parts) == 4 and parts[0] == "apps" and parts[1] == "art" and parts[2] == "tile":
                # THE ProOS tile pack (public — loaded by <img>/CSS): curated
                # graphics shipped with the product, matched by slug/alias.
                if not _appart:
                ...
                try:
                    ...
                    self.send_header("Content-Length", str(len(data)))
                    # Short cache so a replacement upload from Tech Tools shows on
                    # dashboards within minutes, not a day.
                ...
                return
            if parts == ["shield", "targets"]:
                # Harvestable Android TV boxes: android-platform media_players
                # with their room + harvest state. Installer surface.
                u = self._user
                if not (u and (u.get("is_admin") or u.get("is_owner")
                               or (users and users.is_tech(u.get("id"))))):
                    return self._send(403, {"error": "installer access required"})
                targets = []
                try:
                    harvested = _shieldapps.load() if _shieldapps else {}
                    for e in (_client.entity_registry() or []):
                        eid = e.get("entity_id") or ""
                        if eid.startswith("media_player.") and e.get("platform") in ("androidtv_remote", "androidtv"):
                            st = None
                            try:
                                st = _client._req("GET", "/api/states/%s" % eid) or {}
                            except Exception:
                                st = {}
                            h = harvested.get(eid) or {}
                            targets.append({"entity_id": eid,
                                            "name": (st.get("attributes") or {}).get("friendly_name") or eid,
                                            "host": h.get("host") or "",
                                            "apps": len(h.get("apps") or []),
                                            "harvested": bool(h.get("apps"))})
                except Exception as e:  # noqa: BLE001
                    return self._send(500, {"error": str(e)})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parts == ["apps", "art", "upload"]:
                # Tech Tools tile manager: upload/override a tile PNG. Body:
                # {name, image:<data-url|b64>}. Tech/owner only.
                u = self._user
                if not (u and (u.get("is_owner") or (users and users.is_tech(u.get("id"))))):
                    return self._send(403, {"error": "tech access required"})
                if not _appart:
            ...
            if parts == ["apps", "art", "delete"]:
                # Tech Tools tile manager: remove an uploaded tile. Tech/owner only.
                u = self._user
                if not (u and (u.get("is_owner") or (users and users.is_tech(u.get("id"))))):
                    return self._send(403, {"error": "tech access required"})
                if not _appart:
                ...
                return self._send(400 if res.get("error") else 200, res)
            if parts == ["shield", "sync"]:
                # CERTIFIED commissioning step: harvest the box's true installed
                # apps (ADB) and write them into the androidtv_remote entry's
                # Applications List — HA then exposes source_list natively.
                # Installer-gated. Body: {entity_id, host}.
                u = self._user
                if not (u and (u.get("is_admin") or u.get("is_owner")
                               or (users and users.is_tech(u.get("id"))))):
                    return self._send(403, {"error": "installer access required"})
                if not _shieldapps:
                    return self._send(503, {"error": "sync unavailable"})
                b = self._body() or {}
                eid = (b.get("entity_id") or "").strip()
                entry_id = None
                try:
                    ...
                    entry_id = None
                if not entry_id:
                    return self._send(400, {"error": "couldn't find that device's integration entry"})
                # Host comes from the PAIRING — the integration already knows it
                # (entry data in HA storage). Only fall back to a caller-supplied
                # IP if storage doesn't yield one.
                host = (b.get("host") or "").strip()
                if not host:
                    try:
                        with open(_HA_STORAGE_ENTRIES) as f:
                            for en in (json.load(f).get("data") or {}).get("entries") or []:
                                if en.get("entry_id") == entry_id:
                                    host = str((en.get("data") or {}).get("host") or "").strip()
                                    break
                    except Exception:
                        host = ""
                if not host:
                    return self._send(400, {"error": "couldn't read the paired address — enter the box's IP"})
                res = _shieldapps.sync(_client, eid, host, entry_id)
                if res.get("ok"):
                    res["host"] = host
                return self._send(400 if res.get("error") else 200, res)
            if parts == ["shield", "harvest"]:
                # Harvest an Android box's true installed app list over network
                # ADB (+ real icons from its Play listings). Installer-gated.
                # Body: {entity_id, host}. Slow (ADB + one Play fetch per app).
                u = self._user
                if not (u and (u.get("is_admin") or u.get("is_owner")
                               or (users and users.is_tech(u.get("id"))))):
                    return self._send(403, {"error": "installer access required"})
                if not _shieldapps:
                    return self._send(503, {"error": "harvester unavailable"})
                b = self._body() or {}
                res = _shieldapps.harvest((b.get("entity_id") or "").strip(),
                                          (b.get("host") or "").strip())
                return self._send(400 if res.get("error") else 200, res)
