...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        parts = [p for p in self.path.split("?")[0].strip("/").split("/") if p]
        try:
            if len(parts) == 3 and parts[0] == "rooms" and parts[2] == "intent":
                length = int(self.headers.get("Content-Length", 0))
                body = json.loads(self.rfile.read(length) or b"{}")
                ctrl = get_controller(self._room(parts))
                run = ctrl.start(body["activity"])
                return self._send(202, run)
            if len(parts) == 3 and parts[0] == "rooms" and parts[2] == "heal":
                ctrl = get_controller(self._room(parts))
                return self._send(202, ctrl.heal())
            if len(parts) == 3 and parts[0] == "rooms" and parts[2] == "recover":
                ctrl = get_controller(self._room(parts))
                return self._send(202, ctrl.recover())
            return self._send(404, {"error": "not found"})
