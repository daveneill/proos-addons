...
def self_update() -> dict:
    # Store reload FIRST (3 Aug, seen live on the first field update): the
    # store snapshot can lag a repo push, so without this the update can
    # install an older version than the one just reported to the caller —
    # and the caller's watcher waits forever for a version that never comes.
    try:
        _req("POST", "/store/reload", timeout=120)
    ...
    def _fire():
        try:
            _req("POST", "/store/addons/%s/update" % slug, timeout=1800)
        except Exception:  # noqa: BLE001 — we die mid-update by design
            pass

    import threading
    threading.Thread(target=_fire, daemon=True).start()
    # answer BEFORE the update lands: the caller must get this response.
    # "from" rides along so the watcher can tell "settled on a different
    # version" apart from "still rebuilding" (3 Aug field race).
