...
def ma_login(host, port, username: str, password: str, timeout: float = 10.0) -> str:
    ...
    try:
        ...
        if e.code in (401, 403):
            raise MaAuthFailed("Invalid username or password") from None
        raise
...
class MaClient:
    ...
    def __init__(self, host: str, port, token: str | None = None, timeout: float = 10.0):
        ...
        # REGISTER 445 (13 Sep 2026, Build E of the mirror programme): a
        # connection is who its token says it is. The X-Remote-User-* headers
        # that dressed a connection up as a platform admin on the engine's
        # ingress listener are gone — the engine's own auth, or nothing.
        self._sock = None
        self._reader = None
        self._mid = 0
