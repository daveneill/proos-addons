...
# Set to "0" when the factory image bakes the owner WITHOUT a password
# credential (recommended - there is then no shared secret to rotate).
FACTORY_OWNER_HAS_PASSWORD = os.environ.get("PROOS_OWNER_HAS_PW", "1") == "1"
...
def _harden_owner(ws_call) -> bool:
    ...
    try:
        ...
        owner = next(
            (u for u in users
             if u.get("username") == FACTORY_OWNER_USERNAME
             or (u.get("is_owner") and not u.get("system_generated"))),
            None,
        )
        if not owner:
            _LOG.info("provision - baked owner not found, skipping rotation")
            return False
        ws_call(
