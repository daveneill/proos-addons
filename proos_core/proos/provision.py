...
# /homeassistant, older at /config.
#
# A2 PHASE 3, redefined by the DESIGN PASS (23 Aug 2026, regs 272-273;
# Dave's ruling: "If this is the way that supports the final product then
# yes"). The user apps live on HA's OWN origin — served from <config>/www —
# per HA's Auth API / Companion architecture. Core ships them in its image
# (/app/apps, the copies the gate pins byte-identical to deploy/) and
# CONVERGES www/ to them on every boot: install when different, leave alone
# when identical, touch nothing else in www/, never block boot. This is the
# end of the hand-copy ritual — one add-on update carries everything.
# (Before 273 this pointed at /app/www-dist, a folder no longer in the
# image, with overwrite=False — so on a live box it did nothing.)
HA_CONFIG = "/homeassistant" if os.path.isdir("/homeassistant") else "/config"
WWW_SRC = os.environ.get("PROOS_WWW_SRC", "/app/apps")

# Only these ship — the same two names Core's /app/* routes whitelist.
APP_FILES = ("pro.html", "dashboard.html")

try:
    from proos import journal as _journal
except Exception:  # noqa: BLE001 — journal is optional; never block boot
    try:
...
def _app_build(data: bytes) -> str:
    """The PROOS_BUILD stamped inside an app file, for the versioned report."""
    m = re.search(rb"PROOS_BUILD\s*=\s*['\"]([0-9A-Za-z._-]+)['\"]", data)
    return m.group(1).decode("ascii", "replace") if m else "unknown"


def deploy_dashboards() -> dict:
    """Converge <ha_config>/www to the app files shipped in Core's image.

    The ruled law (register 273): a file whose bytes differ (or which is
    absent — a factory-fresh box) is INSTALLED, atomically (temp file then
    os.replace — a live file is never opened for write, rule 9/268); a file
    already identical is LEFT ALONE, not even rewritten; nothing outside
    APP_FILES is ever touched; every install is journaled with its build;
    a per-file failure is recorded and must never block boot."""
    ...
    for name in APP_FILES:
        ...
        try:
            with open(src, "rb") as f:
                want = f.read()
            if not want:
                raise ValueError("shipped copy is empty")
            have = None
...
# --------------------------------------------------------------------------
# A4 — the native door leads to ProOS (24 Aug 2026, reg 278; Dave's GO on
# the corrected proposal — HOMEOWNER-FACING ONLY, installers untouched,
# reg 274). Anyone who browses to the box and signs in lands INSIDE ProOS:
# a "proos" dashboard (a full-screen frame of the homeowner app on HA's
# own origin) is the installation default, and every OTHER created
# dashboard is admin-only. The built-in Overview leaves the sidebar by
# the platform's own documented design once a default is set. Admins —
# installer included — see everything exactly as before.
#
# The write contract was READ from the platform's source (24 Aug):
# lovelace/dashboards/{list,create,update} (collection commands,
# lovelace/__init__.py), lovelace/config + lovelace/config/save
# (websocket.py), and frontend/{get,set}_system_data with the
# core.default_panel key (frontend/storage.py + frontend.ts). The
# system-data write is a MERGE — the platform keeps other keys there.
# --------------------------------------------------------------------------
DOOR_URL_PATH = "proos"
DOOR_TITLE = "ProOS"
DOOR_ICON = "mdi:home-circle"          # matches the add-on's panel icon
DOOR_CONFIG = {"views": [{"title": DOOR_TITLE, "panel": True,
                          "cards": [{"type": "iframe",
                                     "url": "/local/dashboard.html",
                                     "aspect_ratio": "100%"}]}]}


def ensure_native_door(ws_call=None) -> dict:
    """Converge the platform's native door to ProOS. The ruled law:
    converge, don't clobber — only what differs is written; an
    already-right box gets zero writes; every change journaled
    (site/native_door), a no-change pass journals nothing; per-step
    best-effort — a failure is recorded and never blocks boot."""
    result = {"changed": [], "current": [], "errors": []}
    if ws_call is None:
        result["errors"].append("no ws_call")
        return result

    boards = []
    try:
        boards = ws_call("lovelace/dashboards/list") or []
    except Exception as e:  # noqa: BLE001
        result["errors"].append("list: %s" % e)

    # 1 · the door exists, in the sidebar, never admin-only
    door = next((d for d in boards
                 if isinstance(d, dict)
                 and d.get("url_path") == DOOR_URL_PATH), None)
    try:
        if door is None:
            door = ws_call("lovelace/dashboards/create",
                           url_path=DOOR_URL_PATH, title=DOOR_TITLE,
                           icon=DOOR_ICON, require_admin=False,
                           show_in_sidebar=True) or {}
            result["changed"].append("door_created")
        else:
            needs = {}
            if door.get("require_admin"):
                needs["require_admin"] = False
            if door.get("show_in_sidebar") is False:
                needs["show_in_sidebar"] = True
            if door.get("title") != DOOR_TITLE:
                needs["title"] = DOOR_TITLE
            if needs:
                ws_call("lovelace/dashboards/update",
                        dashboard_id=door.get("id"), **needs)
                result["changed"].append("door_settings")
            else:
                result["current"].append("door")
    except Exception as e:  # noqa: BLE001
        result["errors"].append("door: %s" % e)

    # 2 · the door's page — product-owned, converged byte-for-value
    try:
        have = None
        try:
            have = ws_call("lovelace/config", url_path=DOOR_URL_PATH)
        except Exception:  # noqa: BLE001 — a fresh dashboard has no config
            have = None
        if have != DOOR_CONFIG:
            ws_call("lovelace/config/save", url_path=DOOR_URL_PATH,
                    config=DOOR_CONFIG)
            result["changed"].append("door_page")
        else:
            result["current"].append("door_page")
    except Exception as e:  # noqa: BLE001
        result["errors"].append("door_page: %s" % e)

    # 3 · the installation default — MERGED into the platform's core keys
    try:
        res = ws_call("frontend/get_system_data", key="core") or {}
        value = dict(res.get("value") or {})
        if value.get("default_panel") != DOOR_URL_PATH:
            value["default_panel"] = DOOR_URL_PATH
            ws_call("frontend/set_system_data", key="core", value=value)
            result["changed"].append("default")
        else:
            result["current"].append("default")
    except Exception as e:  # noqa: BLE001
        result["errors"].append("default: %s" % e)

    # 4 · every other created dashboard is pros-only
    for d in boards:
        if not isinstance(d, dict) or d.get("url_path") == DOOR_URL_PATH:
            continue
        if d.get("require_admin"):
            result["current"].append("admin_only:%s" % d.get("url_path"))
            continue
        try:
            ws_call("lovelace/dashboards/update",
                    dashboard_id=d.get("id"), require_admin=True)
            result["changed"].append("admin_only:%s" % d.get("url_path"))
        except Exception as e:  # noqa: BLE001
            result["errors"].append("admin_only %s: %s" % (d.get("url_path"), e))

    if result["changed"] and _journal is not None:
        try:
            _journal.emit("site", "native_door",
...
def firstboot(ws_call=None) -> dict:
    ...
    out["dashboards"] = deploy_dashboards()
    return out
...
# the Home-page background. REGISTER 440: the name is used ONCE, at creation;
# from then on the area is known by the dashboard_system label alone
# (proos/sitearea.py) — rename it and nothing in ProOS notices but the words.
...
def ensure_services_area(ws_call=None) -> dict:
    ...
    try:
        ...
        # 2) the site area — by its LABEL first (register 440); the name is only
        #    a way to adopt an unlabelled area from an older install, or the
        #    name to create it with.
        ...
        if not area:
            ...
            if area and area.get("area_id") and label_id not in (area.get("labels") or []):
                # an older install's Home, never labelled: stamp it now so the
                # name is never needed again
                ws_call("config/area_registry/update", area_id=area["area_id"],
                        labels=list(area.get("labels") or []) + [label_id])
        created = False
        if not area:
...
# REGISTER 449 (13 Sep 2026, Build G): there is NO string in the source any
# more. With the option blank, Core mints a per-box secret once and keeps it
# in its own store (owner_secret, beside hardware_id — kept across a factory
# reset, like the fleet catalog), so a bench box derives its Developer
# password from a secret nobody else has. The option, when set, wins.
OWNER_SECRET_PATH = os.path.join(DATA_DIR, "owner_secret")
INSTALLER_USERNAME = os.environ.get("PROOS_INSTALLER_USER", "installer")
# THE DEFAULT INSTALLER PASSWORD STAYS (Dave, 13 Sep 2026): boxes ship with
# it, and the forced change on first login is the mechanism — a white-label
# product must support that. Not a secret: a first-use code.
INSTALLER_DEFAULT_PW = os.environ.get("PROOS_INSTALLER_PW", "proos-install")


def owner_secret() -> str:
    """The option when set; else this box's own kept secret, minted once."""
    env = os.environ.get("PROOS_OWNER_SECRET", "")
    if env:
        return env
    try:
        if os.path.exists(OWNER_SECRET_PATH):
            kept = open(OWNER_SECRET_PATH, encoding="utf-8").read().strip()
            if kept:
                return kept
        val = secrets.token_urlsafe(32)
        _atomic_write(OWNER_SECRET_PATH, val)
        _LOG.info("provision - owner secret minted for this box (option blank; register 449)")
        return val
    except Exception:  # noqa: BLE001
        # A box whose store cannot be written derives from nothing kept —
        # the password would change every boot. Say so rather than fall to
        # a string in the source.
        raise RuntimeError("owner secret: the option is blank and the store cannot be written")
...
def _onboard_bases():
    # The platform by the Supervisor's own name first — public onboarding
    # needs no token and dodges the Supervisor→Core proxy, which doesn't
    # cleanly serve onboarding on a fresh box. The proxy is the last-resort
    # fallback. (Register 439: the typed ha_direct override is gone.)
    bases = []
