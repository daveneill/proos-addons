...
SERVICES_AREA_NAME = os.environ.get("PROOS_SERVICES_AREA", "Services")
DASHBOARD_SYSTEM_LABEL = "dashboard_system"


def ensure_services_area(ws_call=None) -> dict:
    """Idempotently ensure the standing 'Services' area exists and carries the
    dashboard_system label (a global/infrastructure room). Safe to call every boot
    and after a factory reset; only creates what's missing. Never raises."""
    if ws_call is None:
...
def auto_onboard(ws_call=None) -> dict:
    ...
    try:
        ...
        if state != "fresh":
            ...
            if state == "unknown":
                out["ok"] = False
                out["error"] = "no HA onboarding endpoint reachable (tried direct + proxy)"
