...
def _core_req(path, method="GET", payload=None, token=None, form=False, timeout=30):
    if form:
        import urllib.parse
    ...
    req.add_header("Authorization", "Bearer " + (token or SUPERVISOR_TOKEN))
    if data is not None:
...
def _onboarding_state():
    """'done' | 'fresh' | 'unknown'. HA removes /api/onboarding once complete
    (→ 404 = done); a steps list with the user step not done = fresh."""
    try:
        req = urllib.request.Request(CORE + "/api/onboarding", method="GET")
        ...
        return "done" if (u and u.get("done")) else "fresh"
    except urllib.error.HTTPError as e:
        return "done" if e.code == 404 else "unknown"
    except Exception:
        return "unknown"
...
def auto_onboard(ws_call=None) -> dict:
    ...
    try:
        ...
        out["state"] = state
        if state != "fresh":
            # 'done' → already onboarded; 'unknown' → fail-safe, don't risk a 2nd owner
            out["already"] = (state == "done")
            return out
        r = _core_req("/api/onboarding/users", "POST", {
            "client_id": ONBOARD_CLIENT, "name": "ProOS Developer",
            "username": OWNER_USERNAME, "password": owner_password(), "language": "en"})
        out["owner_created"] = True
        code = r.get("auth_code")
        owner_token = None
        if code:
            try:
                tok = _core_req("/auth/token", "POST", {
                    "grant_type": "authorization_code", "code": code,
                    "client_id": ONBOARD_CLIENT}, form=True)
        ...
        if owner_token:
            for step, body in (("core_config", {}), ("analytics", {}),
                ...
                try:
                    _core_req("/api/onboarding/" + step, "POST", body, token=owner_token)
                except Exception:  # noqa: BLE001
                    pass
        ...
        _LOG.info("auto_onboard - provisioned owner+installer on a fresh box")
        return out
    except Exception as exc:  # never block boot
        _LOG.warning("auto_onboard failed, will retry next boot: %s", exc)
