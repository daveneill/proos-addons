...
class Check:
    ok: bool
    detail: str
    # REGISTER 446: a verdict reached on ONE signal where the design asks for
    # two says so. ok=True, unverified=True is the third state — "watching X,
    # on the source's own word" — never the same word as a corroborated one.
...
class Activity:
    ...
    wake: dict | None = None
    # The source media_player this activity watches (for overlay matching and
    # witness verification). None for broadcast/sourceless activities.
    source_eid: str | None = None
    # Independent reachability witness: a binary_sensor (ping/router) that tracks
    # whether the source is actually on the network. Used in the "both must
    # agree" verification -- the source's own state AND this sensor must concur
    # before the room is reported as authoritatively watching. None until a
    # reachability witness is commissioned for the source.
    reachability_sensor: str | None = None
    # Independent AUDIO witness: the audio device (Sonos/AVR) and the source it
    # must be on for this activity's audio to be confirmed routed -- e.g.
    # {"entity":"media_player.family_room","source":"TV"} or
    # {"entity":"media_player.living_room_av_receiver","source":"Blu-ray"}.
    # "watching" requires this to agree (audio actually on the right device/input),
    # not just the picture. None when the room uses the TV's own speakers.
    audio_witness: dict | None = None
    # True when this activity was auto-created from discovery and its routing /
    # wake / witness have NOT been confirmed by commissioning. The room still
    # works (CEC wake-only), but the verdict is flagged unverified so the
    # installer can see what needs setting up.
    provisional: bool = False

    def entity_ids(self) -> list[str]:
        ids = [t.entity_id for t in self.targets]
        if self.reachability_sensor:
            ids.append(self.reachability_sensor)
        if self.audio_witness and self.audio_witness.get("entity"):
            ids.append(self.audio_witness["entity"])
        return ids
