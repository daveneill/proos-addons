...
# REGISTER 439: ma_conn.json is gone — where ProOS Music is (host, port) and
# the platform's token are the PLATFORM's own facts (its music_assistant
# entry, the Supervisor's discovery message), read on every use and never
# copied here. Three stores remain, all ProOS's own.
STORES = ("ma_admin.json",
          "music_speakers.json", "music_genres.json")
_RETIRED = ("ma_conn.json",)   # swept by forget() and reset if an older build left one
...
# ── admin identity ─────────────────────────────────────────────────────────
# REGISTER 451: the admin-token helpers that lived here were never called —
# server.py holds the one set (_load_ma_admin_token and friends).
