...
carry an independent liveness signal -- a `ping` binary_sensor or a router/UniFi
device_tracker, the PLATFORM's witness (register 448: the TCP probe from Core
is gone; an `ip` is an address, never a witness) -- supplied either inline on
the watch (`"reach": {"sensor": ...}`) or via the add-on's `reachability`
...
from .reachability import is_witness as _is_witness
...
class Watcher:
    ...
    def reach_specs(self):
        """Per watched device: the second-signal spec ACTUALLY in use, where it
        came from, and whether it is a witness (register 455).

        Dave, 14 Sep 2026: Health said "5 devices watched · 5 confirmed two
        ways" while the boot log said "0 with a platform witness". The header
        was right — each Sonos publishes its own connection sensor and
        discovery binds it INLINE on the watch. The log was counting the
        reachability CONFIG MAP, which never sees an inline witness, so the
        product understated its own awareness in the record an installer
        reads when something is wrong. One question, one answer: this is it,
        and the summary and Pro's list both read it.

        source: "device" — the spec discovery bound to the watch itself
                "config" — the merged auto/manual reachability map
                None     — no second signal at all
        """
        out = []
        for w in self.watches:
            inline = w.get("reach")
            spec = inline or self.reach_map.get(w["entity"])
            out.append({"entity": w["entity"], "name": w.get("name"),
                        "area": w.get("area"), "spec": spec,
                        "source": ("device" if inline else
                                   "config" if spec else None),
                        "sensor": (spec or {}).get("sensor"),
                        "ip": (spec or {}).get("ip"),
                        "witness": _is_witness(spec)})
        return out
    ...
    def _resolve_reach(self, spec, states):
        ...
        round trip); nothing else is a witness (register 448).
        ...
        # REGISTER 448: an address alone is not a witness — no probe from
        # Core. A device with no platform sensor is single-signal (None).
        return None

    def _reach_cached(self, w, spec, states, now):
        """Sensor specs read from the batch snapshot (free). There is no
        other kind of witness any more (register 448)."""
        if spec.get("sensor"):
    ...
    def tick(self):
        ...
        with self._lock:
            for w in self.watches:
                ...
                live_reachable = None
                # Don't let the witness DOWNGRADE a camera that HA already
                ...
                # offline the way an Apple TV can. NOTE: this only skips the
                # witness while the camera is HEALTHY; the two-signal wedge test
                # still runs below when a camera IS unavailable (reachable ->
                # integration wedged, unreachable -> genuinely offline).
                ...
                if healthy:
                    ...
                    # STAGE 7 BUILD 1 (16 Aug 2026, Dave's switch-test
                    # ruling: "Immediately"): a device whose OWN switch reads
                    # offline needs no confirm window — the controller
                    # flagging its own gear IS the confirmation, and waiting
                    # fault_after on top left the TV reading amber for 30-45s
                    # after ProOS already held the answer. The debounce
                    # survives everywhere a stronger reading has NOT already
                    # answered (switch_speed_bench.py).
                    debounced = ((now - rt.unhealthy_since)
                                 >= w.get("fault_after", 30)) or _my_gear_down
    ...
    def _maybe_recover(self, w, rt, ent, now):
        ...
        rec = w.get("recover")
        if not rec or rt.verdict not in (V_INTEGRATION, V_OFFLINE):
            # REGISTER 450: an OFFLINE device is handed over too — Assist
            # reads its physical path and decides; before, offline got
            # nothing ("usually needs a human").
            return
    ...
    def report(self):
        ...
        with self._lock:
            ...
            for w in self.watches:
                ...
                # devices unreachable.
                _bound = _is_witness(self._reach_spec(w))    # register 448: a sensor, never an address
...
# REGISTER 449: the integration's NAME is the platform's — read from its
# manifests (live_ha.integration_names) when discovery runs with a client and
# kept in process; with no name in hand the domain id itself is shown, never
# a word invented from it (register 440).
_INTEG_NAMES: dict = {}


def _integ_name(platform):
    return _INTEG_NAMES.get(platform) or (platform or "")
...
def _reg_name(ent):
    """What the platform calls the entity — its registry name, else its
    original name, else the id itself. Never a name invented from the id
    (register 440)."""
    return (ent.get("name") or ent.get("original_name") or ent.get("entity_id") or "")
...
def _mk_watch(name, entity, kind, fault_after, platform, power_aware=False):
    ...
    _FRIENDLY = {"media": "streaming", "audio": "audio", "display": "TV",
                 ...
                 "security": "alarm"}
    integ = _FRIENDLY.get(kind) or _integ_name(platform) or "the"
    if kind == "security":
        ...
        gw = (f"{name} panel is on the network but its connection was lost. Reload the "
              f"{_integ_name(platform) or 'panel'} integration — the panel itself is fine.")
    elif power_aware:
        ...
        nvr = _integ_name(platform) or "the camera"
    ...
    if recoverable:
        # REGISTER 450 (Dave, 14 Sep 2026): auto-heal HANDS THE FAULT TO
        # ASSIST — no reload-and-hope, no scripted ladder. The grace covers
        # a model turn with its reads, an act and a read-back.
        w["recover"] = {"tier": "safe", "action": "assist",
                        "cooldown": 900, "grace": 240, "max": 3}
...
def _classify_device(ents, name, entry_domain):
    ...
    if cams:
        p = _best_camera(cams)
        return _mk_watch(name or _reg_name(p), p["entity_id"],
...
# REGISTER 440: _room_from_name is GONE. It filed a device into a room by
# matching the device's NAME against area names ("Bedroom Apple TV" ->
# Bedroom) — a guess, in the same codebase whose doctrine says a guess is
# never a placement. A device's room is the area the platform holds for the
# device, or nothing.
...
def discover_watches(storage_dir=None, client=None):
    ...
    if client is not None and hasattr(client, "integration_names"):
        try:
            _names = client.integration_names() or {}
            if _names:
                _INTEG_NAMES.clear()
                _INTEG_NAMES.update(_names)
        except Exception:  # noqa: BLE001
            pass
            ...
            if "reach" not in w:
                for e in ents:
                    ...
                    if "proos_connection" in str(e.get("unique_id") or "") \
                        ...
                        w["reach"] = {"sensor": ce}
                        break
            # The room is the device's own area in the platform's registry —
            # its ID is the key (register 440); the name rides beside it for
            # display and is re-read on every discovery. Nothing is inferred
            # from a name. The entity-level area override is deliberately NOT
            # used: a stale one is exactly what mis-filed unassigned cameras
            # into a single room.
            aid = dev_area.get(did) if areas.get(dev_area.get(did)) else None
            w["area_id"] = aid
            w["area"] = areas.get(aid) if aid else "Unassigned"
            w["_did"] = did
            watches.append(w)
            seen.add(w["entity"])
