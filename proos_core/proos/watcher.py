...
class Watcher:
            ...
            for w in self.watches:
                ...
                if healthy:
                    ...
                    if pa and (reachable is None or _wall_switched):
                        ...
                        # A-8: when the gear feeding the house is down, a device
                        # that cannot be reached has NOT been shown to be
                        # broken — it has no path. Saying "offline" would send
                        # the installer to check a television's power lead when
                        # the actual job is a switch. Name the real cause.
    ...
    def _guidance(self, w, rt):
        ...
        if rt.verdict == V_NOPATH:
            ...
            _g = self._gear_down()
            _who = _g[0] if len(_g) == 1 else "%d pieces of network gear" % len(_g)
    ...
    def report(self):
        ...
        with self._lock:
            ...
            for w in self.watches:
                ...
                # WHY A CAMERA WAS SILENT, HONESTLY. A healthy camera is
                # deliberately NOT probed (`_cam_up`) — its NVR answers for it
                # and the generic probe uses a port a camera never opens. So it
                # had a witness bound, was never asked, and got reported as
                # "not answering". Nobody asked it. That is a different fact
                # and it now says so.
                _unasked = bool(_bound and w.get("kind") == "camera"
                                and rt.status == OK and rt.reachable is None)
                if _port_live:
