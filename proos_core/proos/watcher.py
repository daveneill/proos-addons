...
def _mk_watch(name, entity, kind, fault_after, platform, power_aware=False):
    ...
    if power_aware:
        w["power_aware"] = True
    # Certified remote-primary devices (Apple TV, Android TV, Fire TV, Roku) get a
    # SAFE auto-recovery: if the device is reachable but the integration wedged,
    # reload the integration (never power-cycle the device). Tiered + rate-limited.
    if platform in _SRC_PLATFORMS:
