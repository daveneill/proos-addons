...
def _classify_device(ents, name, entry_domain):
    ...
    if rems:
        p = rems[0]
        # POWER-AWARE, like TVs and AVRs. A streaming box that's asleep closes
        # its ports (an NVIDIA Shield stops answering the liveness probe in deep
        # sleep — the same reason its ADB port refuses connections), so probing a
        # RESTING box was reporting a perfectly healthy Shield as "offline".
        # Off/asleep is a resting state for a source box, exactly as it is for a
        # display; only a wedged integration (reachable but HA lost it) faults.
        return _mk_watch(name or _pretty(p["entity_id"]), p["entity_id"],
