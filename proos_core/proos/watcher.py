        ...
        with self._lock:
            for w in self.watches:
                ...
                if healthy:
                    ...
                    # wedged); always-on devices only probe once past debounce.
                    reachable = None
                    if spec and (pa or debounced):
...
def _mk_watch(name, entity, kind, fault_after, platform, power_aware=False):
    integ = _INTEG_NAME.get(platform, (platform or "the").replace("_", " ").title())
    if power_aware:
        # Off is normal for these; the only fault we raise is a wedged integration.
        g = f"{name} appears to be off."
        gw = (f"{name} is on the network, but Home Assistant has lost it. "
              f"Restart the {integ} integration -- the device itself is fine.")
    elif kind == "camera":
        g = f"{name} is offline. Check the camera's power/PoE, then the {integ} NVR."
        gw = (f"{name} answers on the network, but Home Assistant lost its feed. "
              f"Restart the {integ} integration -- the camera itself is fine.")
    elif kind == "audio":
        g = f"{name} dropped off the network. Check its power and Wi-Fi/Ethernet."
        gw = (f"{name} is online, but Home Assistant lost it. Restart the {integ} "
        ...
        g = (f"{name} lost network or was powered off. Check Wi-Fi/power; if it "
             f"persists, restart the {integ} integration in Home Assistant.")
        gw = (f"{name} is online, but Home Assistant has lost its connection. "
...
def discover_watches(storage_dir="/homeassistant/.storage"):
    """Derive the watch list from HA's registries. One watch per always-on
    physical device, deduped, sorted. Empty list on any failure (caller keeps
    its current/fallback watches)."""
    from . import netmap
    entries, devices, entities = netmap.load_registries(storage_dir)
    areas = netmap.load_areas(storage_dir)
    ...
    for did, ents in by_dev.items():
        ...
        if w and w["entity"] not in seen:
            aid = ent_area.get(w["entity"]) or dev_area.get(did)
            w["area"] = areas.get(aid) or "Unassigned"
            watches.append(w)
            seen.add(w["entity"])
