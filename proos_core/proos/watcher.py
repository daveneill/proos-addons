...
class Watcher:
    ...
    def _port_of(self, entity):
        ...
        gear itself is online — so the question `OFFNET_WHEN_OFF` guesses at
    ...
    def report(self):
        ...
        with self._lock:
            ...
            for w in self.watches:
                ...
                # THE PHYSICAL READING (step 2). Carried as facts, judged by
                # nobody yet — see _port_of. `None` throughout when ProOS
                # cannot see the port, which is not the same as a port that is
                # down, and the surfaces must keep them apart.
                ...
                items.append({
                    ...
                    "has_signal": _bound,
                    "port": _port,
                    "signal": ("confirmed" if (_bound and rt.reachable is not None)
                               else "silent" if _bound else "none"),
