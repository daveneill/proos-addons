...
def check_room(ctrl) -> RoomHealth:
    ...
    # --- state drift (only meaningful when an activity is asserted and settled) ---
    asserted = getattr(ctrl, "asserted", None)
    mid_run = ctrl.run is not None and ctrl.run.finished_at is None
    if asserted and not mid_run:
        chk = asserted.summary(snap)
        if not chk.ok:
            # The room no longer matches what Core last asserted -- but the user
            # may have switched it by ANY path Core didn't mediate: the dashboard
            # navbar firing the HA script directly, the HA UI, a voice assistant,
            # a schedule. Before calling that drift, re-derive from reality: if the
            # room now cleanly matches some OTHER known activity (including TV Off),
            # adopt it as the set activity. Drift is only real when reality matches
            # NO clean activity at all -- a genuine half-broken state we set once
            # and can re-assert.
            match = next((a for a in ctrl.activities.values()
                          if a is not asserted and a.summary(snap).ok), None)
            if match is not None:
                ctrl.asserted = asserted = match
            else:
                issues.append(Issue(
                    kind="drift",
                    headline=f"{ctrl.area} drifted from '{asserted.label}'",
                    detail=f"Set to '{asserted.label}', but right now: {chk.detail}.",
                    suggested_action=f"Restore '{asserted.label}'.",
                    auto_recoverable=True,    # we set this state once; we can re-assert it
                ))

    fault = any(i.kind in ("device_fault", "unreachable") for i in issues)
    drift = any(i.kind == "drift" for i in issues)
    if fault:
        nfault = sum(i.kind in ("device_fault", "unreachable") for i in issues)
        status, summary = "fault", f"{nfault} device(s) need attention"
    elif drift:
        status, summary = "attention", "Room drifted from its set activity"
    else:
        n = len(devices)
...
class Monitor:
    ...
    def check(self, area: str) -> RoomHealth | None:
        ...
        self.cache[area] = h
        if self.auto_heal:
            self._maybe_heal(ctrl, h)
        return h

    def _maybe_heal(self, ctrl, h: RoomHealth):
        recoverable = next((i for i in h.issues if i.auto_recoverable), None)
        stale = next((i for i in h.issues if i.kind in ("device_fault", "unreachable")), None)
        if not (recoverable or stale):
            return
        # debounce: don't re-heal a room more than once a minute
        if time.time() - self._last_heal.get(ctrl.area, 0) < 60:
            return
        self._last_heal[ctrl.area] = time.time()
        self._log(f"[monitor] auto-heal: recovering '{ctrl.asserted.label}' in {ctrl.area}")
        ctrl.recover()
