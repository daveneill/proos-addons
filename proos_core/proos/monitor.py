...
@dataclass
class RoomHealth:
    area: str                       # the platform's name for the room (words)
    status: str                     # 'ok' | 'attention' | 'fault'
    summary: str
    asserted: str | None
    issues: list[Issue] = field(default_factory=list)
    checked_at: float = 0.0
    area_id: str = ""               # the key (register 441)
...
def check_room(ctrl) -> RoomHealth:
    ...
    for d in devices:
        ...
        if rec.get("missing"):
            # REGISTER 447: not on the platform at all — a plan fault, never
            # "offline"; the professional is sent to the record, not the rack.
            issues.append(Issue(
                kind="entity_missing",
                headline=f"{d.name} is not on the platform",
                detail=(f"The room's record drives {d.entity}, but the platform has no "
                        f"entity by that id — renamed, removed, or re-added under a new "
                        f"id. The device itself may be fine."),
                entity=d.entity,
                suggested_action="Re-commission the room in Pro so its record carries the platform's current id.",
                auto_recoverable=False,
            ))
            continue
        if st in DEAD:
        ...
        if reachable is False:
            ...
            if st in BELIEVES_PRESENT:
                ...
                issues.append(Issue(
                    ...
                    # SAY WHAT CANNOT BE TOLD APART FROM HERE (19 Aug 2026,
                    # register 234). The two readings and the class fact are
                    # honest and stay; what was missing is the professional's
                    # next question — which of the three is it, and what would
                    # settle it. Naming the limit is part of the reading.
                    detail=(f"{d.name} reports '{st}', but it has also dropped off "
                            ...
                            f"Nothing ProOS can read from here tells those three "
                            f"apart — its switch port, or a ping from something "
                            f"beside it, would."),
                    entity=d.entity,
                    suggested_action=SUGGESTED.get(d.integration, "Check the device's power and network."),
                    auto_recoverable=False,
    ...
    return RoomHealth(area=ctrl.area, area_id=getattr(ctrl, "area_id", "") or "", status=status, summary=summary,
