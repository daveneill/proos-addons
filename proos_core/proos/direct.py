...
_SAY = {
    "room_off": lambda a, n, sc: "The %s is off." % n,
    "room_on": lambda a, n, sc: "The %s is on." % n,
    "scene_apply": lambda a, n, sc: ("%s is on." % sc) if sc else None,
}
_SAY_VOL = {"up": "Turned the %s up.", "down": "Turned the %s down.",
            "mute": "The %s is muted.", "unmute": "The %s is unmuted."}
_SAY_MEDIA = {"pause": "Paused.", "stop": "Stopped.", "play": "Playing.",
              "next": "Next.", "previous": "Back."}


def say_for(tool, args, room_name=None, scene_name=None):
    ...
    if not n and tool in ("room_off", "room_on", "room_volume", "area_control"):
        return None
    if tool in _SAY:
        return _SAY[tool](args, n, scene_name)
    if tool == "room_volume":
        act = str(args.get("action") or "")
        if act == "set":
            try:
                return "The %s is at %d%%." % (n, int(args.get("level")))
    ...
    if tool == "area_control":
        ...
        if a == "turn_off":
            return "The %s lights are off." % n
        if a == "turn_on":
            return "The %s lights are on." % n
    return None
