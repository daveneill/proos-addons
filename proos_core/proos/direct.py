...
# ── REGISTER 515 · ONE SPELLING, FOLDED ONCE ────────────────────────────────
# Dave, 19 Sep, having waited all day: "its slower... confirming when turning on
# and quicker when turning off." It was neither. Read out of his own homeowner
# conversation:
#
#     1:08 PM  "Turn office light off."   ->  "Office light is off."
#     1:07 PM  "Turn office light on."    ->  "The Office light is on."
#
# Those replies are the MODEL's words, not this phrasebook's. Every command he
# has ever spoken has gone the slow road, because this list held "turn lights
# on" and "light on" but not "turn light on" -- singular, with a verb in front,
# which is the way he says it. The fast road of register 486 has never once
# fired for the person it was built for, and nothing reported that: a miss is
# silent BY DESIGN, so the product cannot tell you it is not being used.
#
# THE FIX IS NOT TWO MORE STRINGS. Dave's own law: no per-scenario rules, fix
# the structure. The list was enumerating singular and plural by hand and was
# always going to miss a combination. The words are written ONE way here, and
# the sentence is folded to that one way before it is matched -- so every
# phrasing of every verb gets both spellings, for free, for ever.
_PLURALS = {"lights": "light", "blinds": "blind", "curtains": "curtain",
            "shades": "shade"}


def _fold(t: str) -> str:
    """Sentence -> the one spelling the lists are written in. Word by word, so
    a room called 'Lights Out' cannot be mangled: the room has already been
    lifted out before this is reached."""
    return " ".join(_PLURALS.get(w, w) for w in t.split())


_LIGHTS_OFF = ("light off", "light out", "turn the light off",
               "turn light off", "kill the light")
_LIGHTS_ON = ("light on", "turn the light on", "turn light on")
...
def resolve(text, where=None, rooms=(), scenes=()):
    ...
    # REGISTER 515: folded AFTER the room is lifted out, never before.
    r = _fold(_drop_degree(_strip_filler(rest)))
    if not r:
        return _scene_hit(t, _scenes())
