...
def publish(msg: dict):
    """STAGE 1 BUILD 3 (16 Aug 2026): the public door onto the live bus.
    Core tells every connected surface the moment something it serves has
    changed — the watcher's report nudge rides through here — instead of
    the glass discovering it on a poll clock. Verbatim fan-out, no
    interpretation; slow clients are dropped by _fanout as always."""
    _fanout(msg)
