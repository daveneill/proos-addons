...
actually ANSWERS the MCP handshake is used. The handshake is the
identity; the name only narrows where to knock. No answer anywhere →
status says so.

REGISTER 439: nothing is written down. The endpoint that answered is
held in this PROCESS only, so the healthy path costs no probe; a
restart asks the Supervisor again, and a failed call forgets it. The
old mcp_conn.json (register 218) is gone — it was a copy of the
Supervisor's record, retried first even after the record had moved.
"""
...
_KNOWN = {"url": None}          # the endpoint that last answered — this process only
...
def _conn_load():
    return dict(_KNOWN) if _KNOWN.get("url") else {}


def _conn_save(d):
    _KNOWN["url"] = (d or {}).get("url")


def forget():
    """A call failed: the endpoint is no longer proven. Next use asks the Supervisor."""
    _KNOWN["url"] = None


def discover(sv_get, http=None):
    """An HaMcp for the box's MCP server, or raise McpUnavailable with the
    truth. `sv_get(path)` is Core's Supervisor GET. The endpoint that last
    answered (this process only) is tried first; then every started add-on whose slug mentions
