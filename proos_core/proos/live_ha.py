...
class RestHAClient:
    ...
    def set_entry_disabled(self, entry_id: str, disabled: bool) -> dict:
        """Disable (or re-enable) a config entry. WebSocket registry write.

        Disabling stops the integration AND keeps the entry present, so HA won't
        re-discover the device and auto-create a fresh entry (the discovery flow
        aborts on the existing unique_id) — no delete/re-discover loop. Used by the
        no-auto-integration reaper to stop Sonos/Cast/etc. auto-configuring until the
        installer adds them on purpose. `disabled_by='user'` off / None to re-enable.
        """
        from .ha_ws import ws_command
        return ws_command(
            self.base_url, self._token, "config_entries/disable",
            timeout=self._timeout,
            entry_id=entry_id,
