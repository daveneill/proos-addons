...
class RestHAClient:
    ...
    # HA. The HTTP flow API mirrors what the HA frontend uses: list in-progress
    # flows, then POST a step's user_input ({} confirms a confirm-only step).
    def list_flows(self) -> list:
        """In-progress config-entry flows (HA's 'discovered' list)."""
        return self._req("GET", "/api/config/config_entries/flow") or []
