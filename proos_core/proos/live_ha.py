...
class RestHAClient:
    ...
    def integration_names(self) -> dict:
        """{domain: the platform's own name for the integration} from the
        manifests the platform has loaded (WebSocket manifest/list). REGISTER
        449: replaces a per-brand table of integration names in the watcher —
        the platform's word, never ours. Empty on any failure."""
        from .ha_ws import ws_command
        try:
            rows = ws_command(self.base_url, self._token, "manifest/list",
                              timeout=self._timeout) or []
        except Exception:  # noqa: BLE001
            return {}
        out = {}
        for m in rows:
            if isinstance(m, dict) and m.get("domain") and m.get("name"):
                out[m["domain"]] = m["name"]
        return out
