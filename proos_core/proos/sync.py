...
display are skipped (empty stays empty). create-if-absent by default, so re-running
the sync never clobbers installer edits; overwrite=True is the explicit regenerate.
...
def sync_all(client, overwrite: bool = False) -> dict:
    ...
    overwrite=False (default): create-if-absent -- only missing scripts are written,
                    installer edits survive. This is the safe re-runnable sync.
    overwrite=True : force-regenerate every generated script in every room.

    Returns:
      {
        "rooms":   [ {area, display, created[], kept[], object_ids[]}, ... ],
        "skipped": [ {area, display, skipped:<reason>}, ... ],
        "totals":  {rooms, created, kept},
      }
    """
    rooms, skipped = [], []
    created_total = kept_total = 0
