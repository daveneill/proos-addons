...
def list_areas(client) -> list[str]:
    """Every area ID the platform knows, in one server-side render (register
    441: ids, not names — the whole walk is keyed by the immutable id)."""
    raw = client.render_template("{{ areas() | list | to_json }}")
    try:
        ids = json.loads(raw)
    except Exception:
        return []
...
def sync_room(client, area: str, overwrite: bool = False) -> dict:
    """`area` is the platform's AREA ID (register 441)."""
    ...
    if not (rec and rec.get("committed") and rec.get("display")):
        return {"area": area, "area_id": (rec or {}).get("area_id"), "display": None, "skipped": "not committed"}
    try:
        ...
        return {"area": area, "area_id": rec.get("area_id"), "display": None, "skipped": f"committed regen failed: {e}"}
    return {   # register 440: the row carries the platform's area id beside the name
        "area": area, "area_id": cluster.area_id or rec.get("area_id"), "display": cluster.display.entity,
