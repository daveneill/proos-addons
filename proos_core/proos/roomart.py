...
def pictures_needed(areas, project) -> dict:
    ...
    for key, rec in sorted(recs, key=lambda kr: str(kr[1].get("area_id")
        ...
        if area is None or area.get("picture"):
            continue
        # STAGE 2a (23 Aug 2026, register 256): the kind is matched on the
        # LIVE area name — a room renamed Office -> Bedroom in HA gets a
        # bedroom at commit, not a desk. The stored copy only speaks when
        # the live row carries no name.
        _nm = (area.get("name") or rec.get("name") or aid)
        pick = None
        for url in variants(_nm):
            if url not in used:
                pick = url
                break
