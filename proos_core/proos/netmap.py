...
def load_areas(storage_dir: str = _STORAGE) -> dict:
    """{area_id: name} from HA's area registry. Skips entries with no id (a null
    key would otherwise collide and mis-file unassigned devices). Empty on
    read failure."""
    out = {}
    for a in _read(storage_dir, "core.area_registry").get("areas", []) or []:
