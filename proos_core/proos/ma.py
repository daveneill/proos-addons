...
class MaCommissioner:
    ...
    def _client(self) -> MaClient:
        ...
        # A connection that lands on MA's INGRESS listener (:8094) must carry
        # X-Remote-User-* identity headers: an anonymous connection there
        # handshakes fine (the listener is pre-authed by design) but MA drops
        # the socket the moment a privileged command runs — the "WS closed
        # mid-frame" Spotify-save failure. This happens whenever the validated
        # connection fell back to :8094 because the API port requires auth and
        # no token exists (fresh post-reset box).
        return self._mk(host, port, token)

    def _mk(self, host, port, token, timeout: float = 10.0) -> MaClient:
        """EVERY MA connection goes through here so the ingress-identity rule
        can't be bypassed: sessions and OAuth flows previously built MaClient
        directly and stayed anonymous on :8094 — which is exactly where the
        provider-save flow rode, and why it died even when one-shot commands
        were fixed."""
        iu = None
        if int(port or 0) == self.INGRESS_PORT and self.get_ingress_user:
            try:
                iu = self.get_ingress_user()
            except Exception:  # noqa: BLE001
                iu = None
        return MaClient(host, port, token, timeout=timeout, ingress_user=iu)

    def remove_provider(self, instance_id: str) -> dict:
        """Remove ONE provider instance entirely (the Pro 'Remove service'
        action) — credentials and library sync go with it. Runs on the
        identity-carrying connection, so it works on the ingress channel."""
        with self._client() as c:
            c.command("config/providers/remove", instance_id=instance_id)
    ...
    def run_provider_auth(self, provider_domain: str, session_id: str, on_auth_url,
        ...
        c.connect()
        try:
            return c.provider_auth(provider_domain, session_id, on_auth_url,
