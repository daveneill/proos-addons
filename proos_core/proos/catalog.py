...
# ── Per-site exclusions ────────────────────────────────────────────────────
# The synced master catalog is central/read-only. Tech can additionally exclude
# integrations on THIS site (hide them from installers) without editing the
# master. Effective-hidden = catalog 'hidden' tier OR locally excluded here.
EXCL = os.path.join(os.environ.get("PROOS_DATA_DIR", "/data"), "catalog_exclusions.json")
...
def save_exclusions(domains) -> list:
    try:
        ...
        _LOG.warning("exclusions save failed: %s", exc)
