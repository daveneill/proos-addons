...
def observe_snapshot(snap: dict) -> int:
    """Learn every app-capable device's published list from the sweep snapshot.

    WHY THIS AND NOT ON REQUEST (the defect this replaces, 30 Jul 2026):
    recording inside candidates() only fired when something ASKED for a room's
    apps. A Frame that is in Art Mode whenever anyone looks at it is therefore
    never recorded, and the memory stays empty forever -- which is exactly what
    happened in the Family Room. Learning has to be continuous.

    Rides the ctlbridge sweep's existing snapshot, so it costs ZERO extra HA
    traffic -- the same rule healthmon follows. Returns how many entities were
    recorded, for the log line.
    """
    n = 0
    for eid, rec in (snap or {}).items():
        if not (isinstance(eid, str) and eid.startswith("media_player.")):
            continue
        apps = _source_list(rec or {}, apps_only=True)
        if apps:
            before = _remembered_apps(eid)
            _remember_apps(eid, apps)
            if _remembered_apps(eid) != before:
                n += 1
    return n
