...
def candidates(client, project, area_id: str) -> list:
    ...
    def _add(eid, is_display, inp):
        ...
        if plat in _ANDROID_PLATFORMS:
            # Android box with no live source_list: its apps aren't configured
            # yet. The ProOS Android TV integration syncs them from the box
            # (Pro › room › Sync apps), after which they arrive as source_list
            # above like any other device.
            out.append({"entity_id": eid, "name": _fname(st, eid), "is_display": is_display,
