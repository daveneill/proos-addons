...
def candidates(client, project, area_id: str) -> list:
    ...
    def _add(eid, is_display, inp):
        ...
        if plat in _ANDROID_PLATFORMS:
            # Android box with no live source_list (its integration's
            # Applications List isn't configured): the only other honest source
            # is the certified harvest — the box's true installed apps, launched
            # by package id. Neither → shows nothing.
            harvested = []
            try:
            ...
            if harvested:
                ...
                out.append({"entity_id": eid, "name": _fname(st, eid), "is_display": is_display,
                            "input": inp, "apps": [], "apps_unknown": False, "state": state})
            return
