...
# (ANDROID_APPS — a hardcoded fallback list of Android apps — was deleted in
# register 442: the platform's entry holds a box's apps, and nothing is guessed.)
...
def launch(client, project, area_id: str, app: str, device: str = None,
            ...
            if _act:
                ...
                if _fire:
                    client.call_service("script", "turn_on",
                                        _act["entity_id"])
                    # REGISTER 447: a bounded poll for the display to read
                    # on, not a fixed sleep — the wait ends when the
                    # platform says so, and never later than the plan's bound.
                    from . import readback as _rb
                    _rb.confirm(client, disp, lambda r: str(r.get("state") or "").lower()
    ...
    # REGISTER 446: read the device back — the platform's own `source`
    # (or app_id for a package launch) — instead of telling the caller to.
    from . import readback as _rb
    if target.get("app_id"):
        want = _rb.attr_text_is("app_id", target["app_id"])
    else:
        want = _rb.attr_text_is("source", target["source"])
    rb = _rb.confirm(client, target["entity_id"], want, attrs=("source", "app_id", "app_name"))
    out = {"ok": True, "launched": target["source"], "app": target["source"],
           "device": target["entity_id"], "device_name": target["name"],
           "display": disp, "routed_input": routed,
           "confirmed": rb.get("confirmed"), "read_back": rb}
    if routed:
        rbd = _rb.confirm(client, disp, _rb.attr_text_is("source", routed), attrs=("source",))
        out["route_confirmed"] = rbd.get("confirmed")
        out["route_read_back"] = rbd
    return out
