...
# Android TV / Shield: the integration exposes NO source_list, so these devices
# never showed apps. Apps launch by PACKAGE ID via play_media — this curated map
# (same ids the dashboard remote already uses, verified working) makes Shields
# first-class app devices. The ADB harvester will replace this with the box's
# true installed list; until then the curated set covers the majors.
ANDROID_APPS = [
...
def candidates(client, project, area_id: str) -> list:
    ...
    def _add(eid, is_display, inp):
        ...
        if plat in _ANDROID_PLATFORMS:
            # Android box whose integration publishes NO list (androidtv_remote)
            # or that's asleep. Best source: the HARVESTED true installed list
            # (ADB, box's real leanback apps); else the curated map. Either way
            # apps launch by package id via play_media.
            harvested = []
            try:
                from . import shieldapps
                harvested = shieldapps.apps_for(eid)
            except Exception:  # noqa: BLE001
                harvested = []
            if harvested:
                out.append({"entity_id": eid, "name": _fname(st, eid), "is_display": is_display,
                            "input": inp, "apps": [a["name"] for a in harvested],
                            "app_ids": {a["name"]: a["package"] for a in harvested},
                            "apps_unknown": False, "state": state})
            else:
                out.append({"entity_id": eid, "name": _fname(st, eid), "is_display": is_display,
                            "input": inp, "apps": [n for n, _ in ANDROID_APPS],
                            "app_ids": {n: p for n, p in ANDROID_APPS},
                            "apps_unknown": False, "state": state})
            return
