...
# ARCHITECTURE RULE (white-label): a device's apps come ONLY from what the
# device (or its certified integration) actually exposes — live source_list,
# or the certified harvest (the box's true installed list read by ADB, our
# Shield certified-integration capability). No curated pretend-lists: if a
# device exposes nothing, it shows nothing. Device differences live in the
# certified-integration layer, never as hacks here.
...
def candidates(client, project, area_id: str) -> list:
    ...
    def _add(eid, is_display, inp):
        ...
        if plat in _ANDROID_PLATFORMS:
            # Android box: its integration publishes no list, so the ONLY valid
            # source is the certified harvest — the box's true installed apps
            # (launched by package id). Never harvested → unknown, honestly.
            harvested = []
            try:
            ...
            if harvested:
                ...
                out.append({"entity_id": eid, "name": _fname(st, eid), "is_display": is_display,
                            "input": inp, "apps": [], "apps_unknown": True, "state": state})
            return
