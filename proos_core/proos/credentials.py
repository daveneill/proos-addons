...
class CredentialStore:
    ...
    def strip(self, service: str, keys) -> bool:
        """Drop named meta keys from a service (register 439: a host or a login
        the platform owns must not stay here beside ProOS's own token). True
        when something was removed."""
        with self._lock:
            d = self._load()
            ent = d["services"].get(service)
            if not ent:
                return False
            meta = dict(ent.get("meta") or {})
            hit = [k for k in keys if k in meta]
            if not hit:
                return False
            for k in hit:
                meta.pop(k, None)
