...
def _relay_twin(eid, devs, snap):
    """THE SPEAKER'S OWN INPUT DECIDES (register 563, 22 Sep 2026).

    An engine twin is a second entity for a speaker the room already holds
    as its committed audio device. When that speaker is on an EXTERNAL
    input — its `source` is one of its own inputs (TV, Line-in) — the twin
    is idle by definition, whatever it claims: read off Dave's box, the
    Family Room twin reported playing "HOT 102.7 FM" while the Sonos was
    carrying the television. That station spoke on the room row and on
    Pro's mirror. Here the twin is silent.

    Brand-agnostic, attributes only: the twin is the entity the engine
    published (mass_player_type), assigned to the room and not committed;
    the twin shows it is the SAME BOX by listing the speaker's input in its
    own source_list (the engine exposes the provider's inputs). A twin
    actually streaming never trips this — the speaker's source is not an
    input then. A speaker with no twin is untouched."""
    d = (devs or {}).get(eid) or {}
    if d.get("role") != "assigned":
        return False
    if _attr(snap, eid, "mass_player_type") is None:
        return False
    twin_inputs = set(str(x) for x in (_attr(snap, eid, "source_list") or []))
    if not twin_inputs:
        return False
    for e, dd in (devs or {}).items():
        if e == eid or (dd or {}).get("role") in ("source", "display",
                                                  "assigned"):
            continue
        src = _attr(snap, e, "source")
        if not src:
            continue
        own = set(str(x) for x in (_attr(snap, e, "source_list") or []))
        if str(src) in own and str(src) in twin_inputs:
            return True
    return False
