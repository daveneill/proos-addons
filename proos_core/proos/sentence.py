...
def lighting_clause(on, room_name):
    ...
    as it's just lights"). Same voice as every other clause here: a
    finished phrase with the room in it, ready to join. Lights off says
    NOTHING on the home line — an unlit room is not news."""
    if not on:
        return None
    return "The lights are on in %s" % _the(room_name)
...
_WATCH = re.compile(r"^Watching\s+(.+?)\s+in\s+(.+)$", re.S)
_MUSIC = re.compile(r"^Music in\s+(.+)$", re.S)
_LIGHTS = re.compile(r"^The lights are on in\s+(.+)$", re.S)
...
def home_sentence(clauses):
    """THE HOME LINE IS ONE SENTENCE (5 Sep 2026, register 316). Dave, on
    "Watching YouTube in the Family Room · The lights are on in Jarrod's
    Room · Music in the Office": "when a bit is happening in the home it
    looks ridiculous … an actual summary all the time." The clauses were
    right; the JOIN was a list — three capitalised sentences with dots
    between them, in whatever order the rooms happened to be in.

    Pure. Takes the rooms' served home clauses (home_clause /
    lighting_clause — this module's own templates, parsed back here, so
    the two can never drift: benched) and returns ONE sentence:
      · what matters first — watching, then music, then lights, then
        anything else that is on;
      · like grouped with like — "music in the Office and the Study",
        "the lights are on in Jarrod's Room and the Bedroom";
      · commas and "and", a full stop, no dots, no bold;
      · nothing happening -> None (the surfaces keep the quiet-home
        environment line).
    Content-free, like the clauses (the home page says WHERE)."""
    watch, music, lights, other = [], [], [], []
    seen = set()
    for c in (clauses or []):
        c = str(c or "").strip()
        ...
        m = _WATCH.match(c)
        if m:
            watch.append((m.group(1).strip(), m.group(2).strip()))
            continue
        m = _MUSIC.match(c)
        if m:
            music.append(m.group(1).strip())
            continue
        m = _LIGHTS.match(c)
        if m:
            lights.append(m.group(1).strip())
            continue
        # anything else that is on ("The Family Room TV is on", "The Study
        # is on") keeps its own words, lower-cased to sit mid-sentence —
        # a possessive room name ("Bec's Office is on") keeps its capital
        if c.lower().startswith("the "):
            c = "the " + c[4:]
        other.append(c)
    parts = []
    if watch:
        parts.append("watching " + _join_and(
            ["%s in %s" % (w, p) for w, p in watch]))
    if music:
        parts.append("there's music in " + _join_and(music))
    if lights:
        parts.append("the lights are on in " + _join_and(lights))
    parts.extend(other)
    if not parts:
        return None
    s = _join_and(parts, oxford=True)
    return _cap(s) + "."
