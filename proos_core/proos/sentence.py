...
def home_sentence(facts):
    ...
    Dave, on "Watching YouTube in the Family Room · The lights are on in
    Jarrod's Room · Music in the Office": "when a bit is happening in the
    home it looks ridiculous … an actual summary all the time."

    Pure. Takes the rooms' facts (home_fact / lighting_fact dicts; a bare
    string is an old Core's clause and rides along as-is) and returns ONE
    sentence:
      ...
      · like grouped with like — "music in the Office and the Study",
        "the lights are on in Jarrod's Room and the Office" (rooms in
        name order);
      · commas and "and", a full stop, no dots, no bold;
      · nothing happening -> None (the surfaces keep the quiet-home
        environment line).
    Content-free, like the facts (the home page says WHERE)."""
    watch, music, lights, other = [], [], set(), []
    seen = set()
    for f in (facts or []):
        if isinstance(f, str):
            c = f.strip()
            if c and c not in seen:
                seen.add(c)
                other.append(("the " + c[4:]) if c.lower().startswith("the ") else c)
            continue
        if not isinstance(f, dict):
            continue
        ...
        if k == "watch" and f.get("what"):
            watch.append((str(f["what"]).strip(), where))
        elif k == "music":
            music.append(where)
        elif k == "lights":
            lights.add(where)
    ...
    parts = []
    if watch:
        parts.append("watching " + _join_and(["%s in %s" % (w, p) for w, p in watch]))
    if music:
        # "Music in the Office" — Dave, 5 Sep (register 317): "not need to
        # say 'There's', just Music in the office".
        parts.append("music in " + _join_and(_by_name(dict.fromkeys(music))))
    if lights:
        parts.append("the lights are on in " + _join_and(_by_name(lights)))
    parts.extend(other)
    if not parts:
        return None
    return _cap(_join_and(parts, oxford=True)) + "."
