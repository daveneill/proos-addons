        ...
        for key, rec in (proj.get("areas") or {}).items():
            ...
            try:
                ...
                if active_key is None and bcast is not None:   # rung 3: broadcast
                    # The display's own input testimony cuts BOTH ways: if it
                    # names a non-tuner input, the room is NOT "watching TV" --
                    # whatever is (or isn't) on that HDMI, broadcast may not
                    # claim it. Unknown input keeps the old behaviour.
