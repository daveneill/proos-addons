    ...
    def _sweep_rooms(self, snapall: dict) -> None:
        ...
        for key, rec in (proj.get("areas") or {}).items():
            ...
            try:
                ...
                if not acts:
                    # No AV activities. A committed music zone lands here and
                    # used to fall straight out of the sweep, reporting nothing
                    # (What It Cannot Do §7). decide_music() refuses anything
                    # that isn't kind='music', so a TV room with nothing added
                    # behaves exactly as it did before: silent.
