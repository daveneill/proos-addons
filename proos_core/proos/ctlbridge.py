...
class ActivityPublisher:
    ...
    def __init__(self, client, project_mod, get_controller,
        ...
        self._room_lights: dict = {}   # area_slug -> lighting FACT / None (317, facts 318)
    ...
    def _publish_lighting(self, area_name, area_slug, rec, snapall) -> None:
        ...
        so a lit bedroom reads "Lights on", never a room-wide On:
        ...
        self._publish("sensor.proos_activity_%s" % area_slug,
                      ...
                      {"area": area_name, "basis": "lighting",
                       ...
                       "sentence": "Lights on" if on else "Off",
                       "label": "Lights On" if on else "Off",
                       "home_word": "Lights on" if on else None,
                       "home_clause": _sent.lighting_clause(bool(on),
                                                            area_name),
                       "home_fact": _sent.lighting_fact(bool(on), area_name)})
    ...
    def _home_facts(self, snapall: dict) -> list:
        ...
        its clause. Then every AV/music room's lit lights (317)."""
        out = []
        ...
        for eid in eids:
            ...
            if isinstance(c, str) and c.strip():
                out.append(c.strip())
        for slug in sorted(self._room_lights):
            f = self._room_lights.get(slug)
    ...
    def _sweep_rooms(self, snapall: dict) -> None:
        ...
        for key, rec in (proj.get("areas") or {}).items():
            ...
            # LIGHTS SPEAK IN EVERY ROOM (5 Sep 2026, register 317). Dave:
            # "I have lights on in the office and its only saying Jarrod's
            # Room." A lighting-only room's lights were its whole clause
            # (281); an AV or music room's lights were never on the home
            # line at all. Now every committed room's lit lights make a
            # lighting clause for the HOME sentence (which groups them:
            # "the lights are on in the Office and Jarrod's Room"). The
            # room's own AV/music clause is untouched. Same membership law
            # as the env map; unreadable lights say nothing.
            if (rec.get("kind") or "").lower() != "lighting":
                try:
                    _lit = [e for e in (self._env_map().get(area_slug) or [])
                            if e.startswith("light.")
                            and ((snapall.get(e) or {}).get("state") or "") == "on"]
                    self._room_lights[area_slug] = (
                        _sent.lighting_fact(True, area_name) if _lit else None)
                except Exception:                                # noqa: BLE001
                    self._room_lights.pop(area_slug, None)
        ...
        # a room that left the project takes its lights off the home line
        for _slug in [k for k in self._room_lights if k not in _seen_areas]:
            self._room_lights.pop(_slug, None)
    ...
    def sweep(self) -> None:
        ...
        try:
            ...
            _hs = _sent.home_sentence(self._home_facts(snapall))
