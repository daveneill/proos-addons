...
class ActivityPublisher:
    ...
    def _home_clauses(self, snapall: dict) -> list:
        """Every room's served home clause, in a stable order (by room
        sensor id), newest truth first: a clause published THIS sweep is
        read from the publish cache; a room that did not republish keeps
        the clause HA already holds. A sensor HA no longer has (a deleted
        room) contributes nothing."""
        out = []
        ...
        for eid in eids:
            ...
            if sig is not None:
                c = sig.get("home_clause")
            else:
                c = ((snapall.get(eid) or {}).get("attributes") or {}).get("home_clause")
    ...
    def sweep(self) -> None:
        ...
        try:
            ...
            _hs = _sent.home_sentence(self._home_clauses(snapall))
