...
binary_sensor.proos_savant_host -- read from the platform's router tracker for the host's address (register 448).
...
def route_matches(a, disp_src) -> bool:
    ...
    if n1 == n2:
        return True
    # REGISTER 447: "either way containment" let HDMI 1 match HDMI 10 and
    # HDMI 11. A route still matches a LONGER display label ("HDMI 1 - Apple
    # TV", "Apple TV (HDMI 2)") — but never one where the next character is
    # another digit, and never the other way round (the display's word is
    # never a substring of ours).
    if n1 in n2:
        i = n2.index(n1) + len(n1)
        return not (i < len(n2) and n2[i].isdigit()) and not (n2.index(n1) > 0 and n2[n2.index(n1) - 1].isdigit())
    return False
...
def decide(area_slug, snap, src_acts, bcast, witmap, mem, art_check=None):
    ...
    if not _dark_now:
        for a in _eligible:                              # rung 1: verified
            try:
                _ck = a.summary(snap)
                if getattr(_ck, "ok", False):
                    active_key, active = a.key, a
                    # REGISTER 446: a verdict the activity itself reached on
                    # one signal is NOT verified — the third state.
                    if getattr(_ck, "unverified", False):
                        verified, evidence = False, "source_state"
                    break
            except Exception:
                continue
...
def _is_site_area(area):
    ...
    Register 440: the answer is the platform's own `dashboard_system` label
    on the area, never its name -- sitearea.py is the one mechanism.
    """
    from . import sitearea as _sa
    return _sa.is_site(area)
...
def _is_speaker(eid, snapall):
    """Does the PLATFORM call this media player a speaker? (register 454)

    The platform's own device_class decides — never an entity name, never a
    brand. Anything it does not call a speaker is read as AV, exactly as
    before, so a device ProOS cannot classify is never quietly re-read.
    """
    return (((snapall or {}).get(eid) or {}).get("attributes")
            or {}).get("device_class") == "speaker"


def monitored_state(aeids, snapall, pause_s=None, now=None):
    ...
    ONE VOCABULARY FOR A PAUSED SPEAKER (register 454, 14 Sep 2026). Dave,
    with nothing commissioned and every Sonos silent: "why does the bedroom
    report as on … paused for audio is off but only after the time period
    which is why we have the Auto clear when paused on each Sonos device."
    It was fixed — in musicstat, which serves COMMITTED rooms. This path
    carried a second, shorter vocabulary of its own (on/playing/paused light
    the room, idle does not, no clock), so one speaker got two verdicts
    depending on whether its room had been commissioned. On his box exactly
    one room in five showed it: a Sonos on Line-in parks at "paused" while
    the others park at "idle" — the same silence, two words from the
    integration. So the words and the clock are musicstat's, borrowed, not
    copied: for a SPEAKER only "playing" is sound, and the installer's
    per-speaker auto-clear window says when the silence becomes Off — the
    same clock the homeowner's card runs on, which is Pro's own promise
    ("the card AND the room's live status both read Off"). No label =
    Disabled = the room stays in use until it plays or powers off, also as
    Pro's screen says. A VIDEO member is untouched: a paused Apple TV is a
    room that is on.

    Pure: entity ids + a state snapshot (+ the installer's window and the
    clock) in, (state, assigned{}) out.

    pause_s -- {entity_id: auto-clear seconds}; absent = nothing clears
    now     -- epoch seconds for that window; absent = no clock, nothing clears
    """
    ...
    for eid in (aeids or []):
        ...
        if s in DEAD:
            continue
        # A SPEAKER is read as the music room reads it — the platform's own
        # device_class is the word, never a name or a brand. Anything the
        # platform does not call a speaker keeps the AV reading it always
        # had; ProOS never decides a device is audio on its own.
        if _music is not None and _is_speaker(eid, snapall):
            s = _music._eff_state(snapall or {}, eid, pause_s, now)
            asn[eid] = {"state": s}
            # Sound, or a silence the installer's clock has not cleared yet:
            # the room is still in use. Once the window passes, _eff_state
            # has already presented it as off and the room clears with it.
            if s == "playing" or s in _music._IDLE:
                asn[eid]["lit"] = True
        else:
            asn[eid] = {"state": s}
            if s in LIT:
    ...
    # 2 Aug) -- it never lights a room on its own. For a speaker the same
    # is true of 'paused' and 'stopped', and the installer's window decides
    # when that silence is reported as Off (above).
    state = "on" if any(v.get("lit") for v in asn.values()) else "off"
    for v in asn.values():
        v.pop("lit", None)
    return state, asn
...
class ActivityPublisher:
    # The parser for the add-on's witness STRING retired 14 Sep 2026
    # (register 453): a binding is committed through Pro and lives in
    # netevidence's own store — one fact, one place.
    ...
    def _publish_lighting(self, area_name, area_slug, rec, snapall) -> None:
        ...
        AND IT SERVES THE WORDS (25 Aug 2026, register 281 — Dave on the
        installed build: "it says room is on in Jarrod's Room — this is
        misleading as it's just lights"). The state is machine-readable
        truth; the WORDS are the product's. Under the ONE SPEAKER law
        (3 Aug) Core serves them and every surface repeats them verbatim,
        so a lit bedroom's chip reads "Lights On", never a room-wide On
        (its sentence is silent since 327 — lights are not summary words):

          sentence  — the room row and the homeowner's line
          label     — the chip on Health and the room hub
          home_word — the whole-home line's clause ("Lights on in
                      Jarrod's Room"), which is why it is the bare
                      clause with no room name: the caller appends it.
        """
        ...
        self._publish("sensor.proos_activity_%s" % area_slug,
                      ...
                      {"area": area_name, "basis": "lighting",
                       ...
                       # LIGHTS ARE NOT WORDS, ANYWHERE (5 Sep 2026, register
                       # 327). Dave: "make sure lights are not included
                       # across the board default in summary." The room's
                       # sentence is silent (both pages honour null as
                       # deliberate silence; the area page keeps its
                       # environment line), the home clause/word are gone;
                       # the chip keeps "Lights On", the state stays 'on',
                       # and the FACT stays published — Assist and the
                       # counts read facts, not words.
                       "sentence": None,
                       "label": "Lights On" if on else "Off",
                       "home_word": None,
                       "home_clause": None,
                       "home_fact": (dict(_sent.lighting_fact(True, area_name),
                                          area_id=area_slug) if on else None)})
    ...
    def _home_facts(self, snapall: dict) -> list:
        """Every room's served home FACT (register 318 — facts, not parsed
        sentences), in a stable order (by room sensor id), newest truth
        first: a fact published THIS sweep is read from the publish cache;
        a room that did not republish keeps the fact HA already holds. A
        sensor HA no longer has (a deleted room) contributes nothing. A
        room published by an older Core (a clause, no fact) rides along as
        its clause. (317's per-room lights table is gone — 321: the home
        line is AV only again, on Dave's word; a lighting room's own fact
        still rides its sensor and the composer leaves lights unspoken.)"""
        out = []
        eids = sorted(set(e for e in (snapall or {}) if str(e).startswith("sensor.proos_activity_"))
                      | set(e for e in self._lastattrs if str(e).startswith("sensor.proos_activity_")))
        for eid in eids:
            sig = self._lastattrs.get(eid)
            attrs = sig if sig is not None else ((snapall.get(eid) or {}).get("attributes") or {})
            f = attrs.get("home_fact")
            if isinstance(f, dict) and f.get("kind"):
                out.append(f)
                continue
            c = attrs.get("home_clause")
    ...
    def _publish_music(self, area_name: str, area_slug: str,
        ...
        try:
            ...
            if _hw:
                attrs["home_word"] = _hw
            _hf = _sent.home_fact(d["state"], d["label"], False, _mdevs,
                                  snapall, attrs.get("area"),
                                  media_app=attrs.get("media_app"),
                                  content=attrs.get("content"))
            if _hf:
                _hf["area_id"] = area_slug
                attrs["home_fact"] = _hf
                attrs["home_clause"] = _sent.render_fact(_hf)
        except Exception:                                    # noqa: BLE001
            pass
    ...
    def _publish_monitored(self, area_name, area_slug, aeids,
        ...
        """
        _state, _asn = monitored_state(aeids, snapall, self._pause_cfg(),
                                       time.time())
        if not _asn:
        ...
        try:
            ...
            if _hw:
                attrs["home_word"] = _hw
            _hf = _sent.home_fact(_state, attrs["label"], False, _sdevs,
                                  snapall, area_name)
            if _hf:
                _hf["area_id"] = area_slug
                attrs["home_fact"] = _hf
            ...
            if _el:
                attrs["env_line"] = _el
            # register 585 (23 Sep 2026): a relay twin is silent here too —
            # the same reader the sentence uses (563), one mechanism.
            attrs["music_on"] = any(
                str((v or {}).get("state")) == "playing"
                and not _sent._relay_twin(e, _sdevs, snapall)
    ...
    def _sweep_rooms(self, snapall: dict) -> None:
        ...
        for key, rec in (proj.get("areas") or {}).items():
            if not (rec and rec.get("committed")):
                continue
            # register 441: the record's key IS its area id; no name slug
            area_slug = rec.get("area_id") or key
            ...
            try:
                ...
                try:
                    ...
                    attrs["sentence"] = _sent.area_sentence(
                        ...
                        attrs.get("area"), media_app=attrs.get("media_app"),
                        content=attrs.get("content"))
                    # register 585 (23 Sep 2026): music_on told the truth
                    # the sentence had stopped telling — the Family Room's
                    # engine twin claiming a station while the speaker
                    # carried the television still counted as music. The
                    # same reader the sentence uses (563) decides here.
                    attrs["music_on"] = any(
    ...
    def _sweep_host(self) -> None:
        # REGISTER 448: the host's liveness is READ from the platform — the
        # router / UniFi device_tracker the platform holds for that address
        # (home / not_home) — not TCP-probed from Core on a port list. With
        # no tracker for the address there is nothing true to say: nothing
        # is published, and the log says so once.
        if not self.savant_host:
            return
        alive = self._host_witness(self.savant_host)
        if alive is None:
            if not getattr(self, "_host_said", False):
                self._host_said = True
                print("  [ctlbridge] no platform witness (router tracker) for the "
                      "Savant host at %s — its liveness is not published"
                      % self.savant_host, flush=True)
            return
        self._host_said = False
        self._publish("binary_sensor.proos_savant_host",
                      "on" if alive else "off",
                      {"friendly_name": "Savant Host",
                       "ip": self.savant_host,
                       "device_class": "connectivity",
                       "witness": "router tracker",
                       "icon": "mdi:server-network"})

    def _host_witness(self, ip):
        """The platform's router tracker for this address: True/False, or
        None when the platform holds none (or it has not testified)."""
        tmpl = ("{% set ns = namespace(s=none) %}"
                "{% for t in states.device_tracker %}"
                "{% if t.attributes.get('source_type') == 'router' and "
                "t.attributes.get('ip') == " + json.dumps(str(ip)) + " %}"
                "{% set ns.s = t.state %}{% endif %}{% endfor %}"
                "{{ ns.s if ns.s is not none else '' }}")
        try:
            st = (self.client.render_template(tmpl) or "").strip().lower()
        except Exception:                                        # noqa: BLE001
            return None
        if st in ("home", "on"):
            return True
        if st in ("not_home", "off"):
            return False
        return None
    ...
    def _env_map(self):
        ...
        try:
            ...
            try:
                from . import sitearea as _sa
                _ha = _sa.find(self.client.area_registry() or [])   # by label (reg 440)
            ...
            try:
                if getattr(self, "auto_home_weather", True):
                    ...
                    if cand:
                        _hme = _sa.find(self.client.area_registry() or [])
                            ...
                            try:
                                ...
                                if _jrnl is not None:
                                    _jrnl.emit("site", "env_assign", {
                                        "entity": _we,
                                        "area_id": _hme.get("area_id"),
                                        "area": _hme.get("name") or "",
                                        "note": "weather station placed in "
                                                "the site area (standard)"})
                            except Exception:            # noqa: BLE001
                                pass
                        self._wx_homed = done
            except Exception:                            # noqa: BLE001
    ...
    def sweep(self) -> None:
        ...
        # THE HOME LINE IS ONE SENTENCE (5 Sep 2026, register 316): Core
        # joins the rooms' served clauses into a single summary sentence,
        # published here as home_sentence, so both panels print the SAME
        # words instead of each stringing the clauses together with dots.
        # The clauses come from this sweep's publishes first (the cache is
        # the newest truth), then from what HA already holds.
        try:
            _hel = ""
            from . import sitearea as _sa2
            _home = _sa2.find(self.client.area_registry() or [])   # by label (reg 440)
            if _home:
                _hel = _sent.env_line(
                    self._env_map().get(_home.get("area_id"), []), snapall)
            _hf_all = self._home_facts(snapall)
            _hsegs = _sent.home_segments(_hf_all)
            _hs = "".join(x["t"] for x in _hsegs) if _hsegs else None
            self._publish("sensor.proos_home_summary",
                          "ok" if (_hel or _hs) else "empty",
                          {"area": (_home or {}).get("name") or "",
                           "area_id": (_home or {}).get("area_id"),
                           "env_line": _hel,
                           "home_sentence": _hs,
                           # 321: the same sentence in tappable pieces
                           "home_segments": _hsegs})
        except Exception:                                        # noqa: BLE001
            pass
