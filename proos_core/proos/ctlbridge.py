...
class ActivityPublisher:
    ...
    def sweep(self) -> None:
        ...
        # Learn each display's published app list off the SAME snapshot the
        # verdict ladder just used -- no extra HA calls, and it keeps learning
        # while nobody is looking, which is the only way a Frame that sits in
        # Art Mode ever gets recorded at all.
        try:
            from . import appctl as _appctl_mod
            _n = _appctl_mod.observe_snapshot(snapall)
            if _n:
                print("  [appctl] learned app lists for %d device(s)" % _n, flush=True)
        except Exception:
            pass
