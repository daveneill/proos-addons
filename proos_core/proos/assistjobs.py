...
def _spoken(rec):
    """The confirmations this turn has already said aloud."""
    runner = rec.get("runner")
    sp = list(getattr(runner, "spoken", None) or []) if runner is not None else []
    return sp or list(rec.get("spoken") or [])
...
def view(job_id: str, user_id=None) -> dict | None:
    ...
    with _LOCK:
        ...
        out = {"job": rec["id"], "state": rec["state"], "session": rec.get("session"),
               ...
               "steps": _steps(rec),
               # REGISTER 488: what the home has already said out loud while
               # this turn was still working. The runner's own list, read as it
               # is written -- the same discipline as the step list above, and
               # for the same reason: one record, not a second account of it.
               "spoken": _spoken(rec)}
        if rec["state"] == "done":
...
def start(user_id, session, text, work) -> dict:
    ...
    with _LOCK:
        ...
        rec = {"id": jid, "user": user_id, "session": session, "text": text,
               ...
               "runner": None, "trace": [], "spoken": [], "result": None, "error": None}
        _JOBS[jid] = rec
    ...
    def _run():
        try:
            ...
            with _LOCK:
                ...
                rec["spoken"] = list(getattr(rec.get("runner"), "spoken", None) or [])
                rec["runner"] = None
