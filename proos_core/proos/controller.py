...
COMMISSIONING: dict = {
    ...
    "Living Room": {
        # The AVR plan that used to live here named media_player.living_room_av_receiver
        # and media_player.living_room_shield_tv — neither exists in this house. It
        # took precedence over the committed record, so the phantom receiver became
        # the broadcast audio witness and Live TV could never be verified. Removed:
        # the room's AV switch is committed in Pro and read from the record.
...
class RoomController:
    ...
    def _audio_plan(self, cluster) -> dict | None:
        ...
        try:
            # The COMMITTED RECORD wins. The generator has always read the
            # record's AV-switch plan; the live verdict ladder was still reading
            # the legacy static dict below, so what the installer committed in
            # Pro never reached the audio witness. Same defect the record-overlay
            # comment in refresh() describes: one source of truth, both consumers.
            override = None
            try:
                from . import project as _prj
                _rec = _prj._resolve_rec(_prj.load(), self.area)
                if _rec and _rec.get("committed"):
                    override = (_prj._commissioning_from_record(_rec) or {}).get("audio")
            except Exception:
                override = None
            if override is None:
    ...
    def _off_config(self, cluster):
        ...
        try:
            from . import project  # lazy: avoids the import cycle. NOT generator —
            # that is imported at module level, and re-importing it here would
            # make the name local to this whole function (see
            # tests/import_shadow_bench.py for what that costs).
            rec = (project.load().get("areas") or {}).get(self.area)
