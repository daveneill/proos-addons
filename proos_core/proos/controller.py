...
# STAGE 5 BUILD 1 (16 Aug 2026): THE SHIPPED PRODUCT CONTAINS NO HOME.
# This dict used to carry the AUTHOR'S OWN HOUSE — Family Room / Bedroom /
# Living Room with his Apple TVs, Shields and tracker entities — and
# apply_commissioning() silently marked any customer room sharing those
# names as COMMISSIONED (provisional cleared, witnesses set to trackers
# that do not exist on their box). A product is not commissioned by its
# author's floor plan. Deleted; the dict ships EMPTY (the engine below is
# the permanent part). Per-box bindings live where they always should
# have: the committed project record (routes / AV-switch / off-state),
# the `reachability` add-on option, and the identity-joined auto-harvest
# (apply_auto_reachability) — all of which already serve every box,
# including the author's. Pinned by no_shipped_home_bench.py.
COMMISSIONING: dict = {}
...
class RoomController:
    """One room, addressed by its platform AREA ID (register 441). `area_id`
    is the key; `area` is the platform's live name for it, re-read on every
    refresh() and used for words only. A controller was keyed by a NAME for
    a year — the same room reached twice, by name and by id, ran as two
    controllers, and a rename left one of them addressing nothing."""

    def __init__(self, client, area_id: str, reachability: dict | None = None):
        self.client = client
        self.area_id = area_id
        self.area = self._live_name()
    ...
    def _live_name(self) -> str:
        """The platform's name for this area, or the id itself — never kept."""
        try:
            for a in (self.client.area_registry() or []):
                if a.get("area_id") == self.area_id:
                    return a.get("name") or self.area_id
    ...
    def _committed_cluster(self):
        ...
        (un-commissioned rooms discover for device truth only)."""
        try:
            from . import project  # lazy: avoids controller<->project import cycle
            # THE RECORD IS THE TRUTH (4 Aug 2026; by id since register 441).
            rec = project.committed_record(project.load(), self.area_id)
            if rec:
                # ONE BOX SEEN TWICE (register 315): a record that still holds
                # the folded half of a device — the invisible member behind
                # "commit didn't fully apply" — runs WITHOUT it, in memory,
                # from this start; the next Save Room removes it from disk.
                _twins = project.heal_twins(self.client, rec)
                if _twins:
                    print("  [controller] %s one device seen twice — running "
                          "without: %s (kept: %s)"
                          % (self.area, ", ".join(_twins),
                             ", ".join(sorted({(rec.get("folded") or {}).get(t, "?")
                                               for t in _twins}))),
                          flush=True)
                return project._cluster_from_record(self.client, self.area_id, rec)
        except Exception:
            pass
        return None

    def refresh(self):
        self.area = self._live_name()      # the words follow the platform
        # Committed rooms build from the DECLARED record (identical to the generator and
        # to pro.html) so the dashboard mirrors pro exactly.
        #
        # Un-committed rooms still discover, because device truth needs a
        # cluster -- but they get NO activities. The old zero-config
        # provisional path (every discovered source became a PROVISIONAL
        # CEC wake-only activity) is WITHDRAWN: COMMISSIONING earns
        # activities, monitoring is membership (Dave, 3 Aug).
        cluster = self._committed_cluster()
        # NEVER FALL BACK SILENTLY (4 Aug 2026). A room the installer has
        # COMMITTED must run on its record. If the record can't be built,
        # that is a data fault to REPORT — not a cue to guess from whatever
        # HA happens to have in the area. Guessing is what minted phantom
        # provisional activities for devices the installer never committed
        # (the Google Cast twin of a Shield), producing a "commit didn't
        # fully apply" alarm no recommit could ever clear.
        # A wrong answer is worse than none.
        self._record_fault = None
        if cluster is None:
            # A MUSIC ROOM IS NOT A FAULT (4 Aug, same day: the first cut of
            # this rule raised "the room's record can't be read" on every
            # committed music room — Office, Study, Ryan's Room — because a
            # music room legitimately has NO display. The fault is only real
            # when the record NAMES a display and still can't be built.)
            try:
                from . import project as _p
                _rec = _p._resolve_rec(_p.load(), self.area_id) or {}
                _committed = bool(_rec.get("committed") and _rec.get("display"))
            except Exception:                                # noqa: BLE001
                _committed = False
            if _committed:
                self._record_fault = (
                    "committed, but its record could not be read — the room "
                    "is not running until this is repaired")
                self.cluster = None
                self.activities = {}
                print("  [controller] %s COMMITTED but record unbuildable — "
                      "refusing to guess from discovery" % self.area,
                      flush=True)
                return
            cluster = discover_av(self.client, self.area_id)
            _uncommitted = True
        else:
            _uncommitted = False
        self.cluster = cluster
        # COMMISSIONING EARNS ACTIVITIES (Dave, 3 Aug; enforced here 4 Aug).
        # An uncommitted room is MONITORED: it reports device truth, it
        # publishes, it is controllable device by device, and Assist knows
        # everything in it. What it does NOT have is activities.
        #
        # This used to run build_watch_activities() over the DISCOVERY guess
        # -- the "zero-config provisional path", pre-reset machinery from when
        # a room was expected to work without being commissioned. That promise
        # is WITHDRAWN. It was inventing a full activity set for any room that
        # merely had equipment in it: watch_family_room_family_room_apple_tv,
        # watch_bedroom_shield_2 -- carrying script ids for scripts that do
        # not exist. Live consequences, 4 Aug (Dave: "icons on navbar and
        # nothing has been committed"):
        #   * the dashboard navbar built activity buttons from them, because
        #     they have a `script` and so passed the 4 Aug nav gate
        #   * healthmon raised "commit didn't fully apply -- activities still
        #     provisional", naming entities the installer never chose, on
        #     rooms that were never commissioned
        # The comment on the committed branch above already named this exact
        # failure. The committed path was fixed; this one kept doing it.
        if _uncommitted:
            self.activities = {}
            return
        ...
        acts = apply_commissioning(self.area_id, acts)
        ...
        try:
            ...
            _rec = _prj._resolve_rec(_prj.load(), self.area_id)
    ...
    def _audio_plan(self, cluster) -> dict | None:
        ...
        try:
            override = (COMMISSIONING.get(self.area_id) or {}).get("audio")
    ...
    def _off_config(self, cluster):
        ...
        try:
            ...
            # SAME BUG, third site (4 Aug): a raw name-keyed get against an
            # id-keyed project — so a Frame room's committed Art Mode off-state
            # was never read and the room powered the panel fully off.
            rec = project._resolve_rec(project.load(), self.area_id)
    ...
    def generate_scripts(self, overwrite: bool = False) -> dict:
        ...
        cluster = project._cluster_from_record(self.client, self.area_id, rec)
    ...
    def _script_entity_for(self, a) -> str | None:
        ...
        try:
            # STAGE 5 BUILD 2 (16 Aug 2026): this line used to slug the room
            # NAME while the generator keyed on the immutable area_id — so a
            # renamed room fired scripts that do not exist. ONE derivation
            # now, the generator's own (one_script_id_bench.py). The name
            # slug survives only for a cluster-less room, which has no
            # generated scripts to disagree with.
            # register 440: no name slug, even for a cluster-less room — a
            # room with no cluster has no generated scripts to fire.
            area_slug = generator.room_slug(self.cluster) if self.cluster else ""
            if not area_slug:
                return None
