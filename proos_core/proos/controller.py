...
class RoomController:
    ...
    def generate_scripts(self, overwrite: bool = False) -> dict:
        ...
        # The room's committed AV config (project record) is the ONLY generator of activities.
        # Generate from the SAME record cluster + committed routes/off-state the commit uses, so
        # startup, commit and the whole-home self-heal are byte-identical. A room that isn't
        # committed generates nothing (no discovery / static commissioning fallback) -- that's
        # what stopped the periodic self-heal from refreshing a committed script back to the
        # routeless CEC-only default and dropping the discrete HDMI input.
