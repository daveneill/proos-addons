...
def tune(client, device_id: str, num: str) -> dict:
    ...
    # REGISTER 446: read the tuner back through the platform's own
    # media_channel / media_content_id; digits typed on a remote may leave
    # nothing to read — then the answer is unverified, not "done".
    from . import readback as _rb

    def _want(r):
        a = r.get("attributes") or {}
        for k in ("media_channel", "media_content_id"):
            if a.get(k) is not None:
                return str(a.get(k)).strip() == num
        return None
    rb = _rb.confirm(client, eid, _want, attrs=("media_channel", "media_content_id", "media_title"))
