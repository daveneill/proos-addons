...
Credential storage (CredentialStore, service "unifi_protect"):
  token = the Integration API key — ProOS's own, minted inside Protect for
  the integration API. REGISTER 439: the console's host, port, verify_ssl and
  the local username/password are the UniFi Protect INTEGRATION's own facts,
  configured in the integration and read from its entry on every call; Core
  keeps no copy (the meta that used to hold them is stripped at boot). When
  the platform's entry carries an api_key of its own, that one is used.
"""
...
from . import platform_entries as _pe

_REACH = _reach.Backoff("UniFi Protect console")   # register 437
_DOMAIN = "unifiprotect"
PLATFORM_KEYS = ("host", "port", "username", "password", "verify_ssl")   # the integration's, never Core's
...
class ProtectClient:
    ...
    # ---- config ----
    def _conf(self):
        """(key, host, verify, user, pw): host/verify/user/pw from the platform's
        own unifiprotect entry, every call; the key from the entry when it has
        one, else ProOS's own store."""
        d = _pe.data(_DOMAIN)
        key = (d.get("api_key") or "").strip() or self._creds.get(SERVICE)
        host = str(d.get("host") or "").strip().rstrip("/").replace("https://", "").replace("http://", "")
        port = d.get("port")
        if host and port and int(port) not in (443, 0) and ":" not in host:
            host = "%s:%s" % (host, int(port))
        return (key, host or None, bool(d.get("verify_ssl")),
    ...
    def status(self) -> dict:
        ...
        e = _pe.entry(_DOMAIN) or {}
        out = {"configured": bool(key and host), "host": host, "verify_ssl": verify,
               "has_login": bool(user and pw), "username": user,
               # register 439: the address and login are the integration's own
               "entry_id": e.get("entry_id"), "entry": bool(e),
               "key_from_entry": bool((_pe.data(_DOMAIN).get("api_key") or "").strip()),
               "has_key": bool(key)}
        if key and host:
            try:
    ...
    def set_key(self, api_key) -> dict:
        """Keep ProOS's own Integration API key — and nothing else. The
        console's address and login live in the platform's entry (register 439)."""
        api_key = (api_key or self._creds.get(SERVICE) or "").strip()
        if not api_key:
            raise ProtectError(400, "api_key is required")
        self._creds.put(SERVICE, api_key, name="UniFi Protect")
        self._session = None
        return self.status()

    def strip_platform_copies(self) -> bool:
        """Register 439, once at boot: the store used to hold host / username /
        password / verify_ssl beside the key. Those are the integration's;
        drop them, keep the key."""
        return bool(self._creds.strip(SERVICE, PLATFORM_KEYS))
    ...
    def _session_login(self):
        ...
        try:
            _REACH.check()
        except _reach.Unreachable as e:
            raise ProtectError(502, str(e))
        ...
        try:
            with urllib.request.urlopen(req, timeout=_reach.TIMEOUT, context=ctx) as r:
                _REACH.ok()
                token = None
            ...
            _REACH.failed(f"cannot reach console at {host}: {getattr(e, 'reason', e)}")
            raise ProtectError(502, f"cannot reach console for login: {getattr(e, 'reason', e)}")

    # ---- transport ----
    def _req(self, method: str, base: str, path: str, *, body=None,
             query: dict | None = None, raw: bool = False, timeout: int = _reach.TIMEOUT, _retry: bool = True):
        key, host, verify, user, pw = self._conf()
        if not (key and host):
        ...
        try:
            ...
            _REACH.failed(f"cannot reach console at {host}: {getattr(e, 'reason', e)}")
            raise ProtectError(502, f"cannot reach console: {getattr(e, 'reason', e)}")
...
def handle(method, parts, qs, body, creds, layer=None):
    ...
    try:
        ...
        if p == ["config"] and method == "POST":
            # REGISTER 439. Two shapes. With a platform entry present: only the
            # Integration API key may be given (the address and login are the
            # integration's — reconfigure it there). With no entry yet: this is
            # the front door — the host, local user and password go STRAIGHT
            # into the platform's own setup flow (the entry becomes the store)
            # and Core keeps the key only.
            b = body or {}
            key = (b.get("api_key") or b.get("apiKey") or "").strip()
            if key:
                client.set_key(key)
            host = str(b.get("host") or "").strip()
            out = client.status()
            if host and not out.get("entry"):
                if layer is None:
                    return 503, "application/json", {"errorMessage": "integration layer not started"}
                res = layer.ensure_integration(
                    api_key=(key or client._conf()[0] or ""),
                    username=str(b.get("username") or ""), password=str(b.get("password") or ""),
                    host=host, port=int(b.get("port") or 443))
                out = client.status()
                out["setup"] = res
            elif host and out.get("entry"):
                out["note"] = "the console address and login are set in the UniFi Protect integration; reconfigure it there"
            if not out.get("has_key"):
                return 400, "application/json", {"errorMessage": "api_key is required"}
            return 200, "application/json", out
        if p == ["disconnect"] and method == "POST":
            # Full unlink: forget the stored console credentials + curation. The
            # native config entry is removed separately (HA side) by the caller.
            try: creds.delete(SERVICE)
            except Exception: pass
            try:
                if os.path.exists(CURATION_STORE): os.remove(CURATION_STORE)
            except Exception: pass
            return 200, "application/json", {"ok": True, "disconnected": True}
...
class UnifiLayer:
    ...
    def ensure_integration(self, api_key: str = "", username: str = "",
                           password: str = "", host: str = "", port: int = 443) -> dict:
        ...
        # Not configured yet → prefer a discovery flow HA raised for the console
        # (it pre-fills host/port). If there is none — e.g. after the integration
        # was removed and re-added, HA won't re-offer discovery — start our own
        # USER flow and supply host/port ourselves.
        try:
        ...
        if not flow:
            try:
                flow = self.client.start_flow(UNIFI_DOMAIN)
            except Exception as e:  # noqa: BLE001
                return {"action": "error", "loaded": False, "error": f"start flow: {e}"}
            if not flow or not flow.get("flow_id"):
                return {"action": "waiting", "loaded": False,
                        "detail": "Could not start the UniFi Protect setup flow."}
        ...
        try:
            ...
            step = flow
        payload = self._payload_for(step, api_key, username, password, host, port)
    ...
    def _payload_for(self, step: dict, api_key: str, username: str,
        ...
        for f in (step.get("data_schema") or []):
            ...
            if n:
                names.add(n)
        payload: dict = {}
        # host/port: a discovery step pre-fills these, but a USER flow (which we
        # start ourselves after a remove) declares them as required — supply them.
        if host and "host" in names:
            payload["host"] = host
        if "port" in names:
            payload["port"] = int(port or 443)
        ...
        # REGISTER 449: verify_ssl is NOT decided here. The integration's own
        # flow carries its default for that field (read from the step's
        # schema, so an integration that changes its default is followed),
        # and the installer changes it in the integration's Reconfigure door
        # like any other setting; Core's clients read whatever the entry
        # holds (register 439) — nothing is assumed off.
        for f in (step.get("data_schema") or []):
            if f.get("name") == "verify_ssl" and "default" in f:
                payload["verify_ssl"] = f.get("default")
        return payload
    ...
    def report(self) -> dict:
        ...
        try:
            # Count physical cameras (one primary channel each), not every entity
            # variant — HA exposes high/medium/low/insecure/package per camera.
            cams = [e for e in self.client.integration_entities(UNIFI_DOMAIN)
                    if e.startswith("camera.") and e.endswith("_high_resolution_channel")]
        except Exception:
            cams = []
...
def curation_view(client) -> dict:
    ...
    for c in raw:
        ...
        cams.append({
            ...
            # room-assignment is the universal gate (enforced dashboard-side);
            # curation only carries UniFi roles now.
            "exposed": True,
