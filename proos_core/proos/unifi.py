...
# HARD CONCURRENCY CAP on UniFi work. The dashboard's events grid can fire a request per
# event (thumbnails), and Core's HTTP server spawns a thread per request with no limit — so a
# big grid meant dozens–hundreds of threads each reading image bytes / parsing event JSON into
# memory at once, which OOM-killed the add-on (SIGKILL, no traceback → the restart loop). This
# semaphore bounds the number of console requests actually in flight; excess requests wait
# their turn (cheaply) instead of all allocating at once. The single ceiling that makes Core
# un-crashable regardless of how the client behaves or how many cameras exist.
_REQ_SEM = threading.Semaphore(6)
...
def handle(method, parts, qs, body, creds):
    ...
    # Bound concurrent console work so a request storm can never OOM the add-on. If too many
    # are already in flight, wait briefly for a slot rather than piling on; give up with a
    # 503 the client can retry instead of dragging Core down. Cheap requests (status/config)
    # rarely hit the ceiling; the storm is thumbnails, which this throttles.
    if not _REQ_SEM.acquire(timeout=20):
        return 503, "application/json", {"errorMessage": "UniFi proxy busy — retry shortly"}
    try:
        # config / status
        ...
        if p == ["events"] and method == "GET":
            # ALWAYS bound the event count. An unbounded query over a wide window with many
            # cameras returns thousands of events -- large enough to spike this add-on's memory
            # serialising them and to overwhelm a client rendering them. A caller may ask for
            # fewer via ?limit=, but never more than the hard cap, and an absent limit defaults
            # to a sane page rather than "everything".
            _lim = q1("limit")
            try:
                _lim = int(_lim)
            except (TypeError, ValueError):
                _lim = 200
            _lim = max(1, min(_lim, 1000))
            _evs = client.events(
                start=q1("start"), end=q1("end"), types=q1("types"),
                cameras=q1("cameras"), limit=_lim)
            try:
                _n = len(_evs) if isinstance(_evs, list) else (
                    len(_evs.get("events")) if isinstance(_evs, dict) and isinstance(_evs.get("events"), list) else -1)
            except Exception:
                _n = -2
            print("[unifi] events start=%s end=%s limit=%s -> %s items (type=%s)"
                  % (q1("start"), q1("end"), _lim, _n, type(_evs).__name__), flush=True)
            return 200, "application/json", _evs
        ...
        print("[unifi] %s /%s -> ProtectError %s: %s" % (method, "/".join(parts), e.status, e.message), flush=True)
        return e.status, "application/json", {"errorMessage": e.message}
    except Exception as e:  # noqa: BLE001 — never 500 the whole server
        import traceback
        print("[unifi] %s /%s -> %s\n%s" % (method, "/".join(parts), e, traceback.format_exc()), flush=True)
