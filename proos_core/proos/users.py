...
def create_user(ws_call, name: str, role: str = "user",
    ...
    # A HOMEOWNER IS A PERSON (12 Sep 2026, register 429 — stage M3 of the
    # memory plan). Pro Assist's memory and conversation record are kept per
    # LOGIN, and that is right: the product's model is one login per
    # household member. But creating a homeowner made a login and nothing
    # else — the person record (name, picture, tracker) only appeared later,
    # if a picture or a tracker was ever set. So "who is this login" had no
    # answer on the box until something incidental created one. Now the
    # person is made with the login, in one act, and the Users page says
    # plainly: one login per person. Idempotent; a failure here never fails
    # the login that was just created.
    person_id = None
    if group == GROUP_USER:          # a household login, by whichever role name
        try:
            existing = _person_by_user(ws_call, user_id)
            if existing:
                person_id = existing.get("id")
            else:
                made = ws_call("person/create", name=name, user_id=user_id,
                               device_trackers=[]) or {}
                person_id = (made.get("id") if isinstance(made, dict) else None)
        except Exception as exc:                                   # noqa: BLE001
            _LOG.warning("users - person record for %s not created: %s", name, exc)
...
def rename_user(ws_call, user_id: str, name: str) -> dict:
    """Change a user's DISPLAY name — native HA everywhere: the auth user's
    name (what sign-in surfaces show) AND the linked person's name (what the
    dashboard's avatar/name mirror and presence read), kept in lockstep so the
    two can never drift apart. The @username is deliberately untouched: it is
    the login identity credential; changing it silently would lock people out
    of saved sign-ins."""
    name = (name or "").strip()
    if not name:
        raise RuntimeError("name required")
    ws_call("config/auth/update", user_id=user_id, name=name)
    p = _person_by_user(ws_call, user_id)
    if p:
        try:
            ws_call("person/update", person_id=p.get("id"), name=name,
                    user_id=user_id, device_trackers=p.get("device_trackers") or [])
        except Exception:
            pass  # auth rename already applied; person mirror is best-effort
...
def attach_tracker(ws_call, user_id: str, tracker: str) -> dict:
    """Attach a device_tracker to the user's HA person (creating the person if
    needed) so GPS reports flow into native person/zone state. Idempotent."""
    p = _person_by_user(ws_call, user_id)
    if not p:
        ws_call("person/create", name=_name_for(ws_call, user_id),
                user_id=user_id, device_trackers=[tracker])
        return {"user_id": user_id, "tracker": tracker, "attached": True}
    dts = list(p.get("device_trackers") or [])
    if tracker in dts:
        return {"user_id": user_id, "tracker": tracker, "attached": False}
    dts.append(tracker)
    ws_call("person/update", person_id=p.get("id"), name=p.get("name"),
            user_id=user_id, device_trackers=dts, picture=p.get("picture"))
    return {"user_id": user_id, "tracker": tracker, "attached": True}
