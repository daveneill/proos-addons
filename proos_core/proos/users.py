...
def create_user(ws_call, name: str, role: str = "user",
    ...
    return {
        "user_id": user_id, "username": username, "password": password,
        "role": role, "admin": group == GROUP_ADMIN,
    }
...
def set_role(ws_call, user_id: str, role: str) -> dict:
...
def _normalise(u: dict) -> dict:
    ...
    return {
        ...
        "role": "admin" if GROUP_ADMIN in groups else ("readonly" if GROUP_READONLY in groups else "user"),
    }
