...
# ── THE TWO VOLUME ENDPOINT LISTS (Endpoint Model Spec v2 §2, register 555) ──
# Dave: "there were audio end points saved per room and that volume was
# defaulted to these now playing cards."
#
# The v2 record has had video_volume[] and audio_volume[] since 6 August, and
# _normalise() populates both for every committed room — but NOTHING CARRIED
# THEM TO THE HOMEOWNER DASHBOARD. /project is installer-gated (403 to a
# homeowner), and the endpoint's own comment says so: "the dashboard consumes
# membership via the mirrored HA labels, not this endpoint." Only tvaudio was
# ever mirrored, and tvaudio is video_volume[0] — ONE entity out of two lists.
# So the dashboard re-derived the room's volume from a brand ladder (Sonos,
# then HEOS, then HomePod, then device_class) instead of reading what the
# installer committed.
#
# NO NEW ENDPOINT AND NO NEW MECHANISM (register 495: if the platform already
# does it, use the platform). These are mirrored exactly as proos_tvaudio has
# always been: entity labels, managed by this module, reconciled on every
# commit and every boot, removed when a room is de-committed.
#
# BOTH LISTS ARE MIRRORED IN FULL, not their first entry. v2 §2 makes each a
# LIST on purpose — "TV sound to an AVR *and* a second-zone amp is allowed" —
# and mirroring only [0] is the exact mistake tvaudio already made.
LABEL_VIDEO_VOLUME = "proos_video_volume"
LABEL_AUDIO_VOLUME = "proos_audio_volume"
_ROLE_LABELS = {LABEL_AV, LABEL_DISPLAY, LABEL_SOURCE, LABEL_TVAUDIO, LABEL_SPEAKER,
                LABEL_AVSWITCH, LABEL_VIDEO_VOLUME, LABEL_AUDIO_VOLUME}
...
def _derive_kind(rec: dict) -> None:
    ...
    if rec.get("committed"):
        # 24 Aug 2026 (register 280): a third kind joins the two. A saved
        # room with a stored 'lighting' hint and NO AV members derives
        # 'lighting' — its verdict is measured from its assigned lights.
        # Any committed AV member outranks the hint (the 1 Aug law:
        # membership feeds the flag, candidates never do), and every
        # record that existed before this rule derives EXACTLY as it
        # always did: display -> tv, otherwise music. A keep is measured.
        if rec.get("display"):
            rec["kind"] = "tv"
        elif (rec.get("audio") or rec.get("speakers")
              or rec.get("tvaudio") or rec.get("sources")):
            rec["kind"] = "music"
        elif (rec.get("kind") or "").lower() == "lighting":
            rec["kind"] = "lighting"
        else:
            rec["kind"] = "music"
...
def save(project: dict, path: str = DEFAULT_PATH) -> dict:
    ...
    # STAGE 2b (23 Aug 2026, register 257): ONE identity per room record.
    # The identity lived twice — as the dict key AND as the area_id field —
    # and the two could disagree (legacy records were keyed by room NAME
    # with the id in the field; later records the reverse), so every
    # consumer wrote `rec.get("area_id") or key` and hoped. Every save now
    # normalizes: a record whose field names a DIFFERENT identity is
    # re-keyed to it (the same direction the suggestion merge has always
    # keyed), then the field is stamped from the key — after any save the
    # two are byte-equal and there is nothing left to drift. A re-key that
    # would land on an EXISTING record is left alone, field untouched: two
    # records claiming one identity is the duplicate guard's case to name,
    # never a silent merge or a silent erase.
    _areas = project.get("areas") or {}
    for _k in sorted(list(_areas)):
        _r = _areas.get(_k)
...
def _area_record(client, cluster) -> dict:
    ...
    return {
        ...
        # One box seen twice (register 315): the half discovery folded away,
        # so the UI can say "seen twice as …" instead of offering it.
        "folded": dict(getattr(cluster, "folded", None) or {}),
        "kind": kind,
        "warnings": [],
        "committed": False,
    }
...
def drop_twins(rec: dict, folded: dict) -> list:
    """ONE BOX SEEN TWICE, IN A COMMITTED RECORD (register 315). Pure.
    Removes every folded entity from the record's member lists — sources,
    audio, speakers, tvaudio, inputs, the AV-switch input map — and returns
    what was dropped. The DISPLAY and the AV SWITCH are never dropped: the
    installer chose them by hand and a room without its screen is a worse
    fault than a twin; those are reported, not removed."""
    dropped = []
    if not rec or not folded:
        return dropped
    keep = {rec.get("display"), ((rec.get("avswitch") or {}).get("entity"))}
    gone = {e for e in folded if e and e not in keep}
    if not gone:
        return dropped
    ...
    def _sub(lst):
        ...
        for item in (lst or []):
            eid = item.get("entity") if isinstance(item, dict) else item
            if eid in gone:
                if eid not in dropped:
                    dropped.append(eid)
                continue
            out.append(item)
        return out
    for k in ("sources", "audio", "speakers", "displays", "video_volume",
              "audio_volume"):
        if rec.get(k) is not None:
            rec[k] = _sub(rec.get(k))
    if rec.get("tvaudio") in gone:
        if rec["tvaudio"] not in dropped:
            dropped.append(rec["tvaudio"])
        rec["tvaudio"] = None
    if isinstance(rec.get("inputs"), dict):
        rec["inputs"] = {k: v for k, v in rec["inputs"].items() if k not in gone}
    sw = rec.get("avswitch")
    if isinstance(sw, dict) and isinstance(sw.get("inputs"), dict):
        sw["inputs"] = {k: v for k, v in sw["inputs"].items() if k not in gone}
    slots = rec.get("slots")
    if isinstance(slots, dict):
        for key in ("video", "audio", "video_audio", "video_volume", "audio_volume"):
            if isinstance(slots.get(key), list):
                slots[key] = [x for x in slots[key]
                              if not (isinstance(x, dict) and x.get("entity") in gone)]
    if dropped:
        rec["folded"] = dict((rec.get("folded") or {}),
                             **{e: folded[e] for e in dropped})
    return dropped


def heal_twins(client, rec: dict) -> list:
    """Read the device facts for a committed record's members, fold, drop.
    Returns the dropped entities (empty when the record is clean or the
    facts could not be read — no facts, no fold). Never raises."""
    try:
        members = _member_eids(rec)
        if len(members) < 2:
            return []
        _, folded = discovery.fold_twins(discovery.device_facts(client, members),
                                         members)
        return drop_twins(rec, folded)
...
def _assigned_lights(client) -> dict:
    """area_id -> [light entity_ids] by ASSIGNMENT (entity-level override,
    else the device's area) — the same membership law the env map and
    Assist speak (Dave, 3 Aug: 'the devices ASSIGNED to the area').
    Best-effort: a registry hiccup returns {} and suggest simply offers
    what the AV scan found, exactly as before."""
    try:
        devs = {d.get("id"): d.get("area_id")
                for d in (client.device_registry() or [])}
        out = {}
...
def suggest(client) -> dict:
    ...
    LIGHTS-ONLY ROOMS ARE ROOMS (24 Aug 2026, register 280 — Dave's
    ruling at his own press: two assigned WiZ lights, watched two ways
    on Health, and the room page refused to save with 'Add a device
    first'). An area the AV scan finds empty but which HAS assigned
    lights earns a record too — kind 'lighting', empty AV slots, no
    candidates, nothing auto-joined. AV ROLES stay strict opt-in
    exactly as v1.0.74 ruled (roles are choices); non-role membership
    is assignment — 'MEMBERSHIP IS THE ROOM'S ASSIGNMENT', and
    'MONITORING IS MEMBERSHIP; commissioning earns activities'."""
    areas = {}
    for aid in sync.list_areas(client):          # area IDS (register 441)
        try:
            cluster = discover_av(client, aid)
        except Exception:
            continue
        rec = _area_record(client, cluster)
        cand = rec.get("candidates") or {}
        if cand.get("display") or cand.get("sources") or cand.get("audio"):
            areas[rec.get("area_id") or aid] = rec   # key by the IMMUTABLE area_id, never the name
    lit = _assigned_lights(client)
    if lit:
        try:
            _names = {a.get("area_id"): a.get("name")
                      for a in (client.area_registry() or [])}
        except Exception:  # noqa: BLE001
            _names = {}
        for aid in lit:
            if aid in areas:          # the AV record already covers it
                continue
            areas[aid] = {"area_id": aid, "name": _names.get(aid) or aid,
                          "kind": "lighting", "candidates": {},
                          "display": None, "sources": [], "audio": [],
                          "speakers": [], "inputs": {}}
...
def merge(existing: dict, suggestion: dict) -> dict:
    ...
    # Both sides are keyed by the immutable area_id (register 441: the name
    # match is gone — a record an older build keyed by a name is moved onto
    # its live id by reconcile_area_ids, from where its devices actually sit,
    ...
    for area, rec in suggestion.get("areas", {}).items():   # 'area' is the area_id
        ...
        if prior:
            ...
            # One box seen twice (register 315): the fresh scan's fold is a
            # discovery fact, refreshed like the candidates; and the stored
            # record is healed of any twin it still holds as a member.
            _fold = dict(rec.get("folded") or {})
            drop_twins(merged, _fold)
            merged["folded"] = {**(merged.get("folded") or {}), **_fold}
    ...
    # area_id; skip any already present so a room can't appear twice.
...
def label_plan(project: dict) -> dict:
    ...
    for area, rec in (project or {}).get("areas", {}).items():
        ...
        # THE VOLUME ENDPOINTS, IN FULL (v2 §2, register 555). _normalise()
        # guarantees both keys on a committed record: video_volume falls back
        # to the video-audio owner and audio_volume to the audio endpoints, so
        # a room commissioned before the volume slots existed mirrors the same
        # answer it has always behaved as. proos_tvaudio above is UNCHANGED and
        # still carries video_volume[0] — every existing reader keeps working.
        for e in (rec.get("video_volume") or []):
            plan.setdefault(e, set()).add(LABEL_VIDEO_VOLUME)
        for e in (rec.get("audio_volume") or []):
            plan.setdefault(e, set()).add(LABEL_AUDIO_VOLUME)
...
def _live_platforms(client, entities) -> dict:
    """entity_id -> the platform's own `platform` (integration domain) for each
    entity, from the live entity registry (register 442). {} when it cannot be
    read — callers then fall back to the record's last scan."""
    if not entities:
        return {}
    try:
        want = set(entities)
        return {e.get("entity_id"): e.get("platform")
                for e in (client.entity_registry() or [])
                if e.get("entity_id") in want and e.get("platform")}
    except Exception:  # noqa: BLE001
        return {}
...
def _cluster_from_record(client, area, rec):
    ...
    if display_is_source:
        try:
            ...
            display_pos = None
    _eids = ([display_e] if display_e else []) + src_es + aud_es
    names = _friendly_names(client, _eids)
    plats = _live_platforms(client, _eids)    # the platform's own word, live (reg 442)
    meta = rec.get("meta") or {}
    def dev(e):
        m = meta.get(e) or {}
        integ = plats.get(e) or m.get("integration", "unknown")
        # the tier follows the LIVE integration, never the scan's copy
        return discovery.Device(e, names.get(e) or e, integ,
                                None, discovery.tier(integ) if plats.get(e) else m.get("tier", "compatible"))
    # The area id is the key (register 441): the record's, else the key the
    # caller addressed the room by. The name is the platform's LIVE name for
    # that id — read now, never the record's copy.
    aid = (rec.get("area_id") or "").strip() or str(area or "").strip()
    live_name = ""
    try:
        live_name = (client.render_template("{{ area_name(%s) or '' }}" % json.dumps(aid, ensure_ascii=False)) or "").strip()
    except Exception:  # noqa: BLE001
        live_name = ""
    return discovery.AVCluster(
        area=(live_name or rec.get("name") or aid),
        area_id=aid,
...
def reconcile_area_ids(project, areas, entities, devices):
    """STAGE 1 of "commit keeps only what HA can't know" (23 Aug 2026,
    register 254; Dave's go). Re-commit could never clear a RECORD-side
    drift: the button posted back the record it already held — dead room
    identity included — and the drift check compared the same two facts
    again. Six identical Commits on Dave's Master Log, one immortal
    incident.

    At save time this re-reads the home: a COMMITTED room whose stored
    area_id no longer exists in the registry, and whose members now sit
    together in exactly ONE live area, is re-keyed to that area, its name
    refreshed from the registry (display follows the live home; roles,
    members, activities config and the committed flag ride verbatim on the
    same record object). Members scattered across several areas, or a
    target key already holding a committed record, are LEFT ALONE for the
    drift incident to name — a guess is not a repair. An empty registry
    moves nothing (blind is not broken). Pure: no I/O; mutates `project`
    in place; returns [{"from", "to", "name"}] for the caller's record.
    """
    from . import membership
    live = {a.get("area_id"): a.get("name") for a in (areas or [])
            if isinstance(a, dict) and a.get("area_id")}
    if not live:
        return []
    dev_area = {d.get("id"): d.get("area_id") for d in (devices or [])
                if isinstance(d, dict)}
    ent_by_id = {e.get("entity_id"): e for e in (entities or [])
                 if isinstance(e, dict)}
    recs = (project or {}).get("areas") or {}
    moved = []
    for key in sorted(list(recs)):
        rec = recs.get(key)
        ...
        if aid in live:
            continue
        cur = set()
        for eid in _area_members(rec):
            row = ent_by_id.get(eid)
            if row is None:
                continue
            a = membership.area_of(row, dev_area)
            if a:
                cur.add(a)
        if len(cur) != 1:
            continue                    # scattered or unseen: not a repair
        new = cur.pop()
        if new not in live:
            continue
        target = recs.get(new)
        if isinstance(target, dict) and target.get("committed"):
            # STAGE 1b (23 Aug 2026, register 255). Dave committed the LIVE
            # room — the only button the UI offers — and the save was
            # refused as a duplicate: the dead record still held the same
            # device. Two records, one room. HA's own pattern is to clean
            # references at the moment an identity dies and never to hold a
            # room twice; so a GHOST — a dead-identity record whose every
            # device the committed live room already holds — is retired,
            # and the live room (carrying the installer's fresh roles)
            # survives. A dead record holding anything the live room LACKS
            # is NOT a ghost: two real rooms, left alone, and the duplicate
            # refusal stands and names them.
            def _harvest(r):
                out = set()
                for e in ([r.get("display"), r.get("tvaudio")]
                          + list(r.get("displays") or [])
                          + list(r.get("sources") or [])
                          + list(r.get("audio") or [])
                          + list(r.get("speakers") or [])):
                    if isinstance(e, dict):
                        e = e.get("entity")
                    if isinstance(e, str) and e:
                        out.add(e)
                return out
            if _harvest(rec) <= _harvest(target):
                del recs[key]
                moved.append({"from": aid, "to": new,
                              "name": live.get(new) or target.get("name"),
                              "retired": True})
            continue
        rec["area_id"] = new
        rec["name"] = live.get(new) or rec.get("name")
        recs[new] = rec
        if key != new:
            del recs[key]
        moved.append({"from": aid, "to": new, "name": rec.get("name")})
    return moved
...
def _resolve_rec(project, key):
    """Find a room record by its platform AREA ID — and nothing else
    (register 441). The name tolerance that lived here for a year let a
    record be found by a stale copy of a name; a name is resolved to an id
    at the door (server.resolve_area_id) before it reaches this."""
    areas = (project or {}).get("areas", {})
    if key in areas:
        return areas[key]
    for r in areas.values():
        if r and r.get("area_id") == key:
            return r
    return None
...
def heal_records(client, project_dict=None) -> dict:
    """Rebuild every room whose LABELS show a commission but whose record is
    missing or unreadable. The labels ARE the commission, so the room heals
    itself instead of demanding a recommit that (Family Room, 4 Aug) could
    not fix it anyway.

    A WRITE, so it runs only where a write is expected: the explicit scan
    (POST /project/suggest) and the Health incident's Rebuild action
    (POST /project/heal). Never on a read/navigation path.

    Returns {"healed": [room names], "checked": n}. Never raises.
    """
    out = {"healed": [], "checked": 0}
    try:
        ...
        areas = {a.get("area_id"): a.get("name")
                 for a in (client.area_registry() or [])}
        stored = project_dict if project_dict is not None else load()
        for aid, nm in areas.items():
            ...
            srcl = {}
            for e in mine:
                if LABEL_DISPLAY in (e.get("labels") or []):
                    try:
                        snap = client.snapshot([e["entity_id"]]) or {}
                        sv = snap.get(e["entity_id"]) or {}
                        srcl[e["entity_id"]] = ((sv.get("attributes") or {})
                                                .get("source_list") or [])
                    except Exception:                        # noqa: BLE001
                        srcl[e["entity_id"]] = []
            new = record_from_labels(mine, aid, nm, srcl)
            if new:
                stored.setdefault("areas", {})[aid] = new
                out["healed"].append(nm or aid)
        if out["healed"]:
            save(stored)
            try:
                from . import journal as _j
                for nm in out["healed"]:
                    _j.emit(nm, "record_healed",
                            {"note": "the room's record was missing; rebuilt "
                                     "from its committed labels"})
            except Exception:                                # noqa: BLE001
                pass
            print("  [project] record self-heal: %s" % out["healed"],
                  flush=True)
...
def activities_status(client, project, area) -> dict:
    ...
    try:
        cluster = _cluster_from_record(client, area, rec)
        area_slug = generator.room_slug(cluster)
    except Exception:  # noqa: BLE001
        area_slug = rec.get("area_id") or ""
    if not area_slug:
        return {"activities": []}            # no id, no scripts (register 440/441)
...
def sources_without_input(proj: dict):
    ...
    it needs to not be possible." A source in a committed TV room must
    have an input — on the display (rec.inputs) or on the AV switch
    (rec.avswitch.inputs). The display itself and the AV switch entity are
    not sources that need one. Returns [{"room", "entity"}] for every
    offender across the project; empty = the save may proceed. Pure;
    benched by tests/source_no_input_bench.py."""
    out = []
    for key, rec in (proj or {}).get("areas", {}).items():
        ...
        if not display:
            continue                       # a music room has no routes
        ...
        for e in (rec.get("sources") or []):
            ...
            if not isinstance(e, str) or not e or e == display or e == sw_ent:
                continue
            if not (inputs.get(e) or sw_in.get(e)):
                out.append({"room": rec.get("name") or key, "entity": e})
    return out
