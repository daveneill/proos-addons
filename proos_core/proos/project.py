...
# Config entries the no-auto-integration reaper has already acted on (disabled once). We
# reap ONCE per entry so that when the installer later ENABLES it to use it, the reaper
# doesn't re-disable it next scan (the entry's source stays 'zeroconf' forever). Cleared on
# a factory reset so a fresh box re-reaps its freshly auto-configured entries.
REAP_PATH = os.environ.get("PROOS_REAP_PATH", "/data/proos_reaped.json")
SCHEMA_VERSION = 1
...
def clear(path: str = DEFAULT_PATH) -> bool:
    ...
    try:
        ...
        ok = False
    # Drop the reaper memory too, so the fresh box re-reaps its newly auto-configured entries.
    try:
        os.remove(REAP_PATH)
    except FileNotFoundError:
        pass
...
def quarantine_auto_rooms(client, project: dict = None) -> dict:
    """Clear HA's auto‑stamped DEVICE area off any AV media device the installer hasn't
    placed, so it lands in Unassigned instead of appearing in a room uninvited. Idempotent
    (a device already Unassigned is skipped) and never raises. Returns {cleared:[id]}."""
    try:
        devices = client.device_registry() or []
        entities = client.entity_registry() or []
    except Exception:
        return {"ok": False, "error": "registry read failed", "cleared": []}
    av_integrations = set(discovery._KNOWN)
    project = project if project is not None else load()
    member_ents = _all_member_entities(project)
    dev_is_av = {}      # device_id -> has an AV media_player entity
    dev_pinned = {}     # device_id -> ProOS owns/placed it (leave its area alone)
    for e in entities:
        did = e.get("device_id")
        if not did:
            continue
        eid = e.get("entity_id") or ""
        if eid.startswith("media_player.") and e.get("platform") in av_integrations:
            dev_is_av[did] = True
        # Placed/owned signals: committed label, a saved‑project member, or an explicit
        # entity‑level area override (what a ProOS "Add"/assign writes — and what an
        # installer sets by hand in HA to deliberately pin an entity to a room).
        if (LABEL_AV in (e.get("labels") or [])
                or eid in member_ents
                or e.get("area_id")):
            dev_pinned[did] = True
    cleared = []
    for d in devices:
        did = d.get("id")
        if not did or not dev_is_av.get(did) or dev_pinned.get(did):
            continue
        if d.get("area_id"):
            try:
                client.set_device_area(did, None)
                cleared.append(did)
            except Exception:
                pass
...
# ── NO‑AUTO‑INTEGRATION REAPER ───────────────────────────────────────────────
# HA's discovery (zeroconf/ssdp/dhcp) AUTO‑CREATES config entries for integrations that
# self‑complete on discovery — Sonos, Google Cast, and others. The installer adds devices
# on PURPOSE, so ProOS disables any auto‑created (discovery‑sourced) entry for a ProOS‑
# managed DEVICE integration until it's explicitly added. Disabling (not deleting) keeps
# the entry present so HA won't re‑discover + recreate it in a loop; the installer enables/
# adds it when they want it. HA INFRASTRUCTURE (matter/thread/bluetooth adapter/…) is left
# alone — it's the box's own capability, verified on Dave's fresh box, not a device.
_AUTO_DISCOVERY_SOURCES = {"zeroconf", "ssdp", "dhcp", "mdns", "homekit",
                           "integration_discovery", "discovery", "usb", "bluetooth"}


def _managed_device_domains() -> set:
    """Device integrations ProOS expects the installer to add explicitly (AV + lighting +
    climate + security, incl. cast). An auto‑created entry for one of these is stopped;
    anything else (HA infra like matter/thread) keeps auto‑configuring."""
    return set(discovery.ROLE_BY_INTEGRATION) | set(discovery.CLASS_BY_INTEGRATION) | {"cast"}


def _load_reaped(path: str = REAP_PATH) -> set:
    try:
        with open(path) as f:
...
def _save_reaped(s, path: str = REAP_PATH) -> None:
    try:
...
def reap_auto_integrations(client) -> dict:
    """Disable config entries HA auto‑created via discovery for ProOS‑managed device
    integrations the installer hasn't added — ONCE per entry. Once an entry has been reaped
    it's remembered and never touched again, so if the installer later ENABLES it to use it,
    the reaper won't re‑disable it (its source stays 'zeroconf'). Never raises.
    Returns {disabled:[domain]}."""
    try:
        entries = client.config_entries() or []
    except Exception:
        return {"ok": False, "error": "config_entries read failed", "disabled": []}
    managed = _managed_device_domains()
    reaped = _load_reaped()
    disabled, changed = [], False
    for e in entries:
        eid = e.get("entry_id"); dom = e.get("domain"); src = e.get("source")
        if not eid or dom not in managed:
            continue
        if src not in _AUTO_DISCOVERY_SOURCES:   # installer‑added (source 'user') stays
            continue
        if eid in reaped:                        # already handled once — leave it (enable sticks)
            continue
        if e.get("disabled_by"):                 # already disabled elsewhere — record, don't touch
            reaped.add(eid); changed = True
            continue
        try:
            client.set_entry_disabled(eid, True)
            disabled.append(dom); reaped.add(eid); changed = True
        except Exception:
            pass
    if changed:
        _save_reaped(reaped)
...
def activities_status(client, project, area) -> dict:
    """List a committed room's activities with an EDITED flag (installer changed the HA
    script vs the generated original — content hash ≠ the stamped proos_hash). Powers the
    room's Activities list + the per-activity edit/reset popup."""
    from . import generator
    rec = (project or {}).get("areas", {}).get(area)
    ...
    out = []
    for oid, cfg in fresh.items():
        edited, exists = False, False
        try:
        ...
        out.append({"object_id": oid, "entity_id": "script." + oid,
                    "alias": cfg.get("alias") or oid,
                    "kind": (cfg.get("variables") or {}).get("proos_kind"),
                    "edited": edited, "exists": exists})
