...
# Devices ProOS has already "seen". Backs the no-auto-room quarantine: the first run
# records the current home as a baseline (nothing touched), and thereafter each device is
# processed exactly once, the first time it appears — so anything ADDED later that HA
# auto-stamped with a room gets cleared to Unassigned, without ever disturbing the
# established home or re-clearing on every scan.
SEEN_PATH = os.environ.get("PROOS_SEEN_PATH", "/data/proos_seen.json")
SCHEMA_VERSION = 1
...
def clear(path: str = DEFAULT_PATH) -> bool:
    ...
    try:
        ...
        ok = False
    # Drop the no-auto-room baseline too, so a reset re-baselines against the fresh home
    # instead of carrying stale device ids that would mis-flag devices as "already seen".
    try:
        os.remove(SEEN_PATH)
    except FileNotFoundError:
        pass
...
# ── NO‑AUTO‑ROOM QUARANTINE ──────────────────────────────────────────────────
# "Nothing gets a room until the installer places it." HA stamps a device's area at
# pairing when its name matches a room (a HomePod called 'Office' -> Office). Opt‑in
# membership stops ProOS COMMITTING that guess, but the device still LOOKS placed. This
# clears the auto‑stamped area so a newly‑added device — any type — lands in Unassigned.
# Safety: a one‑time baseline means an existing home is never mass‑wiped, and each device
# is handled exactly once (never re‑cleared after the installer places it or edits it).

def _load_seen(path: str = SEEN_PATH):
    try:
        with open(path) as f:
            d = json.load(f) or {}
        return set(d.get("known") or []), bool(d.get("baseline"))
    except Exception:
        return set(), False


def _save_seen(known, baseline: bool, path: str = SEEN_PATH):
    try:
        with open(path, "w") as f:
            json.dump({"known": sorted(known), "baseline": bool(baseline)}, f)
    except Exception:
        pass
...
def quarantine_new_devices(client, project: dict = None, seen_path: str = SEEN_PATH) -> dict:
    """Clear the auto‑stamped room off any NEWLY‑appeared, unplaced device (once).

    First run records a baseline of the current devices and clears nothing. After that,
    every device not seen before is processed a single time: if HA gave it an area and it
    isn't part of a ProOS room (no committed/in‑progress membership, no proos_av label),
    its device area is cleared so it drops into Unassigned for explicit placement.
    Returns {baseline, cleared:[device_id], known}. Never raises."""
    try:
        devices = client.device_registry() or []
    except Exception:
        return {"ok": False, "error": "device registry read failed", "cleared": []}
    ids = [d.get("id") for d in devices if d.get("id")]
    known, baseline = _load_seen(seen_path)
    if not baseline:
        # Establish the baseline: the home as it stands is left exactly as‑is.
        _save_seen(set(ids), True, seen_path)
        return {"ok": True, "baseline": True, "cleared": [], "known": len(ids)}
    ...
    member_ents = _all_member_entities(project)   # committed AND in‑progress selections
    protected = set()                             # device_ids that ProOS owns / is using
    for e in entities:
        did = e.get("device_id"); eid = e.get("entity_id")
        if not did:
            continue
        if eid in member_ents:
            protected.add(did)
    ...
    for d in devices:
        ...
        if not did or did in known:
            continue
        known.add(did)                            # processed once, regardless of outcome
        if d.get("area_id") and did not in protected:
            try:
                client.set_device_area(did, None)
                cleared.append(did)
            except Exception:
                pass
    _save_seen(known, True, seen_path)
    return {"ok": True, "baseline": False, "cleared": cleared, "known": len(known)}
