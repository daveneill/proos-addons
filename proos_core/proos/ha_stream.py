...
class HaStream(threading.Thread):
    ...
    def __init__(self, base_url: str, token: str):
        ...
        # REGISTER 447: the platform's own word that a login changed or went.
        self._user_gen = 0
        self._user_watch = False
    ...
    def user_generation(self) -> int:
        """Bumps when the platform reports a user removed or updated (a
        revoked login, a changed tier), or on a new session — register 447.
        Meaningless unless user_watched() and healthy()."""
        return self._user_gen

    def user_watched(self) -> bool:
        return bool(self._user_watch and self.healthy())
    ...
    def _run_once(self):
        ...
        try:
            ...
            # REGISTER 447: two more on the same connection — the platform's
            # own user_removed / user_updated — so a revoked login is heard,
            # not discovered fifteen minutes later.
            self._user_gen += 1
            self._user_watch = True
            for _i, _et in ((6, "user_removed"), (7, "user_updated")):
                _send_text(sock, json.dumps(
                    {"id": _i, "type": "subscribe_events", "event_type": _et}))
            ...
            while not self._stop.is_set():
                ...
                if mtype == "result":
                    ...
                    if m.get("id") in (6, 7) and not m.get("success", True):
                        self._user_watch = False
                        print("  [ha_stream] user events unavailable (%s)"
                              " — a login is re-verified on the cache clock"
                              % (m.get("error") or "refused"), flush=True)
                    if m.get("id") == 2:
                    ...
                    if _ev.get("event_type") in ("area_registry_updated",
                        ...
                        self._reg_gen += 1
                        continue
                    if _ev.get("event_type") in ("user_removed", "user_updated"):
                        self._user_gen += 1
                        continue
