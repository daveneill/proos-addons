...
def make_watch_activity(*, area: str, key: str, source_label: str,
    ...
    def validate_display(snap: Snapshot) -> Check:
        if _st(snap, display_eid) == "on":
            return Check(True, f"TV on, routed to {hdmi_code} (discrete, not readback-confirmable)")
        return Check(False, f"TV power is '{_st(snap, display_eid)}', expected on")

    targets = [
        Target("display", display_eid, f"TV on + routed to {hdmi_code}",
    ...
    # ---- summary: routing is the thing that makes the activity true. Source
    #      liveness is reported but advisory, so a flaky Apple TV state doesn't
    #      flip the room to "not achieved" when the input is correctly routed.
    def summary(snap: Snapshot) -> Check:
        disp_on = _st(snap, display_eid) == "on"
        if not disp_on:
            return Check(False, "TV is not on")
        if broadcast:
            return Check(True, f"{area} is watching TV (routed to {hdmi_code})")
    ...
    # The discrete route this activity wants on screen. The controller fires it
    # once per activation (power-independent, idempotent). Not a Target because
    # it isn't a convergence loop -- it's a deterministic action.
    act.route = {"tv_remote": tv_remote, "hdmi_code": hdmi_code}
    # The source's remote entity, woken once per activation alongside the route.
    # Fired unconditionally because the source's own 'awake' state is exactly
    # what we can't trust here (tvOS-26 phantom 'paused'); re-asserting an
    # already-awake device is harmless, like re-sending the input code.
    act.wake_remote = wake_remote if (source_eid and not broadcast) else None
    return act


def build_watch_activities(cluster) -> list[Activity]:
    """
    Generate Watch activities from a cluster.

    Requires each source to carry an `hdmi_code` and the cluster to carry a
    `tv_remote`. Discrete codes can't be auto-discovered -- they're mapped once
    per TV at commissioning (the wizard). Until then, callers hardcode them.
    """
    if cluster.display is None:
        return []
    tv_remote = getattr(cluster, "tv_remote", None)
    if not tv_remote:
        return []
    out: list[Activity] = []
    for src in cluster.sources:
        code = getattr(src, "hdmi_code", None)
        if not code:
            continue  # unmapped source: skip rather than route blind
        ...
        out.append(make_watch_activity(
            ...
            display_eid=cluster.display.entity, tv_remote=tv_remote,
            hdmi_code=code, source_eid=src.entity, competing_eids=others,
        ))
    # Broadcast TV, if the cluster declares a tuner code.
    tv_code = getattr(cluster, "tv_code", None)
    if tv_code:
        all_sources = [s.entity for s in cluster.sources]
        out.append(make_watch_activity(
            area=cluster.area, key="watch_tv", source_label="TV",
            display_eid=cluster.display.entity, tv_remote=tv_remote,
            hdmi_code=tv_code, source_eid=None, competing_eids=all_sources,
            broadcast=True,
        ))
    return out
