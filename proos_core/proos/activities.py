...
def make_watch_activity(*, area: str, key: str, source_label: str,
    ...
    def summary(snap: Snapshot) -> Check:
        # The Samsung reports phantom 'off'/'unavailable' while actually on (its
        # integration drops the connection and mis-reports power). So TV power is
        # ADVISORY here, never the gate -- the authoritative verdict comes from
        # the source witness (state + reachability), the two signals that agree.
        disp = _st(snap, display_eid)
        if broadcast:
            # No source device -- the tuner IS the source, so TV power is the
            # only signal and it flaps. Treat 'unavailable' as unknown (not off);
            # only call it not-on when it definitively reports off/standby.
            if disp in ("off", "standby"):
                return Check(False, f"{area} TV is off")
            return Check(True, f"{area} is watching TV")
        # Three signals, none allowed to veto on its own:
        #   1. source alive (playing/paused/on)
        #   2. reachable -- a router device_tracker ('home'/'not_home') or a ping
        #      ('on'/'off'). Only an EXPLICIT down reading vetoes; unknown/
        #      unavailable/missing is inconclusive and never vetoes.
        #   3. display on -- ADVISORY: can soften 'watching' to '(display
        #      unconfirmed)' but never knocks it out (absorbs Samsung phantom-off).
        src_live = source_eid is not None and _st(snap, source_eid) in ALIVE
        disp_on = disp == "on"
        if reachability_sensor:
            reach = _st(snap, reachability_sensor)
            if not src_live:
                return Check(False, f"{area}: {source_label} not playing")
            if reach in ("not_home", "off"):
                return Check(False,
                    f"{source_label} selected but not answering on the network")
            if disp_on:
                return Check(True, f"{area} is watching {source_label}")
            return Check(True,
                f"{area} is watching {source_label} (display unconfirmed)")
        # No reachability witness: source + display only (still no TV-power veto).
        if not src_live:
            return Check(False, f"{area}: {source_label} not playing")
        if disp_on:
            return Check(True, f"{area} is watching {source_label}")
