...
def make_watch_activity(*, area: str, key: str, source_label: str,
    ...
    def summary(snap: Snapshot) -> Check:
        ...
        # REGISTER 446: no reachability witness commissioned used to be read as
        # "home" — a single-signal guess producing the IDENTICAL verdict as a
        # two-signal confirmation. Now: with no witness the verdict is reached
        # on the source's own word and SAYS so (unverified), unless an audio
        # witness independently corroborates it.
        reach = _st(snap, reachability_sensor) if reachability_sensor else None
        if src_live and reach not in ("not_home", "off") and _audio_confirmed(snap, audio_witness):
            second = bool(reachability_sensor) or bool(audio_witness and audio_witness.get("entity"))
            return Check(True, f"{area} is watching {source_label}"
                         + ("" if second else " (on the source's own word)"),
                         unverified=not second)
        return Check(False, f"{area} is off")
...
# STAGE 5 BUILD 1 (16 Aug 2026): the hardcoded Family Room activity set
# that stood here — the author's own panel, Apple TV, Shield and their
# HDMI codes, shipped as module-level activities in every copy of the
# product — is DELETED. Nothing consumed it (build_watch_activities builds
# every room from its discovered cluster + committed record); it was the
# author's house riding along in the box. The shipped product contains no
# home (no_shipped_home_bench.py).
