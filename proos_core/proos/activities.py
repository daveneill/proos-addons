...
def _audio_confirmed(snap: Snapshot, witness: dict | None) -> bool:
    ...
    if rec is None:
        # The plan names a device this house does not have (stale id, typo, a
        # renamed entity). That is a BROKEN PLAN, not evidence that audio is
        # unrouted -- and it must never be readable as 'off', or one bad id
        # pins a lit room dark forever with nothing on screen to explain it.
        # Live house, 30 Jul 2026: this cost the Living Room every Live TV
        # verdict it should have had, for as long as the plan existed.
        return True
...
def make_watch_activity(*, area: str, key: str, source_label: str,
    ...
    def summary(snap: Snapshot) -> Check:
        ...
        if broadcast:
            ...
            # Audio is ADVISORY here and deliberately not a gate. For a source
            # activity the audio witness disambiguates WHICH source is on air;
            # for broadcast there is no source device, so the panel's own power
            # and tuner input are the direct evidence of what is on screen.
            # Gating on audio meant a receiver left off -- or merely named
            # wrongly in the plan -- could deny a plainly-lit room its verdict.
