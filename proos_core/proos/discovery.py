...
def discover_av(client, area: str) -> AVCluster:
    ...
    for r in rows:
        ...
        # endpoint, not a video source. Detect by TYPE, not name: the apple_tv integration
        # gives a HomePod device_class "speaker", while a real Apple TV is device_class "tv"
        # or None. Keying off the name (which HA derives from the room, e.g. office_2_office_2)
        # would miss it. This is the auto-role that saves the installer a manual fix.
        if role == "source" and dev.integration == "apple_tv" and dev.device_class == "speaker":
            role = "audio"
