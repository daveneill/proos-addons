...
UniFi today, TP-Link Omada or others later. This module is the CONTRACT:
  ...
  * The witness map (source entity -> rate sensors + threshold) is
    ...
    THAT FILE IS THE ONLY SOURCE OF A BINDING (register 453, 14 Sep 2026).
    The add-on option `traffic_witnesses` that once seeded it is gone: a
    factory reset wipes Core's store but cannot reach the add-on's options,
    so the option put ghost bindings back on every boot — naming sensors of
    a provider the restored box no longer had — and the clean-up path could
    never win, because its tombstone went into the file the reset had just
    wiped. One fact, one place.
  * THE RUNG IS OPTIONAL. With no certified provider on the platform there
    is no witness layer to be broken: `certified_present` says so from the
    platform's own config entries, and the integrity card stays shut.
...
# ---------------------------------------------------------------------------
# Witness store (installer-committed, survives restarts, the only source)
# ---------------------------------------------------------------------------
def load_witnesses() -> dict:
    """The installer-committed witness map, and nothing else (register 453)."""
    merged = {}
    try:
        with _LOCK:
...
def save_witness(source: str, sensors: list | None, min_rate: float | None) -> dict:
    ...
    with _LOCK:
        ...
        if sensors:
            ...
            data[source] = None                      # tombstone: this source has no witness
        tmp = _DATA + ".tmp"
...
def certified_present(client):
    """Does this home have a certified provider of traffic witnesses?

    Register 453 (Dave, 14 Sep 2026): "it's an option that is supposed to be
    auto linked upon a network controller like UniFi or later TP-Link Omada
    as an option not a requirement." A home with no network controller has no
    witness layer — there is nothing to link and nothing to be broken.

    The platform's own config entries answer, not a guess from entity names:
    True  — a certified provider of traffic witnesses is configured
    False — none is (stated as a fact, not as a failure)
    None  — the platform could not be asked; ProOS accuses nothing either way
    """
    if client is None:
        return None
    seen = False
    for dom, facts in PROVIDERS.items():
        if facts.get("planned") or not facts.get("traffic_sensor_pattern"):
            continue                     # not certified for witnesses (yet)
        try:
            entries = client.config_entries(dom) or []
        except Exception:                                        # noqa: BLE001
            return None                  # unreadable: not an answer
        seen = True
        if entries:
            return True
...
def autobind(client, project_mod) -> dict:
    ...
    cl = classify(load_witnesses(), ids, sources)
    real = cl["real"]
...
# ---------------------------------------------------------------------------
def _committed_sources(project_mod, area_names=None) -> list:
    """Every watch-source entity in committed rooms, with its area — named
    LIVE-first (STAGE 2a, register 256): the record's stored name speaks
    only when no live reading is in hand."""
    out = []
    try:
        ...
        for key, rec in (proj.get("areas") or {}).items():
            if not (rec and rec.get("committed")):
                continue
            nm = ((area_names or {}).get(rec.get("area_id") or key)
                  or rec.get("name") or key)
            for e in (rec.get("sources") or []):
                eid = e.get("entity") if isinstance(e, dict) else e
                if isinstance(eid, str) and eid:
                    out.append({"area": nm, "entity": eid})
    except Exception:
        pass
    return out
...
def inspect(client, project_mod) -> dict:
    ...
    # STAGE 2a: the witness rows speak the home's live names.
    try:
        from . import netmap as _nm2
        _anames = _nm2.load_areas(client=client) or {}
    except Exception:                                            # noqa: BLE001
        _anames = {}
    sources = _committed_sources(project_mod, area_names=_anames)
