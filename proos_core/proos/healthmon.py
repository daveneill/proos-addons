...
def _gear_mac(snapall, gear_eid):
    return str(((snapall.get(gear_eid) or {}).get("attributes") or {})
               .get("mac") or "").lower()


def _downstream_of(snapall, gear_eid):
    """(names, confirmed). Clients the CONTROLLER placed behind this gear that
    are now off the network.
    ...
    _learn_topology()
    gmac = _gear_mac(snapall, gear_eid)
...
def _infra_check(now, snapall, seen):
    ...
    for eid, name in (_infra_gear(snapall, now) or {}).items():
        ...
        took, confirmed = _downstream_of(snapall, eid)
