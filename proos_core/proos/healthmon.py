...
UNAVAIL_SECS = 3 * 60
FROZEN_SECS = 10 * 60
INFRA_SECS = 2 * 60      # sustain for the WITNESS-BLACKOUT card only (an
                         # inferred claim with no controller confirmation).
                         # The gear card itself no longer waits — Stage 7
                         # build 1, Dave's ruling: the controller flagging
                         # its own switch IS the confirmation.
...
_PREP_TTL = 30 * 60
_PREP_STALE_MAX = 2 * _PREP_TTL   # how long a last-known answer may ride
                                  # when the audit itself cannot run (build 2)
...
def _prep_posture(slug, rec, snapall):
    ...
    if c and now - c["ts"] < _PREP_TTL:
        return c["fails"]
    # STAGE 6 BUILD 2 (16 Aug 2026): EVIDENCE HAS A LIFETIME. The old
    # fail-open served the last-known fails on an audit error — and
    # RE-STAMPED them as fresh, so a permanently-broken audit asserted its
    # last answer forever, and Assist (which relays Health verbatim, as
    # its doctrine orders) relayed a claim whose evidence could be hours
    # dead. Now: a failed refresh keeps the OLD stamp; the last-known
    # answer rides at most _PREP_STALE_MAX; past that the claim is
    # WITHDRAWN — no reading, no claim. A stale assertion is not
    # fail-open, it is a guess (prep_evidence_lifetime_bench.py).
    try:
        ...
        if c and now - c["ts"] < _PREP_STALE_MAX:
            return c["fails"]            # last-known, original stamp kept
        _prep_cache.pop(slug, None)      # the evidence is dead: withdraw
        return []
...
# THE CARD A PERSON IS LOOKING AT MAY ALREADY BE OVER (Dave, 6 Sep 2026,
# register 347: "music stopped and I get this but button does not work" —
# auto-heal had restarted Music and the sweep had cleared the card 20 s
# before his tap; the repair answered "unknown incident" and the button said
# "Try again"). A cleared incident is remembered here for a while, so a tap
# on a card that has since cleared is answered with the truth — it is over.
_recent_cleared = {}   # incident-id -> (cleared ts, title)
RECENT_CLEARED_S = 10 * 60
# And a repair ProOS is ALREADY doing is not asked for twice: the auto-heal
# stamps the incident, the card says so, the button waits.
HEALING_GRACE_S = 120
_loaded = False
...
def provisional_reasons(rec, acts, snapall):
    """WHY each provisional activity is provisional (5 Sep 2026, register
    320). Pure. Returns (no_input, unexplained):
      no_input    — [(source_eid, source name)] : the source is SAVED in the
                    room but has NO input — neither a display input
                    (rec['inputs']) nor an AV-switch input
                    (rec['avswitch']['inputs']) — so no activity can be
                    built for it. That is a fact about the saved room; no
                    recommit can change it. The fix lives in Pro.
      unexplained — [activity keys] still provisional for some other reason.
    Names come from the snapshot's friendly_name, else the record's meta
    name, else the entity id's last part in words — never a raw id."""
    rec = rec or {}
    ...
    no_input, unexplained, seen = [], [], set()
    for key, a in (acts or {}).items():
        if not getattr(a, "provisional", False):
            continue
        se = getattr(a, "source_eid", None)
        if se and se != display and not inputs.get(se) and not sw_in.get(se):
            if se in seen:
                continue
            seen.add(se)
            attrs = ((snapall or {}).get(se) or {}).get("attributes") or {}
            name = (attrs.get("friendly_name")
                    or ((rec.get("meta") or {}).get(se) or {}).get("name")
                    or se)     # the id itself, never a name invented from it (reg 440)
            no_input.append((se, str(name)))
        else:
            unexplained.append(key)
    return no_input, unexplained
...
def _infra_check(now, snapall, seen):
    ...
    for mac, g in sorted(gear.items()):
        ...
        # STAGE 7 BUILD 1 (16 Aug 2026): THE SUSTAIN WINDOW IS DELETED —
        # Dave's ruling after his stopwatch switch test ("Immediately"). The
        # controller flagging its own switch IS the confirmation; native HA
        # speaks the moment the controller does, and the 2-minute wait that
        # stood here was ProOS sitting on a confirmed reading. The honesty
        # moved into the WORDING instead: a fresh card says the switch just
        # went offline and that a deliberate reboot clears itself — the
        # reading was never wrong, only late (switch_speed_bench.py; the
        # old sustain pins in the infra benches rewritten with this reason).
        ...
        # Fresh vs aged wording: the same reading, said at the age it has.
        _age_txt = (("just went offline to the network controller"
                     if mins < 1 else
                     "has been offline to the network controller for "
                     "%d minute%s" % (mins, "" if mins == 1 else "s")))
        _ensure(cid, {
            ...
            "cause": ("%s — the network switch/access point itself, not a "
                      "device on it — %s.%s Everything wired through "
                      "it has no network path. Check its power, PoE port or "
                      "uplink cable. A deliberate reboot or firmware update "
                      "clears itself in a few minutes."
                      % (name, _age_txt, took_txt)),
            "subject": "gear_" + mac,
            "actions": []})
        # The card's words follow its age: refresh the cause on later passes
        # (an _ensure alone would freeze the first sentence forever).
        try:
            if cid in _open:
                _open[cid]["cause"] = ("%s — the network switch/access point "
                                       "itself, not a device on it — %s.%s "
                                       "Everything wired through it has no "
                                       "network path. Check its power, PoE "
                                       "port or uplink cable. A deliberate "
                                       "reboot or firmware update clears "
...
def membership_drift_check(now, seen, project_mod):
    ...
    for key, rec in sorted(committed.items()):
        ...
        # STAGE 2a (register 256): the LIVE name first.
        room_name = area_names.get(room_area) or rec.get("name") or room_area
        # STAGE 1 (23 Aug 2026, register 254): the RECORD's side can be the
        # stale one. When the stored identity no longer exists in the home,
        # the device did not move — the room's identity did — and the card
        # must say that, not tell the installer to re-assign a device that
        # is exactly where it belongs. And when two rooms wear ONE display
        # name, the old sentence read "committed to Bec's Office but filed
        # in Bec's Office" (Dave's screenshot) — the twin is named as a
        # twin now. `dead` is only claimed off a real area reading: an
        # empty area_names says nothing (blind is not broken).
        dead = bool(area_names) and room_area not in area_names
        for eid in project_mod._area_members(rec):
            ...
            cur_name = area_names.get(cur, cur) if cur else None
            twin = bool(cur_name) and cur_name == room_name
            if dead:
                _ensure(cid, {
                    "kind": "membership_drift", "room": room_name,
                    "slug": room_area, "severity": "warning",
                    "title": ("%s's room was renamed or rebuilt — "
                              "re-commit %s" % (name, room_name)),
                    "cause": ("%s is committed to '%s', but that room's "
                              "stored identity no longer exists in the "
                              "home%s. The device did not move; the room's "
                              "identity did. Tap Adopt Its New Home below — "
                              "or re-commit the room in Pro (Rooms); both "
                              "run the same repair."
                              % (name, room_name,
                                 (" — the room it sits in now wears the "
                                  "same name") if twin else "")),
                    "subject": eid,
                    # 2c (23 Aug 2026, register 260): the card carries its
                    # own repair — the SAME proven reconciler a commit runs.
                    # The tap is the consent; ProOS never runs it alone.
                    "actions": [{"kind": "adopt_home", "room": room_area,
                                 "label": "Adopt Its New Home"}]})
                continue
            where = (("in %s" % (cur_name + (" (a different room wearing "
                                             "the same name)"
                                             if twin else "")))
                     if cur else "in no room")
            _ensure(cid, {
                "kind": "membership_drift", "room": room_name,
                "slug": room_area, "severity": "warning",
                "title": "%s has drifted out of %s" % (name, room_name),
                "cause": ("%s is part of %s's committed setup, but the "
                          "home's registry now files it %s. The room's "
                          "activities still run exactly as committed; room "
                          "views and monitoring group by the registry's "
                          "area, so this device "
...
# REFUSAL LANGUAGE ONLY (Stage 3 remainder build 1, 16 Aug 2026): the old
# bare `credential` alternation matched HEALTHY lines — "Saved credential
# to store" opened the critical card. A mark must be a refusal, not a word
# that merely mentions the subject.
_AUTH_MARK_RE = re.compile(
    r"invalid[_ ]credentials|unauthori[sz]ed|not authori[sz]ed|"
    r"token expired|login failed|credentials? (?:rejected|revoked|expired)",
    re.I)
...
def provider_stream_check(now, seen, text):
    ...
    for prov, f in faults.items():
        # A STANDING card rests on CURRENT, DATEABLE evidence (Stage 3
        # remainder build 1): the detector computes how far the refusal sits
        # behind the log's newest line, and this check used to ignore it —
        # a refusal from days ago held a critical card open forever (the
        # audit's "immortal card"). Same window as the command-time gate;
        # an unstamped refusal cannot prove it is current, so it cannot
        # hold a standing card (the command-time classifier keeps its own
        # rules — a live failure is its own freshness).
        behind = f.get("behind")
        if behind is None or behind >= PROVAUTH_WINDOW:
            continue
...
def _scan(snapall, project_mod, get_controller, witnesses):
    ...
    try:
        from . import netmap as _nm3
        _anames3 = _nm3.load_areas(client=CLIENT) or {}
    except Exception:                                            # noqa: BLE001
        _anames3 = {}
    for key, rec in (proj.get("areas") or {}).items():
        if not (rec and rec.get("committed")):
            continue
        # STAGE 2a (register 256): cards speak the home's LIVE names; the
        # stored copy only when the registry cannot be read.
        slug = rec.get("area_id") or key
        room = _anames3.get(slug) or rec.get("name") or key
        ...
        if _rf:
            ...
            _ensure(cid, {
                ...
                "cause": "%s is %s. Its devices and roles are still in the "
                         "home's registry, so ProOS can rebuild the record "
                         "from them — or open the room and commit it again."
                         % (room, _rf),
                "subject": "rec",
        ...
        for eid in committed_eids:
            row = snapall.get(eid) or {}
            st = row.get("state")
            # REGISTER 447: MISSING IS NOT DEAD. An entity id the platform
            # does not have (renamed, removed, mistyped in the record) is a
            # fault in the ROOM'S PLAN, not a device that stopped answering
            # — one word did two jobs and the professional was sent to check
            # power and network on a device that was fine under a new name.
            if row.get("missing") or (not row and eid not in snapall):
                mid = _iid("committed_missing", slug, eid)
                seen.add(mid)
                _ensure(mid, {
                    "kind": "committed_missing", "room": room, "slug": slug,
                    "severity": "critical",
                    "title": "%s — a committed device is not on the platform" % room,
                    "cause": "%s is in this room's record but the platform has "
                             "no entity by that id. It was renamed, removed, or "
                             "re-added under a new id — the device itself may be "
                             "fine." % eid,
                    "subject": eid,
                    "actions": [
                        {"kind": "recommit", "room": slug,
                ...
                if now - first >= UNAVAIL_SECS:
                    _ensure(cid, {
                        ...
                        "cause": "%s has been %s for %s, confirmed over a "
                                 "%d-second window. If it was re-added "
        ...
        # 3 · provisional flags on a committed room — SAY WHY (5 Sep 2026,
        #     register 320). "Commit didn't fully apply" + Recommit was a
        #     misdiagnosis for the case that actually happens: a source
        #     SAVED without an input (the Shield's Chromecast half, 315).
        #     The commit applied exactly; the saved room is what is wrong,
        #     and no recommit can change it. So: one card per input-less
        #     source, named, with the fix where it lives (Pro), and never a
        #     Recommit button for it. Anything provisional for a reason
        #     this cannot name keeps the old card.
        _no_input, _unexplained = provisional_reasons(rec, acts, snapall)
        for _se, _sname in _no_input:
            cid = _iid("source_no_input", slug, _se)
            seen.add(cid)
            _ensure(cid, {
                "kind": "source_no_input", "room": room, "slug": slug,
                "severity": "warning",
                "title": "%s — %s has no input assigned" % (room, _sname),
                "cause": "%s was saved to %s without an input, so no "
                         "activity can be built for it. Open the room in "
                         "Pro and give it an input (Devices & AV), or "
                         "remove it from the room if it is not a source. "
                         "Saving again without doing that changes nothing."
                         % (_sname, room),
                "subject": _se,
                "actions": [{"kind": "room", "room": slug,
                             "label": "Open %s" % room}]})
...
def _witness_integrity(seen, snapall, witnesses, src_eids):
    ...
    # THE RUNG IS OPTIONAL (register 453, 14 Sep 2026). Dave, on a box with
    # nothing in it yet: "it's an option that is supposed to be auto linked
    # upon a network controller … not a requirement." With no certified
    # provider on the platform there is no witness layer to be broken and
    # nothing for anyone to link — so there is no card. Only a stated
    # absence closes this; unknown (None) behaves exactly as it always did.
    if _netev.certified_present(CLIENT) is False:
        return
    try:
...
def _mark_healing(cid, now):
    """ProOS is repairing this one itself: stamp it so the card says so and a
    tap does not ask for the same repair twice (register 347)."""
    with _lock:
        _load()
        if cid in _open:
            _open[cid]["healing_at"] = now
            _save()


def _remember_cleared(inc, now=None):
    now = time.time() if now is None else now
    for k, (ts, _t) in list(_recent_cleared.items()):
        if now - ts > RECENT_CLEARED_S:
            _recent_cleared.pop(k, None)
    _recent_cleared[inc["id"]] = (now, inc.get("title") or "")


def recently_cleared(iid):
    """(when, title) if this incident cleared within RECENT_CLEARED_S, else None."""
    got = _recent_cleared.get(iid)
    if not got or time.time() - got[0] > RECENT_CLEARED_S:
        return None
    return got
...
def resolve_action(client, iid, action_kind=None):
    ...
    inc = get(iid)
    if not inc:
        # The card they tapped has since cleared — the condition is OVER
        # (register 347). That is the truth, not an error; the page takes
        # the card down.
        gone = recently_cleared(iid)
        if gone:
            return {"ok": True, "cleared": True,
                    "note": "Already fixed — %s is over." % (gone[1] or "that")}
    ...
    if act["kind"] == "restart_music":
        ...
        _h = inc.get("healing_at")
        if _h and time.time() - _h < HEALING_GRACE_S:
            # ProOS is already restarting it (auto-heal): a second restart
            # would only stretch the outage. Say so; the card waits.
            return {"ok": True, "did": "restart_music", "cleared": False,
                    "already": True,
                    "note": "ProOS Music is already restarting — it takes "
                            "about a minute to come back."}
        _mark_healing(iid, time.time())
        try:
            MUSIC_RESTART()
    ...
    if act["kind"] == "apply_settings":
        ...
        try:
            ...
            # Build 2: the tap CHANGED the evidence, so the cached audit is
            # dead — drop it, or the next sweep re-raises the card from the
            # stale answer for up to 30 minutes (the bounce-back).
            if any(_applied.values()):
                _prep_cache.pop(inc.get("slug"), None)
            journal.emit(inc.get("slug", "site"), "repair",
