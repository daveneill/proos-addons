...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        verify = []
        # Source wake. Apple TV / Android TV DROP their integration connection when
        # asleep on tvOS 26 (media_player 'off' + connectivity 'unavailable'); once
        # disconnected NO command reaches them -- the native remote only works because
        # it's Bluetooth-direct. PROVEN FIX (tested live): reload the integration to
        # reconnect, then media_player.turn_on wakes it. So: fire the wake; if it didn't
        # take, RECONNECT (reload) and wake again; bounded; surface an incident if it
        # never wakes. 'idle' counts as awake (Apple TV woken to home screen).
