...
def _wake_display_steps(client, disp, timeout=15):
    ...
    return [
        # continue_on_error: a display that has lost its control authorization (e.g. a
        # Samsung throwing ms.channel.unauthorized) errors here -- we must NOT let that abort
        # the whole activity, or nothing downstream runs and, crucially, the outcome advisory
        # at the end never fires. So we swallow the command error and let the final check
        # decide (did the panel actually reach a live input?) and advise if not.
...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        verify.append(_outcome_advisory(disp, cluster.area, area_slug,
            "is_state('%s','on')" % disp, _WATCH_ADVISORY_MSG))
        seq = burst + verify
        scripts[oid] = {
