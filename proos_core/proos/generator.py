...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        # Apple TV: media_player.turn_off doesn't reliably sleep tvOS; its paired
        # remote.<object_id> does (pyatv exposes both under the same object_id).
        # Derive the remote from the media_player. Every other source sleeps via
        # media_player.turn_off. A missing remote entity simply no-ops.
        if src.integration == "apple_tv":
            seq.append({
