...
def _wake_display_steps(client, disp, timeout=15):
    ...
    steps = [{
        # Idempotent: turning on an already-on TV is a no-op; makes AVR rooms robust
        # where CEC one-touch-play from the source may not chain-power the screen.
    ...
    if art:
        steps.append({
            "alias": "Exit Art Mode (wake to live view)",
            "action": "switch.turn_off",
            "target": {"entity_id": art},
        })
        steps.append({
            "alias": "Wait for the display to be on a live input",
            "wait_template": "{{ is_state('%s','on') and is_state('%s','off') }}" % (disp, art),
            "timeout": {"seconds": timeout},
            "continue_on_timeout": True,
        })
    else:
