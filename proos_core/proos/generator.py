...
def _wake_display_steps(client, disp, timeout=15):
    ...
    if art:
        # A Frame TV can power on straight INTO Art Mode -- and crucially it often enters
        # art a moment AFTER turn_on, so a single immediate "if art is on" check misses it
        # (it isn't in art YET), the input then fires onto the artwork, and the activity
        # looks like it did nothing. This is the "works from Art Mode, not from full-off"
        # bug: from art the panel is already in art so the one-shot check catches it; from
        # full-off the TV slips into art too late.
        #
        # So: wait for the panel to actually power up, THEN drive Art Mode off and CONFIRM
        # we've left it, re-firing if the TV slips back into art during boot. Bounded, and
        # short-circuits instantly when already on a live input (no start-up delay).
        steps.append({
            "alias": "Wait for the panel to power on",
            "wait_template": "{{ not is_state('%s','off') and not is_state('%s','unavailable') }}" % (disp, disp),
            "timeout": {"seconds": timeout},
            "continue_on_timeout": True,
        })
        steps.append({
            "alias": "Leave Art Mode (onto a live input)",
            "repeat": {
                "sequence": [
                    {"if": [{"condition": "state", "entity_id": art, "state": "on"}],
                     "then": [{"action": "switch.turn_off", "target": {"entity_id": art}}]},
                    {"wait_template": "{{ not is_state('%s','on') }}" % art,
                     "timeout": {"seconds": 3}, "continue_on_timeout": True},
                ],
                "until": [{"condition": "template",
                           "value_template": "{{ not is_state('%s','on') or repeat.index >= 4 }}" % art}],
            },
        })
    else:
        # Non-Frame display: the one necessary gate (can't switch input on an off screen).
        # Short-circuits instantly when already on.
        steps.append({
            "alias": "Wait until on",
            "wait_template": "{{ is_state('%s','on') }}" % disp,
            "timeout": {"seconds": timeout},
...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        if input_name and input_name in disc:
            ...
            if disp_verifies:
                route_confirm = conf           # confirm + incident only where readback exists
