...
def _wake_display_steps(client, disp, timeout=15):
    ...
    if art:
        ...
        # Proceed the instant the TV is on AND not in Art Mode. When it's already on a
        # live input this passes immediately (no start-up delay -- speed). If Art-Mode
        # state can't be read it still proceeds (not 'on'), and the verify+retry route
        # settles the input. Brand-agnostic: a non-Frame display uses the plain wait.
        steps.append({
            "alias": "Wait until on a live input",
            "wait_template": "{{ is_state('%s','on') and not is_state('%s','on') }}" % (disp, art),
            "timeout": {"seconds": timeout},
            "continue_on_timeout": True,
        })
    else:
        steps.append(_wait_display_on(disp, timeout))
    return steps
...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        seq = _wake_display_steps(client, disp) + [{
