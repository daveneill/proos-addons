...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        verify = []
        # Source wake. VERIFIED LIVE on this Apple TV: a plain media_player.turn_on does
        # NOTHING when it's asleep (state stays 'off') -- because on tvOS the integration's
        # connection goes stale in standby, so the command never reaches the device. What
        # DOES wake it: reload_config_entry (reconnect) THEN media_player.turn_on -> the
        # media_player goes off -> idle. (remote.turn_on only flips the remote ENTITY to
        # 'on'; it does not wake the screen -- so it must NOT be used as the wake, nor as
        # the awake witness, or the confirm passes on a still-asleep device.)
        #
        # So for a remote-sleep source: if it's asleep, reconnect + settle FIRST, then
        # media_player.turn_on -- on the first pass, gated so an already-awake device is
        # never disrupted or slowed. The awake witness is the media_player leaving 'off'/
        # 'unavailable' (idle/on/playing). If it still didn't take, reconnect + wake again.
        if src.integration in _REMOTE_SLEEP:
            asleep = "is_state('%s','off') or is_state('%s','unavailable')" % (src.entity, src.entity)
            burst.append({
                "alias": "Reconnect %s if asleep (stale tvOS connection won't wake otherwise)" % label,
                "if": [{"condition": "template", "value_template": "{{ %s }}" % asleep}],
                "then": [
                    {"action": "homeassistant.reload_config_entry", "target": {"entity_id": src.entity}},
                    {"wait_template": "{{ not is_state('%s','unavailable') }}" % src.entity,
                     "timeout": {"seconds": 12}, "continue_on_timeout": True},
                    {"delay": {"seconds": 2}},   # let pyatv finish connecting before turn_on
                ],
            })
