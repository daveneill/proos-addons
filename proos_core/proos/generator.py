...
def _wake_display_steps(client, disp, timeout=15):
    """Power the display on AND get it onto a live input, ready to route.

    A Frame TV that rests in Art Mode reports its media_player as 'on' while it's
    only showing artwork -- so a plain "wait for on" passes instantly and the input
    route then fires onto the art screen and is ignored (the activity looks like it
    did nothing). If the display has an Art Mode switch we therefore also turn Art
    Mode OFF and wait until it is genuinely on a live input before continuing. A
    normal (non-Frame) display just powers on and waits for 'on' as before."""
    art = _art_mode_switch(client, disp)
    steps = [{
        # Idempotent: turning on an already-on TV is a no-op; wakes it from a real
        # power-OFF (e.g. the client turned it fully off, not to Art Mode). Makes AVR
        # rooms robust where CEC one-touch-play may not chain-power the screen.
        "alias": "Turn on display",
        "action": "media_player.turn_on",
        "target": {"entity_id": disp},
    }]
    if art:
        # Exit Art Mode ONLY if it is actually resting on artwork -- so a TV the client
        # left fully OFF just wakes normally (no stray art toggle), while a TV resting
        # in Art Mode is pulled to a live input. Handles both scenarios regardless of
        # what the room's off-state is configured to.
        steps.append({
            "alias": "Exit Art Mode if resting on artwork",
            "if": [{"condition": "state", "entity_id": art, "state": "on"}],
            "then": [{"action": "switch.turn_off", "target": {"entity_id": art}}],
        })
    # Confirm the display is on a LIVE input; re-power ONLY if it didn't come on (the
    # control fire above already commanded power -- this is the state confirm on top).
    # Short-circuits instantly when already live (speed); still proceeds if Art-Mode
    # state is unreadable. Brand-agnostic (non-Frame uses just is_state on).
    live = ("is_state('%s','on') and not is_state('%s','on')" % (disp, art)) if art \
        else ("is_state('%s','on')" % disp)
    steps.append({
        # The ONE necessary gate (can't switch input on an off screen). Short-circuits
        # instantly when already live -> no start-up delay. Everything else fires right
        # after and is verified in the backing pass.
        "alias": "Wait until on a live input",
        "wait_template": "{{ %s }}" % live,
        "timeout": {"seconds": timeout},
        "continue_on_timeout": True,
    })
    return steps
...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        verify = []
        # Source wake. On tvOS 26 / recent Android TV, media_player.turn_on is a SILENT
        # NO-OP (verified live: the media_player state stays 'off'); the device wakes only
        # via its paired remote.turn_on (verified live: the remote goes 'on', the
        # integration reconnects, the source list repopulates). And when the device sleeps
        # the integration DROPS its connection (entity 'unavailable'), after which no
        # command lands until the config entry is reloaded to reconnect. So for
        # remote-sleep sources: (1) wake via remote.turn_on -- NOT media_player.turn_on;
        # (2) the awake WITNESS is the remote's own 'on' state, because the media_player
        # can sit at 'off' on the home screen while the device is genuinely awake (verified
        # live) -- so it is not reliable here; (3) if it doesn't take, RECONNECT (reload
        # config entry) then remote.turn_on again; bounded, with an incident if it never
        # wakes. Non-remote sources keep media_player.turn_on + media_player witness.
        wake_action, wake_entity = _source_wake(src)
        burst.append({"alias": "Wake %s" % label, "action": wake_action, "target": {"entity_id": wake_entity}})
        if src.integration in _REMOTE_SLEEP:
            remote_eid = wake_entity  # remote.<oid>
            awake = ("is_state('%s','on') or (not is_state('%s','off') and not is_state('%s','unavailable'))"
                     % (remote_eid, src.entity, src.entity))
            refire = [
                {"action": "homeassistant.reload_config_entry", "target": {"entity_id": src.entity}},
                {"wait_template": "{{ not is_state('%s','unavailable') }}" % remote_eid,
                 "timeout": {"seconds": 8}, "continue_on_timeout": True},
                {"action": "remote.turn_on", "target": {"entity_id": remote_eid}},
            ]
            wake_note = "reconnect + re-wake if not"
        else:
        ...
        # Input routing. The wake above (CEC one-touch-play) requests the display follow;
        # on a readback display we also drive the exact input over IP control.
        ...
        if input_name and input_name in disc:
            ...
            burst.append(fire)                 # control fire in the burst
            if disp_verifies:
                verify += conf                 # confirm + incident only where readback exists
