...
def _wake_display_steps(client, disp, timeout=15):
    """Fire the display ON. CONTROL ONLY -- no blocking state wait, so the picture comes up
    as fast as the panel physically can (Control4 behaviour).

    CONFIRMED BY THE INSTALLER for this Samsung Frame:
      * art switch OFF -> turns the TV ON, onto a live input   <-- the wake
      * art switch ON  -> puts the TV into Art Mode            <-- the rest (tv_off)
    We deliberately do NOT wait for the art switch to read back before the input is sent:
    verified live, a select_source issued while still on artwork IS accepted and the panel
    lands on that input as it wakes. Waiting on the readback only added dead time. The
    live-input confirmation happens once in the verify pass (and feeds the advisory).
    media_player is never used as the live signal -- it reports 'on' while showing art."""
    art = _art_mode_switch(client, disp)
    if art:
        return [{"alias": "Turn the TV on (Art Mode off = TV on)", "action": "switch.turn_off",
...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        if input_name and (not disc or input_name in disc):
            ...
            # Fire the input ONCE. A second (blind re-assert) select_source made this Samsung
            # surface its source/menu overlay a second time on every activity -- one command,
            # trusted, is the Control4 behaviour.
            route_confirm = []
        ...
        burst += af
        verify += ac
        verify.insert(0, {"alias": "Confirm the panel reached a live input",
            "wait_template": "{{ %s }}" % _live_expr(client, disp),
    ...
    if _display_has_tv_source(client, disp):
        ...
        # Fire the tuner input ONCE (no blind re-assert -- it double-popped the source menu).
