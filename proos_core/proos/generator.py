...
def _wake_display_steps(client, disp, timeout=15):
    ...
    # Capture the START state (was the panel FULLY off?) before we touch anything, so the
    # verify pass can re-assert the input ONLY on the cold-wake path -- from full-off the
    # burst's input select is fired at a panel that cannot accept it yet.
    cap = {"variables": {"proos_cold_wake": "{{ is_state('%s','off') }}" % disp}}
    if art:
        return [cap,
...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        if input_name and (not disc or input_name in disc):
            ...
            # start-state captured in the wake burst, so it never fires (and never double-pops)
            # in the common already-on / Art-Mode path.
    ...
    if _display_has_tv_source(client, disp):
        ...
        # lands on the tuner without double-popping the source menu on the common path.
        verify += [_cold_wake_input_reassert(disp, "TV")]
