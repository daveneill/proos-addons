...
def room_slug(cluster) -> str:
    """THE one derivation of a room's script-id slug (Stage 5 build 2,
    16 Aug 2026). The IMMUTABLE HA area_id when the cluster carries one;
    the name slug only for pre-migration rooms that never got an id.

    This function exists because there were TWO derivations: generation
    keyed on area_id while the dashboard's fire path (controller.
    _script_entity_for / fire_plan) slugged the room NAME — so renaming a
    room made the dashboard fire scripts that do not exist, silently.
    Every consumer calls THIS and nothing else
    (one_script_id_bench.py pins the inline copies out of existence)."""
    # REGISTER 440: the id, or NOTHING. There is no slug-of-the-name
    # fallback any more — a script keyed by a name's slug is exactly the
    # thing a rename orphans. A cluster with no area id generates nothing.
    return getattr(cluster, "area_id", "") or ""
...
def _paired_remote(client, mp_entity: str) -> str:
    ...
    sleep bug). REGISTER 447: there is no fallback to that guess any more — a device the
    platform shows no remote for gets "" and the step that needed one is skipped, said so."""
...
def _marker(kind, audio, area=None, area_id=None):
    ...
    # REGISTER 440: the room is stamped by its platform AREA ID. The name is
    # not written into the script — a rename would leave a stale copy in
    # every script of the room. (Older scripts carry proos_area, a name;
    # Pro's orphan sweep reads the id first and the name only for those.)
    if area_id:
        m["proos_area_id"] = area_id
    elif area:
        m["proos_area"] = area
    return m
...
_WATCH_ADVISORY_MSG = ("The TV did not turn on or switch input for this activity. It may "
    "have lost its connection to ProOS (a Samsung can drop its authorization). On "
    "the TV, accept the 'Allow this device' prompt (Settings > General > External Device "
    "Manager > Device Connection Manager), or re-pair the TV in ProOS, then try the "
    "activity again.")
...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    area_slug = room_slug(cluster)   # THE one derivation (build 2)
    if not area_slug:
        # register 440: no platform area id, no scripts — never a name slug
        print("  [generator] %s: no area id from the platform — nothing generated"
              % (getattr(cluster, "area", "") or "?"), flush=True)
        return scripts
    ...
    scripts[display_on_oid] = {
        ...
        "variables": _marker("display_on", None, cluster.area, room_slug(cluster)),
        "sequence": [{
    ...
    def _wake_source_steps(src):
        ...
        remote_eid = (_paired_remote(client, src.entity)
                      if profile["method"] == "remote_reconnect" else "")
        if profile["method"] == "remote_reconnect" and not remote_eid:
            # REGISTER 447: the platform shows no remote for this device —
            # no step is written against a guessed id. The wake is the plain
            # media_player call and the script says why.
            print("  [generator] %s: no remote on the platform for %s — wake by "
                  "media_player only" % (cluster.area, src.entity), flush=True)
        if profile["method"] == "remote_reconnect" and remote_eid:
            wake = [
    ...
    for src in cluster.sources:
        ...
        # ── Sleep the sibling sources ────────────────────────────────────────
        # Watching THIS source means the others go to rest — one thing on at a
        # time is what an activity IS. This went unnoticed for as long as the
        # Apple TV wake was broken (two sources could never be up together);
        # the moment the wake worked, Watch Apple TV left the Shield running
        # beside it. Guarded on the sibling actually being awake, sleeps via
        # each one's certified method, and runs LAST: it's the invisible tail,
        # and continue_on_error so a stubborn sibling can't break the watch.
        for other in cluster.sources:
            if other.entity == src.entity:
                continue
            # The room's AUDIO DEVICE is infrastructure, never a sibling. When
            # the AVR is also committed as a source (its own tuner/HEOS), the
            # rest below would switch the amp on and kill it in the SAME
            # script — measured live 1 Aug 2026: the Marantz power-cycled on
            # every watch (102 recorder transitions), the media card flapped
            # between the volume and plain cards, and the verdict held on the
            # contradictory evidence. TV Off still powers it down — that is
            # the audio plan's job, not sibling logic.
            if other.entity == (audio or {}).get("entity"):
                continue
            other_label = cluster.label_for(other)
            _rem = _paired_remote(client, other.entity) if other.integration in _REMOTE_SLEEP else ""
            if _rem:
                sleep_step = {"action": "remote.turn_off",
                              "target": {"entity_id": _rem},
                              "continue_on_error": True}
            else:
                sleep_step = {"action": "media_player.turn_off",
                              "target": {"entity_id": other.entity},
                              "continue_on_error": True}
            seq.append({
                "alias": f"Rest {other_label} (not being watched)",
                "if": [{"condition": "template", "value_template":
                        "{{ states('%s') not in ['off','standby','unavailable','unknown'] }}"
                        % other.entity}],
                "then": [sleep_step],
            })
        scripts[oid] = {
            ...
            "variables": {**_marker("watch_source", audio, cluster.area, room_slug(cluster)), "proos_source": src.entity},
            "sequence": seq,
        }
    ...
    if cluster.display_is_source:
        ...
        scripts[oid] = {
            ...
            "variables": _marker("watch_tv", audio, cluster.area, room_slug(cluster)),
            "sequence": seq,
        }
    ...
    seq = []
    for src in cluster.sources:
        _rem = _paired_remote(client, src.entity) if src.integration in _REMOTE_SLEEP else ""
        if _rem:
    ...
    scripts[oid] = {
        ...
        "variables": _marker("tv_off", None, cluster.area, room_slug(cluster)),
        "sequence": seq,
    }
