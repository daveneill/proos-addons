...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    for src in cluster.sources:
        ...
        verify = []
        # Source wake -- correct command per platform (Apple TV / Android TV via the
        # paired remote.turn_on; others via media_player.turn_on).
        wake_action, wake_ent = _source_wake(src)
        burst.append({"alias": "Wake %s" % label, "action": wake_action, "target": {"entity_id": wake_ent}})
        # Confirm it woke; re-wake if not; SURFACE an incident if it never woke -- so a
        # source that fails to turn on is visible, not silent (exactly what this exists for).
        # Confirm on the WAKE entity: Apple TV / Android TV wake via their remote, and on
        # tvOS 26 the remote reports 'on' when awake while the media_player lags/stays 'off'
        # -- so checking the media_player would raise a FALSE "never woke". The remote is the
        # truthful signal; a media_player-wake source confirms on its own state.
        if src.integration in _REMOTE_SLEEP:
            alive = "is_state('%s','on')" % wake_ent
        else:
            alive = "not is_state('%s','off') and not is_state('%s','unavailable')" % (src.entity, src.entity)
        verify.append({
            "alias": "Confirm %s awake (re-wake if not)" % label,
            "repeat": {"sequence": [
                {"wait_template": "{{ %s }}" % alive, "timeout": {"seconds": 3}, "continue_on_timeout": True},
                {"if": [{"condition": "template", "value_template": "{{ not (%s) }}" % alive}],
                 "then": [{"action": wake_action, "target": {"entity_id": wake_ent}}]},
            ], "until": [{"condition": "template", "value_template": "{{ %s or repeat.index >= 2 }}" % alive}]},
        })
        verify.append({
            ...
            "then": [{"action": "logbook.log", "data": {
                ...
                "message": "%s did not wake for this activity -- check the device / integration" % label,
