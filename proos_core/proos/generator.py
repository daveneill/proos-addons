...
def _wake_display_steps(client, disp, timeout=15):
    """Bring the display up on a LIVE input.

    CONFIRMED BY THE INSTALLER (eyes on the panel) for this Samsung Frame:
      * art switch OFF -> turns the TV ON, onto a live input   <-- this IS the wake
      * art switch ON  -> puts the TV into Art Mode            <-- this is the rest (tv_off)
    The media_player is NOT a reliable live-input signal: it reports 'on' while the panel
    is still showing artwork (verified), so gating the wake on it passes instantly and the
    input then fires at an art screen. We therefore drive AND confirm on the ART switch,
    which is the authoritative control for this display. A non-Frame display (no art
    switch) just powers on via media_player and waits for 'on' as before."""
    art = _art_mode_switch(client, disp)
    if art:
        return [
            {"alias": "Turn the TV on (Art Mode off = TV on)", "action": "switch.turn_off",
             "target": {"entity_id": art}, "continue_on_error": True},
            {"alias": "Wait until out of Art Mode (on a live input)",
             "wait_template": "{{ is_state('%s','off') }}" % art,
             "timeout": {"seconds": timeout}, "continue_on_timeout": True},
        ]
    return [
        {"alias": "Turn on display", "action": "media_player.turn_on",
         "target": {"entity_id": disp}, "continue_on_error": True},
        {"alias": "Wait until on", "wait_template": "{{ is_state('%s','on') }}" % disp,
         "timeout": {"seconds": timeout}, "continue_on_timeout": True},
    ]
