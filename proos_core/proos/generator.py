...
def build_room_scripts(client, cluster, commissioning=None) -> dict:
    ...
    def _wake_source_steps(src):
        ...
        if src.integration in _REMOTE_SLEEP:
            # Certified reconnect wake: the paired remote re-establishes the connection,
            # then power on -- guarded so an already-awake device is not disturbed. All
            # standard actions plus one delay; no loop.
            remote_eid = _paired_remote(client, src.entity)
            # A sleeping source reports "standby", not "off" — guarding on
            # 'off' alone skipped the wake entirely (TV on, input right, box
            # still dark; the exact "have to grab the real remote" failure).
            # And a deep-sleep box can accept the reconnect yet keep its video
            # output down, so after waking we CONFIRM it left standby and, if
            # not, send the integration's explicit wake keypress — the same
            # thing pressing the physical remote's button does.
            wake_cmd = "WAKEUP" if src.integration == "androidtv_remote" else "wakeup"
            asleep = ["off", "standby"]
            ...
            return [{
                ...
                "then": [
                    ...
                    {"alias": f"Confirm {label} is awake", "wait_template":
                     ...
                     "continue_on_timeout": True},
                    {"alias": f"Press wake if it still hasn't come up",
                     "if": [{"condition": "template", "value_template": "{{ %s }}" % still}],
                     "then": [{"action": "remote.send_command",
                               "target": {"entity_id": remote_eid},
                               "data": {"command": wake_cmd},
                               "continue_on_error": True}]},
                ],
            }]
        return [{"alias": f"Turn on {label}", "action": "media_player.turn_on",
