...
def _no_empty_thinking(m):
    """A stored message with an EMPTY thinking block in it, healed.

    Register 492. Register 487's streaming reader dropped thinking_delta, so
    every thinking block was stored with nothing in it — and the API refuses an
    empty one the moment that message is sent back ("each thinking block must
    contain thinking"). The reader is fixed at its source, but the sessions
    already written to disk still carry the empty blocks, and a session is kept
    across restarts (register 426). Without this, the fix would heal new
    conversations and leave every existing one refusing EVERY message until its
    window happened to roll past the damage — which is exactly the bricked
    session register 467 was written about, in a new costume.

    The empty block is dropped, not repaired: a thinking block's content and its
    signature are the model's, and Core does not invent either. Everything else
    in the message — the text, the tool call — is untouched, and a message is
    never emptied by this, because a turn always ends in text or a tool call.
    """
    c = m.get("content")
    ...
    if len(kept) == len(c):
        return m
    if not kept:
        return m                   # never empty a message; leave it as it was
    return dict(m, content=kept)
...
def _safe_trim(msgs, limit):
    ...
    """
    out = [_no_empty_thinking(m) for m in list(msgs)[-limit:] if isinstance(m, dict)]
...
TOOLS = [
    ...
    {"name": "room_media",
     ...
     "input_schema": {"type": "object", "properties": {
         ...
         "required": ["area_id", "action"]}},
    {"name": "room_group",
     "description": "GROUP ROOMS SO THEY PLAY AS ONE, or release them — the same music, in step, "
                    "across several rooms. Use this for 'play it everywhere', 'put the kitchen "
                    "with the office', 'group them', 'ungroup the bedroom', and whenever someone "
                    "asks for music in more than one room: starting the same thing separately in "
                    "each room is NOT the same answer, it drifts out of time. "
                    "host is the room whose player leads (the one already playing, if any); rooms "
                    "are the rooms joining it or leaving it. action: join | unjoin | status. "
                    "status answers 'is it grouped' for a room and needs only host. Works for the "
                    "home's native speakers and for the music engine's players alike, because it "
                    "is the platform's own grouping — the same one the homeowner app's group "
                    "sheet uses.",
     "input_schema": {"type": "object", "properties": {
         "host": {"type": "string", "description": "area_id or room name — the room that leads"},
         "rooms": {"type": "array", "items": {"type": "string"},
                   "description": "area_ids or room names joining or leaving"},
         "action": {"type": "string", "description": "join | unjoin | status (default join)"}},
         "required": ["host"]}},
    {"name": "usage_history",
...
class ToolRunner:
    ...
    def t_room_group(self, args):
        """ROOMS THAT PLAY AS ONE — the PLATFORM's grouping, not a ProOS idea.

        Register 516, 19 Sep 2026. Dave: "I asked it to play triple m 80s in all
        the rooms and it just played the same in every room, not one room and
        then grouped it, and when I asked it to group it it said it does not
        have control of grouping or zoning — we have that I believe with the
        native audio players and ProOS Music, can you confirm."

        CONFIRMED, off his own box, before this was written:

            media_player.office       supported_features 8322623  (GROUPING set)
            media_player.family_room  group_members [ryans_room, family_room]

        — two rooms grouped at that moment. And the homeowner app has grouped
        rooms since long before this: dashboard.html's group sheet calls
        media_player.join / unjoin directly. THE PRODUCT COULD ALWAYS DO IT.
        ASSIST HAD NO VERB FOR IT, so it told the truth about itself and the
        person heard "ProOS cannot do this" about something ProOS does.

        So this adds no mechanism (register 495: if the platform already does
        it, use the platform). It is the same two service calls the glass has
        always made, reachable by speech, and the answer is READ BACK from
        group_members rather than assumed (register 446)."""
        act = str(args.get("action") or "join").strip().lower()
        if act not in ("join", "unjoin", "status"):
            return {"error": "action must be join | unjoin | status"}
        host_area = self._resolve_area_id(args.get("host") or args.get("area_id"))
        if not host_area:
            return {"error": "host (a room name or area_id) required"}
        host_t, _ctx = _room_vol_targets(self, host_area)
        if not host_t:
            return {"message": "There is no speaker committed in that room, so "
                               "there is nothing to group."}
        host = host_t[0]

        def _read(eid):
            try:
                a = ((self.client.snapshot([eid]) or {}).get(eid) or {}).get("attributes") or {}
                m = a.get("group_members")
        ...
        if act == "status":
            members = _read(host)
            self.actions.append({"tool": "room_group", "action": "status",
                                 "host": host, "members": members})
            return {"ok": True, "host_area": host_area, "host": host,
                    "grouped": len(members) > 1, "members": members}

        # THE ROOMS, RESOLVED THE SAME WAY THE HOST WAS. A room this box does
        # not know, or one with no speaker, is NAMED back rather than silently
        # dropped -- "I grouped three of the four" is a lie by omission.
        want, missing = [], []
        for r in (args.get("rooms") or []):
            aid = self._resolve_area_id(r)
            t, _c = _room_vol_targets(self, aid) if aid else ([], None)
            if aid and t:
                if t[0] != host:
                    want.append(t[0])
            else:
        ...
        if not want:
            return {"error": "no rooms with a speaker to %s" % act,
                    "not_found": missing} if missing else                    {"message": "No other rooms were named."}

        before = _read(host)
        try:
            if act == "join":
                self.client.call_service("media_player", "join", host,
                                         {"group_members": want})
            else:
                for e in want:
                    self.client.call_service("media_player", "unjoin", e, None)
        except Exception as e:                                   # noqa: BLE001
            return {"error": "the platform refused to %s: %s" % (act, str(e)[:160])}

        # READ IT BACK. Sonos takes a few seconds to settle -- the homeowner
        # app's own comment says so -- so this waits the way readback does
        ...
        for _i in range(12):
            ...
            members = _read(host)
            joined = set(members)
            if act == "join" and all(e in joined for e in want):
                break
            if act == "unjoin" and not any(e in joined for e in want):
                break
        ok = (all(e in set(members) for e in want) if act == "join"
              else not any(e in set(members) for e in want))
        self.actions.append({"tool": "room_group", "action": act, "host": host,
                             "asked": want, "members": members, "confirmed": ok})
        out = {"ok": True, "action": act, "host_area": host_area, "host": host,
               "members": members, "confirmed": ok}
        if missing:
            out["not_found"] = missing
        if not ok:
            out["note"] = ("the platform accepted it but the players do not "
                           "show it yet — read group_members again in a moment")
        return out
...
def _music_doctrine(engine: bool) -> str:
    ...
    # REGISTER 516. Dave asked for a station "in all the rooms" and got the same
    # thing started separately in each one, which drifts out of time, and then
    # "I don't have grouping" -- about something the homeowner app has done for
    # months. The verb exists now; the model has to know that MORE THAN ONE ROOM
    # IS A GROUPING QUESTION, not a loop.
    many = ("MORE THAN ONE ROOM IS ONE ANSWER, NOT SEVERAL. 'Everywhere', 'all "
            "the rooms', 'in here and the kitchen' — start it in ONE room and "
            "room_group the others to it. Starting the same music separately in "
            "each room is not the same thing: it drifts out of time within "
            "seconds and it is what a person means when they say it isn't "
            "grouped. room_group is the platform's own grouping and works for "
            "the home's native speakers and the engine's players alike. Never "
            "say grouping or zoning cannot be done here.\n")
    if not engine:
            ...
            with urllib.request.urlopen(req, timeout=_HTTP_TIMEOUT) as r:
                ...
                for raw in r:
                    ...
                    if kind == "message_start":
                        ...
                        # ── THE MODEL'S THINKING IS PART OF THE MESSAGE ──────
                        # REGISTER 492. This reader handled text and tool
                        # arguments and silently dropped these two, so every
                        # thinking block was stored EMPTY — and the API refuses
                        # an empty one the moment that message is sent back:
                        # "each thinking block must contain thinking". Every
                        # turn that used a tool died on its second round, which
                        # is every turn that does anything. The signature rides
                        # with it: a thinking block without one is not valid
                        # either, and it is not something Core may invent.
...
# THE MUSIC ENGINE'S NAME IN THIS PRODUCT, exported (register 557) so another
# module can ask for it WITHOUT writing the supplier's name to do the asking.
# ma.py needs it to name the engine on the network at commissioning; spelling
# product_words("<supplier>") there would have put the supplier's brand into a
# served string in ma.py, which platform_words_bench forbids -- and it caught
# exactly that. Derived from the mapping above, never typed twice.
MUSIC_PRODUCT_NAME = _PRODUCT_WORDS[0][1]
...
def _say_key(tool, args):
    """WHICH ACT THIS WAS, for deciding whether the home has already said it.

    Register 511. The tool plus every argument it was called with, as a
    comparable tuple. Deliberately NOT a list of argument names I think matter:
    a key that has to be kept in step with five verbs' arguments is a key that
    will be out of step the first time one gains an option, and the cost of
    being out of step here is the home saying the same thing twice.

    It is a module-level function, not a closure inside chat(), for one reason:
    a closure cannot be benched without standing up a model and a whole turn.
    This can be called directly, so the rule it enforces is checked rather than
    described.
    """
    return (tool, tuple(sorted((str(k), str(v))
...
def chat(client, ws_call, project_mod, user: dict, text: str,
    ...
    def _on_act(tool, args):
        # REGISTER 512: the model's road asks the same question the fast road
        # asks -- was this room spoken about moments ago -- so a person gets
        # "On." either way. One phrasebook means one set of words AND one rule
        # about when the room's name is worth saying.
        _aid = (args or {}).get("area_id")
        key = _say_key(tool, args)
        # ONE ACT GETS ONE SENTENCE, AND IT IS COMPOSED ONCE. Found by a bench,
        # not by reading: with a set of forms to choose from, the fast road
        # composed its answer at resolve time and this composed AGAIN when the
        # tool ran -- so the home SAID "Lights off in the Office." and the
        # answer on the glass read "Office lights, off." Register 498's law is
        # that on that road the answer IS the sentence already spoken; two
        # draws of a dice broke it the moment there was more than one face.
        # It is composed FIRST and the preset only replaces the wording. The
        # preset must never CREATE speech: a room this box cannot name still
        # gets no sentence here, even when the fast road found a name in where
        # the person is standing. That guard is register 488's and it is not
        # this build's to relax.
        line = _direct.say_for(tool, args, _say_rooms.get(_aid),
                               brief=_direct.brief_for(_aid))
        if line:
            line = runner.said_for_act.get(key, line)
            runner.said_for_act[key] = line
        # A room this box does not know by name gets no sentence: "the room is
        # off" is not something a person can check.
        if not line:
            return
        # REGISTER 511 -- THE GUARD LANDS BEFORE THE THING IT GUARDS AGAINST.
        # This remembered THE SENTENCE, and that was safe only while one act
        # could produce exactly one fixed sentence. The personality work makes
        # the phrasebook vary its wording ("Office lights, on." one time, "On."
        # the next), and two spellings of ONE act stop matching -- so both are
        # spoken. That is register 497's double-speak arriving by a new door,
        # and this is written now, in its own build, rather than after the
        # first varied phrase has already been heard.
        #
        # BOTH CHECKS STAY, and that is deliberate. Two DIFFERENT acts can
        # honestly produce the same words -- pausing two rooms both say
        # "Paused." -- and today that is said once. Dropping the sentence check
        # would make it twice. Keeping both can only ever suppress MORE than
        # today, never less, which is the only direction this may move in.
        key = _say_key(tool, args)
        if line in runner.spoken or key in _said_acts:
            return
        _said_acts.add(key)
        _direct.spoke_of(_aid)       # only now has the room's name been SAID
    ...
    if _hit:
        ...
        if _err:
            ...
            _direct.spoke_of((_hit.get("args") or {}).get("area_id"))
        ...
        # ── AND IT DOES NOT SAY IT TWICE (register 498) ─────────────────────
        # Dave, 19 Sep: "the words show darker, it says the office lights are
        # off, then the words turn white and it says the office lights are off
        # again." Register 488 gave the MODEL road spoken_already and
        # speak_suppress on the way out; THIS road returns from here, hundreds
        # of lines earlier, and carried neither -- so the page read "not
        # suppressed", which was never true, and said the answer a second time.
        #
        # IT IS SUPPRESSED WITHOUT A THRESHOLD, and that is the difference from
        # the model road. There, length decides, because a long closing line
        # means the model found something worth telling. NOTHING IS COMPOSED
        # HERE: the answer is _hit["say"], and the act-time confirmation is the
        # same phrasebook entry (register 489), so they are not similar, they
        # are THE SAME STRING. There is no wording in which saying it again is
        # anything but a stutter.
        #
        # A FAILED ACT NEVER SPOKE (register 488 gates on "not error"), so
        # _said is empty, nothing is suppressed, and the person still hears why.
        _said = list(getattr(runner, "spoken", None) or [])
        _suppress = bool(_said)
