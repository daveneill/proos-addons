...
TOOLS = [
    ...
    {"name": "get_states",
     ...
     "input_schema": {"type": "object", "properties": {
         ...
         "required": ["entity_ids"]}},
    {"name": "app_launch",
     "description": "Open a streaming app (Netflix, Disney+, YouTube…) in a room — fully "
                    "self-contained: if the room is off it AUTOMATICALLY runs the watch activity, "
                    "waits for devices to wake, then launches (may take ~20-30s; no need to power "
                    "anything first). Call with area_id + app: one capable device → it launches; "
                    "several → {needs_choice, options:[{entity_id,name,asleep}]} — ASK the user "
                    "which (options marked asleep will be woken on pick), then call again with "
                    "`device` set. NEVER claim an app doesn't exist on a sleeping device. If the "
                    "app truly isn't anywhere you get the available list — say so, don't guess. "
                    "Verify after.",
     "input_schema": {"type": "object", "properties": {
         "area_id": {"type": "string"},
...
def _system_prompt(user: dict, home_name: str) -> str:
    ...
    return (
        ...
        "Disney+, YouTube…) just call app_launch — it wakes the room itself if needed (takes a few "
        "seconds; tell the user you're getting the room ready). If it returns needs_choice ASK "
        "which device, then call again with device set — never pick for them.\n"
...
def _safe_trim(full: list) -> list:
    """Trim the session window WITHOUT orphaning tool exchanges. A naive slice
    can cut an assistant tool_calls message while keeping its role:'tool'
    replies (OpenAI) or an assistant tool_use while keeping the user
    tool_result blocks (Claude) — the provider then 400s ('messages with role
    tool must be a response to a preceding message with tool_calls', observed
    live). After slicing, drop leading messages until the history starts with a
    plain user text message."""
    out = list(full)[-_MAX_TURNS:]
    def _is_plain_user(m):
        if not isinstance(m, dict) or m.get("role") != "user":
            return False
        c = m.get("content")
        if isinstance(c, str):
            return True
        if isinstance(c, list):   # Claude content blocks — tool_result = not plain
            return not any(isinstance(b, dict) and b.get("type") == "tool_result" for b in c)
        return False
    while out and not _is_plain_user(out[0]):
        out.pop(0)
    return out
