...
def _active_tools(runner) -> list:
    ...
    if runner.user.get("is_owner"):
        ...
        mirror = [t for t in runner._mcp_list() if t.get("read_only")]
    else:
        mirror = []
...
class ToolRunner:
    ...
    def _run_mcp(self, tool, args):
        ...
        elif not tool.get("read_only") and not self.user.get("is_owner"):
            out = {"error": "that tool ACTS on the home — Developer only"}
        else:
