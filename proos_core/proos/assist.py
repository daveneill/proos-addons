...
_OPENAI_NO_REASONING: set = set()   # models the provider told us need reasoning_effort 'none' with tools
...
def _chat_openai(cfg, system, history, runner):
    ...
    for _ in range(_MAX_TOOL_ROUNDS):
        ...
        if model in _OPENAI_NO_REASONING:
            body["reasoning_effort"] = "none"
        try:
            resp = _http_json("https://api.openai.com/v1/chat/completions", body, hdrs)
        except RuntimeError as e:
            # REGISTER 443 (13 Sep 2026, Dave: "my assist has stopped"). The
            # provider refused every turn: "Function tools with
            # reasoning_effort are not supported for gpt-6-astra in
            # /v1/chat/completions … set reasoning_effort to 'none'." Core
            # sent no reasoning_effort; the provider's default for that
            # model is what it rejects. The provider's own sentence is the
            # instruction: when a 400 names reasoning_effort, send it as
            # 'none' — once per model per process, no model table.
            if ("HTTP 400" in str(e) and "reasoning_effort" in str(e)
                    and model not in _OPENAI_NO_REASONING):
                _OPENAI_NO_REASONING.add(model)
                print("  [assist] %s: the provider wants reasoning_effort 'none' "
                      "with tools — sending it from now on" % model, flush=True)
                body["reasoning_effort"] = "none"
                resp = _http_json("https://api.openai.com/v1/chat/completions", body, hdrs)
            else:
                raise
...
def test_provider() -> dict:
    ...
    try:
        if cfg["provider"] == "claude":
            ...
            # max_completion_tokens, not max_tokens: current OpenAI models
            # refuse the old name with HTTP 400 (Dave's Test button on
            # gpt-6-astra, 13 Sep 2026, register 433). The chat path never
            # sent either; only this ping did, so only the Test broke.
