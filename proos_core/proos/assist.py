...
TOOLS = [
    ...
    {"name": "scenes_list",
     ...
     "input_schema": {"type": "object", "properties": {}, "required": []}},
    {"name": "scene_create",
     "description": "Create OR update a scene: a saved set of device states you can recall in one "
                    "go (e.g. 'Movie Night' = TV on, lights 15%). states is a list of "
                    "{entity_id, state, attributes}. Pass the same name to update. photo_query is a "
                    "short vivid description of the scene's MOOD (e.g. 'dim cinema room, warm glow') "
                    "— it's matched to a real photo for the dashboard, so make it evocative. After "
                    "creating, apply and verify (the test loop). Reuse committed member ids from "
                    "rooms_overview; never AV power here (use activities).",
     "input_schema": {"type": "object", "properties": {
         "name": {"type": "string"},
         "states": {"type": "array", "items": {"type": "object", "properties": {
             "entity_id": {"type": "string"},
             "state": {"type": "string"},
             "attributes": {"type": "object", "description": "e.g. brightness_pct, color_temp, source"}},
...
class ToolRunner:
    ...
    def t_scenes_list(self, args):
        ...
        for s in states:
            eid = s.get("entity_id", "")
            if eid.startswith("scene.proos_assist_"):
    ...
    def t_scene_create(self, args):
        ...
        for st in states:
            ...
            if "." not in e:
                continue
            if e.startswith("media_player.") and (st.get("state") in ("on", "off")):
                return {"error": "media/TV power belongs in a room activity, not a scene — leave %s out" % e}
            ent = {"state": st.get("state")}
            for k, v in (st.get("attributes") or {}).items():
                ent[k] = v
            entities[e] = ent
        if not entities:
            return {"error": "no valid entities"}
        sid = self._slugify(name, "proos_assist")
        seid = "scene.%s" % sid
        ...
        # Match a photo to the scene's mood (or its name) so the dashboard shows a
        # fitting image the moment it's pinned — not name-keyword guesswork.
        try:
            from . import scenephotos
            scenephotos.set_photo(seid, photo=scenephotos.match(args.get("photo_query") or name))
        ...
        return {"ok": True, "scene_entity_id": seid, "name": name,
    ...
    def t_scene_photo(self, args):
        ...
        try:
            from . import scenephotos
            res = scenephotos.set_photo(eid, photo=scenephotos.match(q))
        except Exception as e:  # noqa: BLE001
            return {"error": str(e)}
        self._audit("scene_photo", entity=eid)
        return {"ok": True, "scene_entity_id": eid, "photo": (res.get("record") or {}).get("photo")}
    ...
    def t_scene_dashboard(self, args):
        ...
        return {"ok": True, "scene_entity_id": eid, "on_dashboard": bool(show),
                "note": "the dashboard picks a photo from the scene name"}

    def t_scene_delete(self, args):
        eid = (args.get("scene_entity_id") or "").strip()
        oid = eid.split(".", 1)[1] if "." in eid else eid
        if not oid.startswith("proos_assist_"):
            return {"error": "can only delete scenes ProOS Assist created (id must start proos_assist_)"}
        try:
            self.client._req("DELETE", "/api/config/scene/config/%s" % oid)
            self.client._req("POST", "/api/services/scene/reload", {})
        except Exception as e:  # noqa: BLE001
            return {"error": str(e)}
...
def _system_prompt(user: dict, home_name: str) -> str:
    ...
    return (
        ...
        "5. SCENES: scene_create saves a set of device states; ALWAYS follow the create-test "
        "loop — create, scene_apply, then verify the entities really reached those states, and "
        "adjust if not, before telling the user it's ready. THEN ask if they'd like it on their "
