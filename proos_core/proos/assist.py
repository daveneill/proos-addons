...
TOOLS = [
    ...
    {"name": "scene_create",
     "description": "Create OR update a scene: a saved set of device states you can recall in one "
                    ...
                    "{entity_id, state, attributes}. Pass the same name to update. After creating, "
                    "apply it and verify — that's the test loop. Reuse committed members' ids from "
                    "rooms_overview; never AV power here (use activities for that).",
     "input_schema": {"type": "object", "properties": {
         "name": {"type": "string"},
...
class ToolRunner:
    ...
    def t_scene_create(self, args):
        ...
        self._audit("scene_create", name=name, entity="scene.%s" % sid, entities=len(entities))
        return {"ok": True, "scene_entity_id": "scene.%s" % sid, "name": name,
                "next": "apply it with scene_apply, then verify the entities reached these states"}
...
def _system_prompt(user: dict, home_name: str) -> str:
    ...
    return (
        ...
        "adjust if not, before telling the user it's ready. Keep AV power out of scenes (use "
        "activities). automation_create builds time/state automations (installer/tech only); test "
        "with automation_trigger then verify.\n"
