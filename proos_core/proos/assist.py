    ...
    {"name": "scene_create",
     ...
     "input_schema": {"type": "object", "properties": {
         ...
         "remove_entities": {"type": "array", "items": {"type": "string"},
             "description": "UPDATE only: entity_ids to drop from the scene"},
...
def _attr_close(key, got, want) -> bool:
    ...
    quantise: ask for brightness 77 and a lamp may report 76 or 78; a volume of
    0.25 may land at 0.2496. Exact matching turns working devices into reported
    failures."""
    if got == want:
        return True
    try:
...
def apply_scene(client, ws_call, project_mod, ma, scene_entity_id: str,
    ...
    """Apply a scene AND its music companion, if it has one. The single apply
    path: the chat tool and the dashboard's scene tap both land here, so a
    moment behaves identically wherever it's triggered."""
...
class ToolRunner:
    ...
    def t_scene_create(self, args):
        ...
        # A pure removal is a legal update: scene_entity_id + remove_entities,
        # no new states. Everything else needs name + states.
        ...
        try:
            ...
            pass
        # Music companion: the half of a "moment" an HA scene can't hold.
        # Stored in the sidecar keyed by the scene's entity_id; fired by the
        # shared apply path so chat and dashboard taps behave identically.
        music = args.get("music") or {}
        music_note = None
        if isinstance(music, dict) and (music.get("query") or "").strip():
            sm = _scene_music_load()
            rec2 = {"area_id": (music.get("area_id") or "").strip(),
                    "query": music.get("query").strip()}
            v = music.get("volume")
            ...
            if not rec2["area_id"]:
                ...
                music_note = "music attached: '%s' will start in that room whenever the scene runs" % rec2["query"]
