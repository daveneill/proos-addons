...
def resolve_scene_photo(name: str, mood: str, slug: str):
    """One place that decides a scene's photo: AI-generate a bespoke image when
    an OpenAI image key is configured (saved to /www, persistent), else fall
    back to matched search / keyword. Returns a URL/path or None."""
    from . import scenephotos
    ik = _image_key(None)
    if ik:
        png = scenephotos.generate(scenephotos.build_prompt(name, mood), ik)
        if png:
            p = scenephotos.save_generated(slug, png)
            if p:
                return p
    return scenephotos.match(mood or name)
...
class ToolRunner:
    ...
    def t_scene_create(self, args):
        ...
        try:
            from . import scenephotos
            photo = resolve_scene_photo(name, args.get("photo_query"), sid)
            if photo:
    ...
    def t_scene_photo(self, args):
        ...
        try:
            ...
            photo = resolve_scene_photo(nm, q, cfg_id)
