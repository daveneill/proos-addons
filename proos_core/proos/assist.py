...
TOOLS = [
    ...
    {"name": "get_states",
     ...
     "input_schema": {"type": "object", "properties": {
         ...
         "required": ["entity_ids"]}},
    {"name": "app_launch",
     "description": "Open a streaming app or input (Netflix, Disney+, YouTube, Apple TV…) on a "
                    "room's display. First make sure the display is ON — run the room's watch "
                    "activity (room_activity) — THEN call this. It matches the app to the display's "
                    "real available sources; if it isn't available you'll get the list of apps that "
                    "ARE, so tell the user honestly rather than guessing. Verify after.",
     "input_schema": {"type": "object", "properties": {
         "area_id": {"type": "string"},
         "app": {"type": "string", "description": "app/source name, e.g. 'Netflix'"}},
...
class ToolRunner:
    ...
    def t_app_launch(self, args):
        """Open a named app/source (Netflix, Disney+, YouTube…) on a room's
        display. Matches the request to the display's real source_list — honest:
        if the app isn't an available source we say what IS, we don't fake it.
        The display should already be ON (run the watch activity first)."""
        aid = self._resolve_area_id(args.get("area_id"))
        app = (args.get("app") or "").strip()
        if not aid or not app:
            return {"error": "area_id and app required"}
        disp = self._room_display(aid)
        if not disp:
            return {"error": "this room has no committed display — set one in the AV activity setup"}
        try:
            st = self.client._req("GET", "/api/states/%s" % disp) or {}
        except Exception as e:  # noqa: BLE001
            return {"error": "couldn't read the display: %s" % e}
        if (st.get("state") or "") in ("off", "unavailable", "standby"):
            return {"error": "the display is %s — run the room's watch activity first, then launch the app"
                             % (st.get("state") or "off"),
                    "display": disp}
        sources = [s for s in ((st.get("attributes") or {}).get("source_list") or []) if isinstance(s, str)]
        want = app.lower().replace("+", " plus").replace("&", " and ")
        match = None
        for s in sources:
            sl = s.lower()
            if sl == app.lower() or sl.replace("+", " plus") == want or want in sl or sl in want:
                match = s
                break
        if not match:
            return {"error": "'%s' isn't an available app on this display" % app,
                    "display": disp,
                    "available_apps": sources[:30],
                    "note": "tell the user which apps ARE available; don't invent one"}
        try:
            self.client._req("POST", "/api/services/media_player/select_source",
                             {"entity_id": disp, "source": match})
        except Exception as e:  # noqa: BLE001
            return {"error": "couldn't launch %s: %s" % (match, e)}
        self._audit("app_launch", app=match, display=disp, area=aid)
        return {"ok": True, "launched": match, "display": disp,
                "next": "verify with get_states that the display's source is now '%s'" % match}
