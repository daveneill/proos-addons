...
class ToolRunner:
    ...
    # -- music (phase 2) ----------------------------------------------------
    def _room_ma_speaker(self, area_id: str):
        """The committed MA player entity for a room — where music plays. Prefer
        a music_assistant-platform speaker among the room's committed audio
        members; fall back to the first committed audio member."""
        proj = self.project.load()
        rec = None
        for k, r in (proj or {}).get("areas", {}).items():
            if r and (r.get("area_id") == area_id or k == area_id) and r.get("committed"):
                rec = r
                break
        if not rec:
            return None
        ...
        for e in audio:
            try:
                p = self.client.render_template("{{ state_attr('%s','mass_player_type') }}" % e)
                if p and p not in ("None", "unknown", ""):
                    return e
            except Exception:
                pass
        # platform check via entity registry
        try:
            reg = {x.get("entity_id"): x for x in (self.client.entity_registry() or [])}
            for e in audio:
                if (reg.get(e) or {}).get("platform") == "music_assistant":
                    return e
        except Exception:
            pass
...
def _system_prompt(user: dict, home_name: str) -> str:
    ...
    return (
        ...
        "5. After any control action, verify and tell the user what ACTUALLY happened; if it "
        "failed, say so plainly and offer the fix.\n"
        "6. Ambiguity → ask one short question instead of guessing.\n"
        "7. Scenes and automations aren't available yet — say that capability is coming if asked.\n"
