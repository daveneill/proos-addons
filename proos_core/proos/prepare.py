...
PREPARE_FACTS = {
    "samsungtv_smart": [
        # AUDITED AGAINST THE LIVE BOX, 1 Aug 2026, after the first render
        # claimed 8 issues on a freshly repaired house. Corrections:
        #  * ip_control is decided by TOKEN PRESENCE in entry data (the fork's
        #    own rule, switch.py), not by an enable_ip_control option — the
        #    option check false-alarmed on all three TVs.
        #  * options are compared as strings ("1" vs 1 false-alarmed WOL).
        #  * the fork's diagnostics export only app_list/channel_list/
        #    ping_port/source_list — an option ABSENT from that view is
        #    UNKNOWN (not visible), never a failure. The product fix is
        #    proos.8: diagnostics must export full options; a certified
        #    integration exposes its preparation surface.
        {"id": "ip_control", "kind": "obs_data", "field": "ip_control_token",
         ...
         "fix": "Configure → enable IP Control → Pair now, accept on the panel"},
        {"id": "art_readback", "kind": "opt", "option": "ip_control_art_mode",
         "want": True, "when": lambda d: bool(d.get("is_frame_tv")),
         "absent": None,
         "absent_note": "not visible in this fork's diagnostics — verify in "
                        "Configure (proos.8 will export it)",
        ...
        {"id": "power_on_wol", "kind": "opt", "option": "power_on_method",
         ...
         "fix": "Configure → set Power On Method to WOL"},
