...
def audit(app_names) -> dict:
    ...
    Turns "why is this one a wordmark" into a finite checklist of files to drop
    in, rather than a guess at what a home might own."""
    have, need = [], []
    seen = set()
    for nm in (app_names or []):
        ...
        (have if tile_path(nm) else need).append(rec)
    have.sort(key=lambda r: r["name"].lower())
    need.sort(key=lambda r: r["name"].lower())
    return {"have": have, "missing": need,
