...
def _canon_index() -> dict:
    """alias (slug or package id) -> (canon_slug, display_name). Built once."""
    global _CANON_INDEX
    if _CANON_INDEX is None:
        idx = {}
        for canon, (name, aliases) in CANONICAL_APPS.items():
            idx[canon] = (canon, name)
            idx[slug(name)] = (canon, name)
            for a in aliases:
                idx[a] = (canon, name)
                idx[slug(a)] = (canon, name)
        _CANON_INDEX = idx
    return _CANON_INDEX
...
def identity_table() -> dict:
    """Flat alias -> {'name', 'slug'} map for the surfaces (GET
    /apps/identity). Dashboards resolve locally with zero further calls."""
    return {a: {"name": n, "slug": c} for a, (c, n) in _canon_index().items()}
