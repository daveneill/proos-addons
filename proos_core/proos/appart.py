...
def list_tiles() -> dict:
    ...
    shipped, uploaded, hid = _ls(_BUNDLED), _ls(_UPLOADED), hidden()
    tiles = []
    for s in sorted(shipped | uploaded):
        if s in uploaded:
            ...
            origin = "shipped"
        tiles.append({"slug": s, "origin": origin,
    ...
    if os.path.isfile(up):
        ...
        if ship:
            return {"ok": True, "slug": s, "action": "reverted",
                    "note": "upload removed — the tile that ships with ProOS is showing again"}
