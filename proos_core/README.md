...
| `reachability` | Per device, an independent liveness signal. List of `{entity, sensor}` — a `ping` binary sensor or a router device tracker the platform holds. An `ip` is the device's address only, never a witness. |
