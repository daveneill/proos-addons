...
# ABSENT means UNSET, and unset means the integration DEFAULT applies (the
# diagnostics export full persisted options; the "not visible" theory was
# wrong). ip_control_art_mode defaults OFF -> a real failure on a Frame.
res = audit_room(record=rec(), snap=snap(), entry=entry(art='absent'))
c = by_id(res, "art_readback")
check("UNSET art option is a real failure (default is off)",
      c and c["ok"] is False, True)
...
check("UNSET power-on method PASSES (integration default is WOL)",
