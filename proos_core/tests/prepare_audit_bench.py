...
def entry(*, frame=True, ip=True, art=True, wol=True, ping=True, app_list=None):
    data = {"is_frame_tv": frame, "ip_control_port": 1516 if frame else 1515,
            "ip_control_token": "tok" if ip is not None else None}
    options = {}
    if ip:
        options["enable_ip_control"] = True
    if art:
        options["ip_control_art_mode"] = True
    if wol:
        options["power_on_method"] = 1
    if ping:
...
# ── 2. THIS WEEK #1: ip_control_art_mode absent on a Frame ─────────────────
res = audit_room(record=rec(), snap=snap(), entry=entry(art=False))
c = by_id(res, "art_readback")
check("missing art option is caught", c and not c["ok"], True)
...
check("IP control disabled is caught", by_id(res, "ip_control")["ok"], False)
