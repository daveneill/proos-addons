...
# ── 19. LIVE-HOUSE REGRESSION (Living Room, 30 Jul 2026) ──────────────────
# The panel is ON and sitting on its OWN committed tuner, no source alive.
# The room's broadcast activity carries an audio witness naming an entity that
# is NOT IN THE SNAPSHOT (a stale/typo'd id from a hard-coded plan). Four days
# of history: watch_tv never fired once, while Family Room and Bedroom — whose
# plans carry no such witness — fired it constantly.
#
# The rule this pins: an audio witness pointing at an entity that does not
# exist is NOT testimony that audio is unrouted. It is a broken plan, and a
# broken plan must not silently pin a lit room to 'off' forever. Absent entity
# == no witness. A witness naming a REAL device still counts (scenario 21).
from proos.activities import make_watch_activity, _audio_confirmed

PHANTOM = {"entity": "media_player.living_room_av_receiver", "source": "TV Audio"}
REAL_AVR = {"entity": AVR, "source": "TV Audio"}

def real_bc(aw):
    b = make_watch_activity(area="Living Room", key="watch_tv", source_label="TV",
                            display_eid=DISP, source_eid=None, audio_witness=aw,
                            competing_eids=[ATV, SHIELD], broadcast=True)
    b.route = {"select_entity": DISP, "select_source": "TV"}
    return b

s19 = ent(ent(S("on", {"source": "TV"}), ATV, "off"), SHIELD, "off")
o19 = decide(AREA, s19, [atv(), shd()], real_bc(PHANTOM), {}, {"last": "off"})
check("panel on its tuner + phantom audio witness still names Live TV",
      o19["state"], "watch_tv")
check("a witness entity absent from the snapshot is not evidence",
      _audio_confirmed(s19, PHANTOM), True)

# ── 20. the same room with no witness at all (Family Room / Bedroom shape) ──
s20 = ent(ent(S("on", {"source": "TV"}), ATV, "off"), SHIELD, "off")
o20 = decide(AREA, s20, [atv(), shd()], real_bc(None), {}, {"last": "off"})
check("no audio plan -> Live TV on tuner evidence alone", o20["state"], "watch_tv")

# ── 21. a REAL witness on the wrong input is still real testimony ──
# The AVR exists and is on the wrong input: that IS evidence about audio. It
# must not be laundered away by the fix above -- but it is also not evidence
# about what is ON SCREEN, and the panel's own tuner is. Live TV stands; the
# audio gate stays advisory for broadcast (there is no source device to
# disambiguate, so the display IS the witness).
s21 = ent(ent(ent(S("on", {"source": "TV"}), ATV, "off"), SHIELD, "off"),
          AVR, "on", {"source": "Phono"})
o21 = decide(AREA, s21, [atv(), shd()], real_bc(REAL_AVR), {}, {"last": "off"})
check("real AVR on the wrong input does not unmake Live TV", o21["state"], "watch_tv")
check("but the audio witness still reports honestly",
      _audio_confirmed(s21, REAL_AVR), False)

# ── 22. and a dark panel is still dark, witness or no witness ──
s22 = ent(ent(S("off"), ATV, "off"), SHIELD, "off")
o22 = decide(AREA, s22, [atv(), shd()], real_bc(PHANTOM), {}, {"last": "watch_tv"})
o22 = decide(AREA, s22, [atv(), shd()], real_bc(PHANTOM), {}, {"last": "watch_tv", "darkn": 2})
check("panel off is still off", o22["state"], "off")
