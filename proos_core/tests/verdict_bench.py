...
# ── 19-22. LIVING ROOM LIVE TV (live house, 30 Jul 2026) ─────────────────────
# Recorder history: ZERO watch_tv verdicts for Living Room across 26-30 Jul,
# while Family Room produced 25+ in the same window. On 30 Jul the panel sat on
# its OWN committed tuner (source: "TV") for 1 min 52 sec with the verdict at
# 'off'. Cause: COMMISSIONING["Living Room"]["audio"] (a hard-coded per-house
# dict in controller.py) names media_player.living_room_av_receiver, which HA
# reports as ENTITY_NOT_FOUND. That became the broadcast activity's audio
# witness, and _audio_confirmed() reads a missing entity as "audio unrouted".
#
# THE RULES BEING PINNED
#  1. A witness naming an entity that is NOT IN THE SNAPSHOT is a broken plan,
#     not evidence of silence. It must never read as 'off'.
#  2. For BROADCAST there is no source device, so the panel's own power and
#     tuner input ARE the evidence. Audio is advisory there -- a receiver left
#     off cannot deny a plainly-lit room its verdict.
#  3. A witness naming a REAL device still reports honestly (rule 1 must not
#     launder that away).
...
def real_bc(aw):
    """A REAL broadcast activity -- not the bench's stubbed bc() -- so the
    audio-witness gate inside summary() is under test."""
...
check("panel on its own tuner + phantom witness still names Live TV",
      o19["state"], "watch_tv")
...
check("no audio plan at all -> Live TV on tuner evidence alone",
      o20["state"], "watch_tv")

s21 = ent(ent(ent(S("on", {"source": "TV"}), ATV, "off"), SHIELD, "off"),
          AVR, "on", {"source": "Phono"})
o21 = decide(AREA, s21, [atv(), shd()], real_bc(REAL_AVR), {}, {"last": "off"})
check("a REAL AVR on the wrong input does not unmake Live TV",
      o21["state"], "watch_tv")
check("but that witness still reports honestly",
      _audio_confirmed(s21, REAL_AVR), False)

s22 = ent(ent(S("off"), ATV, "off"), SHIELD, "off")
o22 = decide(AREA, s22, [atv(), shd()], real_bc(PHANTOM), {},
             {"last": "watch_tv", "darkn": 2})
check("a dark panel is still dark, witness or no witness", o22["state"], "off")
