...
#
# PROOS (15 Sep 2026, ProOS Core register 458): the three provider/player CONFIG
# writes are added. On a ProOS box the platform IS the product: music services are
# added, removed and tuned on ProOS's own installer screen, and every one of those
# calls arrives on the platform's link as `homeassistant_system`. The engine minted
# that user with role "user", so `config/providers/save` answered "22 Admin access
# required" and a service could not be added at all (Dave, live, adding Apple Music).
# ProOS Core used to reach admin by setting the ingress headers itself — pretending
# to be the platform's own proxy — and that forgery was deleted in Core 1.0.638; the
# refusal is the engine being honest, so the permission is granted here, in the open,
# instead. Read from MA 2.9.5's own controllers: these three carry
# required_role="admin"; every other command ProOS uses (music/sync, favourites,
# playlists, queues, all reads) does not.
SYSTEM_USER_ALLOWED_ADMIN_COMMANDS = (
    "players/remove",
    "config/players/remove",
    "auth/users",
    "config/providers/save",      # add or edit a music service
    "config/providers/remove",    # remove one, credentials and all
    "config/players/save",        # a player's own settings (enable/disable, name)
)
